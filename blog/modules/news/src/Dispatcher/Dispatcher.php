...
namespace Joomla\Module\BlogNews\Site\Dispatcher;
...
final class Dispatcher extends AbstractModuleDispatcher
{
    protected function getLayoutData(): array
    {
        ...
        $items = [];
        if ($mode === 'categories') {
            $q = $db->createQuery()->select(['id','title','alias','level'])->from('#__blog_categories')->where('published=1')->whereIn('access', $groups)->where('level>0')->order('lft,title');
            $items = $db->setQuery($q, 0, $limit)->loadObjectList();
            foreach ($items as $item) {
                $item->link = Route::_('index.php?option=com_blog&view=category&id='.$item->id);
            }
        } elseif ($mode === 'tagspopular') {
            $q = $db->createQuery()->select(['t.id','t.title','t.alias','COUNT(DISTINCT m.content_item_id) AS count'])->from('#__blog_tag_map AS m')->join('INNER', '#__blog_tags AS t ON t.id=m.tag_id')->join('INNER', '#__blog AS p ON p.id=m.content_item_id')->where('m.type_alias='.$db->quote('com_blog.post'))->where('t.published=1')->whereIn('t.access', $groups)->where('p.state=1')->whereIn('p.access', $groups)->where('(p.publish_up IS NULL OR p.publish_up <= UTC_TIMESTAMP())')->where('(p.publish_down IS NULL OR p.publish_down >= UTC_TIMESTAMP())')->where('EXISTS (SELECT 1 FROM #__blog_categories c WHERE c.id=p.catid AND c.published=1 AND c.access IN ('.implode(',', $groups).'))')->group(['t.id','t.title','t.alias'])->order('count DESC');
            $items = $db->setQuery($q, 0, $limit)->loadObjectList();
            foreach ($items as $item) {
                $item->link = Route::_('index.php?option=com_blog&view=tags&tag_id='.$item->id);
            }
            ...
            $q = $db->createQuery()->select(['YEAR(created) AS year','MONTH(created) AS month','COUNT(*) AS count'])->from('#__blog')->where('state=1')->whereIn('access', $groups)->group(['YEAR(created)','MONTH(created)'])->order('year DESC, month DESC');
            $items = $db->setQuery($q, 0, $limit)->loadObjectList();
            foreach ($items as $item) {
            ...
            $q = $db->createQuery()->select(['p.id','p.title','p.alias','p.catid','p.summary','p.excerpt','p.body','p.created','p.modified','p.publish_up','p.hits'])->from('#__blog AS p')->where('p.state=1')->whereIn('p.access', $groups);
        ...
        $excerptParams = \Joomla\Component\Blog\Site\Helper\ListExcerptHelper::settings($params);
        \Joomla\CMS\Plugin\PluginHelper::importPlugin('content');
        \Joomla\CMS\Plugin\PluginHelper::importPlugin('blog');
        foreach ($items as $item) {
            $item->readmore = !empty($item->body);
            $item->text = \Joomla\Component\Blog\Site\Helper\ListExcerptHelper::render($item, $excerptParams);
            Factory::getApplication()->triggerEvent('onContentPrepare', ['com_blog.module', &$item, &$excerptParams, 0]);
