...
namespace Joomla\Plugin\Blog\Finder\Extension;
...
/**
 * Smart Search adapter for com_blog.
 *
 * @since  2.5
 */
final class Content extends Adapter implements SubscriberInterface
{
     ...
     * @since  2.5
     */
    protected $type_title = 'Blog Post';
    ...
    {
        // Make sure we're handling com_blog categories.
        if ($event->getExtension() === 'com_blog') {
    ...
    public function onFinderAfterSave(FinderEvent\AfterSaveEvent $event): void
    {
        ...
        if ($context === 'com_blog.post' || $context === 'com_blog.form') {
    ...
    protected function index(Result $item)
    {
        ...
        $item->context = 'com_blog.post';

        // Initialise the item parameters.
        $registry     = new Registry($item->params);
        $item->params = clone ComponentHelper::getParams('com_blog', true);
        ...
        $categories = $this->getApplication()->bootComponent('com_blog')->getCategory(['published' => false, 'access' => false]);
        ...
        $nativeTags = $tagDb->setQuery('SELECT t.title,t.published,t.access,t.language FROM #__blog_tags t INNER JOIN #__blog_tag_map m ON m.tag_id=t.id WHERE m.type_alias=' . $tagDb->quote('com_blog.post') . ' AND m.content_item_id=' . (int) $item->id)->loadObjectList();
        foreach ($nativeTags as $tag) {
            $item->addTaxonomy('Tag', $tag->title, (int) $tag->published, (int) $tag->access, $tag->language);
        }
        // Get content extras.
        Helper::getContentExtras($item);
        Helper::addCustomFields($item, 'com_blog.post');
