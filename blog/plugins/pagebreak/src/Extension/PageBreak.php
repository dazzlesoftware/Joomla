...
namespace Joomla\Plugin\Blog\Pagebreak\Extension;
...
final class PageBreak extends CMSPlugin implements SubscriberInterface
{
    ...
    public function onContentPrepare(ContentPrepareEvent $event)
    {
        ...
        $canProceed = $context === 'com_blog.post';
                ...
                foreach ($text as $key => $subtext) {
                    ...
                    if ($key >= 1) {
                        ...
                        if (isset($match['alt'])) {
                            ...
                            $title = Text::sprintf('PLG_BLOG_PAGEBREAK_PAGE_NUM', $key + 1);
                        }
    ...
    {
        $heading     = $row->title ?? $this->getApplication()->getLanguage()->_('PLG_BLOG_PAGEBREAK_NO_TITLE');
        $input       = $this->getApplication()->getInput();
        ...
        if ($this->params->get('post_index', 1) == 1) {
            $headingtext = $this->getApplication()->getLanguage()->_('PLG_BLOG_PAGEBREAK_POST_INDEX');
        ...
        foreach ($matches as $bot) {
            if (@$bot[0]) {
                ...
                if (@$attrs2['alt']) {
                    ...
                    $title = Text::sprintf('PLG_BLOG_PAGEBREAK_PAGE_NUM', $i);
                }
            } else {
                $title = Text::sprintf('PLG_BLOG_PAGEBREAK_PAGE_NUM', $i);
            }
        ...
        if ($this->params->get('showall')) {
            ...
            $this->list[$i]->title  = $this->getApplication()->getLanguage()->_('PLG_BLOG_PAGEBREAK_ALL_PAGES');
            $this->list[$i]->active = ($limitstart === $i - 1);
        }

        $list = $this->list;
        $path = PluginHelper::getLayoutPath('blog', 'pagebreak', 'toc');
        ob_start();
        include $path;
    ...
    private function createNavigation(&$row, $page, $n)
    {
        ...
        $path = PluginHelper::getLayoutPath('blog', 'pagebreak', 'navigation');
        ob_start();
        include $path;
