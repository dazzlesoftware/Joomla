...
namespace Joomla\Plugin\Blog\Joomla\Extension;
...
final class Joomla extends CMSPlugin implements SubscriberInterface
{
    ...
    public function onContentAfterSave(AfterSaveEvent $event): void
    {
        ...
        foreach ($users as $user_id) {
            if ($user_id != $user->id) {
                ...
                if ($receiver->authorise('core.login.admin') && $receiver->authorise('core.manage', 'com_blog')) {
                    $templateData['url'] = Route::link('administrator', 'index.php?option=com_blog&view=posts&filter[search]=id:' . $post->id, false, $linkMode, true);
                } elseif ($post->state === 1 && \in_array($post->access, $receiver->getAuthorisedViewLevels())) {
                    $templateData['url'] = Route::link('site', 'index.php?option=com_blog&view=post&id=' . $post->id, false, $linkMode, true);
                } elseif ($post->state !== 1 && $receiver->authorise('core.edit', 'com_blog.post.' . $post->id)) {
                    $templateData['url'] = Route::link('site', 'index.php?option=com_blog&view=post&id=' . $post->id, false, $linkMode, true);
                } elseif ($post->state !== 1) {
                ...
                // Send email
                try {
                    $mailer = new MailTemplate('plg_blog_joomla.newpost', $receiver->getParam('admin_language', $this->getLanguage()->getTag()));
    ...
    public function onContentBeforeDelete(BeforeDeleteEvent $event)
    {
        ...
        if (!\in_array($context, ['com_blog.category', 'com_workflow.stage', 'com_workflow.workflow'])) {
            return;
        }
    ...
    public function onSchemaBeforeCompileHead(BeforeCompileHeadEvent $event): void
    {
        ...
        if ($extension === 'com_blog' && $this->params->get('schema_content', 1)) {
    ...
    /**
     * Inject com_blog schemas if needed
     *
     * @param   string    $context  The com_blog context like com_blog.post.5
    ...
    private function injectContentSchema(string $context, Registry $schema)
    {
        ...
        $component = $this->getApplication()->bootComponent('com_blog')->getMVCFactory();
        ...
        if ($view == 'post' && $id > 0) {
            ...
            $additionalSchemas = $cache->get(function ($view, $id) use ($component, $baseId, $app, $db) {
                ...
                $schemaId = $baseId . 'com_blog/' . $view . ($view == 'category' ? '/' . $id : '');
    ...
    {
        $baseId   = Uri::root() . '#/schema/';
        $schemaId = $baseId . 'com_blog/post/' . (int) $post->id;
        ...
        if ($dateValue = \Joomla\Component\Blog\Site\Helper\DateHelper::value($post, $post->params)) {
            [, $dateType] = \Joomla\Component\Blog\Site\Helper\DateHelper::options($post->params);
    ...
    private function checkMenuItemBeforeSave($context, $table, $isNew, $data)
    {
        ...
        if ($table->link !== 'index.php?option=com_blog&view=form&layout=edit') {
            return true;
        }
        ...
        if (isset($params['enable_category']) && $params['enable_category'] === 1 && empty($params['catid'])) {
            $table->setError($this->getApplication()->getLanguage()->_('COM_BLOG_CREATE_POST_ERROR'));
