...
namespace Joomla\Plugin\Blog\Notification\Extension;
...
final class Notification extends CMSPlugin implements SubscriberInterface
{
    ...
    public function onWorkflowAfterTransition(WorkflowTransitionEvent $event)
    {
        ...
        if (empty($userIds)) {
            $this->getApplication()->enqueueMessage($this->getApplication()->getLanguage()->_('PLG_BLOG_NOTIFICATION_NO_RECEIVER'), 'error');
        ...
        foreach ($pks as $pk) {
            ...
            $title = $this->getApplication()->getLanguage()->_('PLG_BLOG_NOTIFICATION_NO_TITLE');
            ...
            foreach ($userIds as $user_id) {
                ...
                if ($receiver->authorise('core.manage', 'com_messages')) {
                    ...
                    $lang->load('plg_blog_notification');
                    $messageText = \sprintf(
                        $lang->_('PLG_BLOG_NOTIFICATION_ON_TRANSITION_MSG'),
                        $title,
                    ...
                    $message = [
                        ...
                        'subject'    => \sprintf($lang->_('PLG_BLOG_NOTIFICATION_ON_TRANSITION_SUBJECT'), $title),
                        'message'    => $messageText,
                    ];
        ...
        $this->getApplication()->enqueueMessage($this->getApplication()->getLanguage()->_('PLG_BLOG_NOTIFICATION_SENT'), 'message');
    }
