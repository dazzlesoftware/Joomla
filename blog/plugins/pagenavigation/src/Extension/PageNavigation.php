...
namespace Joomla\Plugin\Blog\Pagenavigation\Extension;
...
final class PageNavigation extends CMSPlugin implements SubscriberInterface
{
    ...
    public function onContentBeforeDisplay(BeforeDisplayEvent $event)
    {
        ...
        if ($context === 'com_blog.post' && $view === 'post' && $params->get('show_item_navigation')) {
            $db         = $this->getDatabase();
            ...
            $query->select($db->quoteName(['a.id', 'a.title', 'a.catid', 'a.language']))
                ...
                ->join('LEFT', $db->quoteName('#__blog_categories', 'cc'), $db->quoteName('cc.id') . ' = ' . $db->quoteName('a.catid'));
            ...
            if ($row->prev || $row->next) {
                ...
                $path = PluginHelper::getLayoutPath('blog', 'pagenavigation');
