...
namespace Joomla\Component\Blog\Api\View\Posts;
...
use Joomla\Component\Blog\Api\Helper\ContentHelper;
use Joomla\Component\Blog\Api\Serializer\ContentSerializer;
...
'created',
    ...
    {
        foreach (FieldsHelper::getFields('com_blog.post') as $field) {
    ...
    protected function prepareItem($item)
    {
        ...
        Factory::getApplication()->triggerEvent('onContentPrepare', ['com_blog.post', &$item, &$item->params]);

        foreach (FieldsHelper::getFields('com_blog.post', $item, true) as $field) {
        ...
        if (!empty($item->tags->tags)) {
            ...
            $item->tags = [];
            $tags       = new TagsHelper();
            $tagsIds    = $tags->getTagIds($item->id, 'com_blog.post');
