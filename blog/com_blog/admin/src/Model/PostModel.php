...
namespace Joomla\Component\Blog\Administrator\Model;
...
use Joomla\Component\Blog\Administrator\Event\Model\FeatureEvent;
...
class PostModel extends AdminModel implements WorkflowModelInterface, VersionableModelInterface
{
     ...
     * @since  1.6
     */
    protected $text_prefix = 'COM_BLOG';

    /**
     * The type alias for this content type (for example, 'com_blog.post').
     *
     * @var    string
     * @since  3.2
     */
    public $typeAlias = 'com_blog.post';
     ...
     * @since  3.4.4
     */
    protected $associationsContext = 'com_blog.item';
    ...
    public function __construct($config = [], ?MVCFactoryInterface $factory = null, ?FormFactoryInterface $formFactory = null)
    {
        ...
        $this->setUpWorkflow('com_blog.post');
    }
    ...
    {
        // Check if the post was featured and update the #__blog_frontpage table
        if ($table->featured == 1) {
            $db    = $this->getDatabase();
            $query = $db->createQuery()
                ...
                ->from($db->quoteName('#__blog_frontpage'))
            ...
            if ($featured) {
                $query = $db->createQuery()
                    ->insert($db->quoteName('#__blog_frontpage'))
        ...
        $oldItem->load($oldId);
        $fields = FieldsHelper::getFields('com_blog.post', $oldItem, true);
        ...
        Factory::getApplication()->triggerEvent('onContentAfterSave', ['com_blog.post', &$this->table, false, $fieldsData]);
    }
    ...
    protected function batchMove($value, $pks, $contexts)
    {
        ...
        foreach ($pks as $pk) {
            ...
            $fields = FieldsHelper::getFields('com_blog.post', $this->table, true);
    ...
    protected function canDelete($record)
    {
        ...
        return $this->getCurrentUser()->authorise('core.delete', 'com_blog.post.' . (int) $record->id);
    }
    ...
    protected function canEditState($record)
    {
        ...
        if (!empty($record->id)) {
            return $user->authorise('core.edit.state', 'com_blog.post.' . (int) $record->id);
        }

        // New post, so check against the category.
        if (!empty($record->catid)) {
            return $user->authorise('core.edit.state', 'com_blog.category.' . (int) $record->catid);
        }
    ...
    public function getItem($pk = null)
    {
        if ($item = parent::getItem($pk)) {
            ...
            if (!empty($item->id)) {
                ...
                $item->tags->getTagIds($item->id, 'com_blog.post');
    ...
    public function getForm($data = [], $loadData = true)
    {
        ...
        // Get the form.
        $form = $this->loadForm('com_blog.post', 'post', ['control' => 'jform', 'load_data' => $loadData]);
    ...
    protected function loadFormData()
    {
        ...
        $app  = Factory::getApplication();
        $data = $app->getUserState('com_blog.edit.post.data', []);
        ...
        if (empty($data)) {
            ...
            if ($this->getState('post.id') == 0) {
                $filters     = (array) $app->getUserState('com_blog.posts.filter');
        ...
        $this->preprocessData('com_blog.post', $data);
    ...
    {
        if (!$this->getCurrentUser()->authorise('core.admin', 'com_blog')) {
            if (isset($data['rules'])) {
                unset($data['rules']);
            }
        }
    ...
    public function save($data)
    {
        ...
        if (!empty($data['catid']) && !CategoriesHelper::validateCategoryId((int) $data['catid'], 'com_blog')) {
        ...
        if (parent::save($data)) {
            ...
            try {
                (new \Joomla\Component\Blog\Administrator\Service\AutopostService())->publish(
    ...
    public function featured($pks, $value = 0, $featuredUp = null, $featuredDown = null)
    {
        ...
        try {
            ...
            if ($value === 0) {
                ...
                $query = $db->createQuery()
                    ->delete($db->quoteName('#__blog_frontpage'))
                ...
                if (\count($oldFeatured)) {
                    $query = $db->createQuery()
                        ->update($db->quoteName('#__blog_frontpage'))
                        ->set(
                            [
    ...
    /**
     * Custom clean the cache of com_blog and content modules
     *
    ...
    {
        return $this->getCurrentUser()->authorise('core.create', 'com_blog');
    }

    /**
     * Delete #__blog_frontpage items if the deleted posts was featured
     *
    ...
    public function delete(&$pks)
    {
        ...
        if ($return) {
            // Now check to see if this posts was featured if so delete it from the #__blog_frontpage table
            $db    = $this->getDatabase();
            $query = $db->createQuery()
                ->delete($db->quoteName('#__blog_frontpage'))
                ->whereIn($db->quoteName('content_id'), $pks);
    ...
    protected function batchTags($value, $pks, $contexts, $removeTags = false)
    {
        ...
        $user = $this->getCurrentUser();
        foreach ($pks as $pk) {
            if (!$user->authorise('core.edit', 'com_blog.post.' . (int) $pk)) {
