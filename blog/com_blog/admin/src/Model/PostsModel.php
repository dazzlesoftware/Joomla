...
class PostsModel extends ListModel
{
    ...
    protected function getListQuery()
    {
        ...
        $query->select(
            ...
            ->join('LEFT', $db->quoteName('#__workflow_associations', 'wa'), $db->quoteName('wa.item_id') . ' = ' . $db->quoteName('a.id') . ' AND ' . $db->quoteName('wa.extension') . ' = ' . $db->quote('com_blog.post'))
            ...
            $subQuery = $db->createQuery()
                ...
                ->where(
                    [
                        ...
                        $db->quoteName('asso1.context') . ' = ' . $db->quote('com_blog.item'),
                    ]
                );
        ...
        if ($published !== '*') {
            if (is_numeric($published)) {
                ...
                $query->whereIn(
                    $db->quoteName('a.state'),
                    [
                        BlogComponent::CONDITION_PUBLISHED,
                        BlogComponent::CONDITION_UNPUBLISHED,
                    ]
                );
            }
        }
        ...
        if (is_numeric($authorId)) {
            ...
            if ($authorId === 0) {
                // Only show deleted authors' posts
                $subQuery = $db->createQuery()
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where('(' . $db->quoteName('a.summary') . ' LIKE :search1 OR ' . $db->quoteName('a.body') . ' LIKE :search2)')
                    ->bind([':search1', ':search2'], $search);
        ...
        if (is_numeric($checkedOut)) {
            if ($checkedOut == -1) {
                ...
                // Only not checked out posts (checked_out is 0 or NULL)
        ...
        if ($tag && \is_array($tag)) {
            ...
            if ($includeNone) {
                $subQuery2 = $db->createQuery()
                    ->select('DISTINCT ' . $db->quoteName('content_item_id'))
                    ->from($db->quoteName('#__blog_tag_map'))
                    ->where($db->quoteName('type_alias') . ' = ' . $db->quote('com_blog.post'));
                $query->join(
                    'LEFT',
            ...
            if ($tag === 0) {
                ...
                ->where(
                    [
                        ...
                        $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_blog.post'),
                    ]
                )
    ...
    /**
     * Method to get all transitions at once for all posts
     *
        ...
        try {
            if (\count($stage_ids) || \count($workflow_ids)) {
                ...
                foreach ($transitions as $key => $transition) {
                    if (!$user->authorise('core.execute.transition', 'com_blog.transition.' . (int) $transition['value'])) {
