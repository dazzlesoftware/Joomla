...
class Icon
{
    ...
    public function create($category, $params, $attribs = [], $legacy = false)
    {
        ...
        $url = 'index.php?option=com_blog&task=post.add&return=' . base64_encode($uri) . '&a_id=0&catid=' . $category->id;
    ...
    public function edit($post, $params, $attribs = [], $legacy = false)
    {
        ...
        if (
            ...
            $tooltip      = Text::sprintf('COM_BLOG_CHECKED_OUT_BY', $checkoutUser->name)
                . ' <br> ' . $date;

            $text = LayoutHelper::render('blog.content.icons.edit_lock', ['post' => $post, 'tooltip' => $tooltip, 'legacy' => $legacy]);
        ...
        if ($post->state == Workflow::CONDITION_UNPUBLISHED) {
            $tooltip = Text::_('COM_BLOG_EDIT_UNPUBLISHED_POST');
        } else {
            $tooltip = Text::_('COM_BLOG_EDIT_PUBLISHED_POST');
        }

        $text = LayoutHelper::render('blog.content.icons.edit', ['post' => $post, 'tooltip' => $tooltip, 'legacy' => $legacy]);
