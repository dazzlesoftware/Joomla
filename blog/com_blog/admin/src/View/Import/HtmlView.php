...
namespace Joomla\Component\Blog\Administrator\View\Import;
...
final class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null): void
    {
        ...
        $q = $db->createQuery()->select(['id','title','level'])->from('#__blog_categories')->where('published=1')->order('lft');
        $this->categories = $db->setQuery($q)->loadObjectList();
        $this->batches = $db->setQuery($db->createQuery()->select('*')->from('#__blog_import_batches')->order('created DESC'), 0, 20)->loadObjectList();
