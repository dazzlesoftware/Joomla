...
namespace Joomla\Component\Blog\Administrator\View\Posts;
...
use Joomla\Component\Blog\Administrator\Model\PostsModel;
...
class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null)
    {
        ...
        $this->vote          = PluginHelper::isEnabled('blog', 'vote');
        $this->hits          = ComponentHelper::getParams('com_blog')->get('record_hits', 1) == 1;
    ...
    {
        $canDo    = ContentHelper::getActions('com_blog', 'category', $this->state->get('filter.category_id'));
        $user     = $this->getCurrentUser();
        $toolbar  = $this->getDocument()->getToolbar();

        ToolbarHelper::title(Text::_('COM_BLOG_POSTS_TITLE'), 'copy post');
        ...
        if (!$this->isEmptyState && ($canDo->get('core.edit.state') || \count($this->transitions))) {
            ...
            if ($canDo->get('core.edit.state')) {
                ...
                if ($this->state->get('filter.published') != BlogComponent::CONDITION_TRASHED) {
            ...
            if (
                ...
                $childBar->popupButton('batch', 'JTOOLBAR_BATCH')
                    ->popupType('inline')
                    ->textHeader(Text::_('COM_BLOG_BATCH_OPTIONS'))
        ...
        if (!$this->isEmptyState && $this->state->get('filter.published') == BlogComponent::CONDITION_TRASHED && $canDo->get('core.delete')) {
