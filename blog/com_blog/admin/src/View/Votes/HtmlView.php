<?php
namespace Joomla\Component\Blog\Administrator\View\Votes;
...
final class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null): void
    {
        ...
        if ($this->getLayout() === 'edit') {
            ...
            $this->vote = $id ? $db->setQuery($db->createQuery()->select('*')->from('#__blog_rating_votes')->where('id=' . $id))->loadObject() : (object) ['id' => 0, 'post_id' => 0, 'rating' => 5];
            if (!$this->vote) { throw new \RuntimeException('Vote not found', 404); }
            $this->posts = $db->setQuery($db->createQuery()->select(['id', 'title'])->from('#__blog')->order('title ASC'))->loadObjectList();
            ToolbarHelper::title($id ? 'Edit vote' : 'New vote', 'star');
            parent::display($tpl);
            return;
        }
        $q = $db->createQuery()->from('#__blog_rating_votes AS v')->join('LEFT', '#__blog AS p ON p.id=v.post_id');
