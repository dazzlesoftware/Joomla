...
final class CategoriesHelper
{
    ...
    private const ASSOCIATIONS_CONTEXT = 'com_blog.category';
    ...
    {
        $db = self::db();
        $q = $db->createQuery()->select('id AS value,title AS text')->from('#__blog_categories')->order('lft,title');
        if ($site) {
    ...
    public static function save(array $data): int
    {
        ...
        $n = 2;
        while ((int)$db->setQuery('SELECT id FROM #__blog_categories WHERE alias='.$db->quote($alias).($id ? ' AND id<>'.$id : ''))->loadResult()) {
            $alias = $base.'-'.$n++;
        }
        ...
        $row = (object)['id' => $id ?: null,'title' => $title,'alias' => $alias,'description' => (string)($data['description'] ?? ''),'published' => (int)($data['published'] ?? 1),'access' => (int)($data['access'] ?? 1),'language' => (string)($data['language'] ?? '*'),'parent_id' => $parentId,'extension' => 'com_blog','is_default' => $isDefault,'allow_autoposting' => (int)($data['allow_autoposting'] ?? 1),'default_image' => (string)($data['default_image'] ?? ''),'default_tags' => (string)($data['default_tags'] ?? ''),'created_time' => Factory::getDate()->toSql(),'created_user_id' => $userId,'modified_time' => Factory::getDate()->toSql(),'modified_user_id' => $userId,'metadata' => '{}','params' => '{}'];
        if ($id) {
            $old = $db->setQuery('SELECT * FROM #__blog_categories WHERE id='.$id)->loadObject();
            if (!$old) {
            ...
            }$db->updateObject('#__blog_categories', $row, 'id');
        } else {
        ...
        if ($isDefault) {
            $db->setQuery('UPDATE #__blog_categories SET is_default=0 WHERE id<>' . (int) $row->id)->execute();
        }
    ...
    {
        return (int) self::db()->setQuery('SELECT id FROM #__blog_categories WHERE is_default=1 AND published=1')->loadResult();
    }
    ...
    public static function makeDefault(int $id): bool
    {
        ...
        $db->transactionStart();
        try {
            $db->setQuery('UPDATE #__blog_categories SET is_default=0')->execute();
            $db->setQuery('UPDATE #__blog_categories SET is_default=1 WHERE id=' . $id)->execute();
    ...
    public static function removeDefault(array $ids): int
    {
        ...
        self::db()->setQuery('UPDATE #__blog_categories SET is_default=0 WHERE id IN (' . implode(',', $ids) . ')')->execute();
        return count($ids);
    }
    ...
    {
        $db = self::db();
        $q = $db->createQuery()->select('id,title')->from('#__blog_categories')
    ...
    public static function getAssociations(int $id): array
    {
        ...
        $rows = Associations::getAssociations('com_blog', '#__blog_categories', self::ASSOCIATIONS_CONTEXT, $id, 'id', 'alias', '');
        $out = [];
