...
final class TagsHelper
{
    ...
    {
        $id = (int) self::db()->setQuery('SELECT id FROM #__blog_tags WHERE is_default=1 AND published=1')->loadResult();
        return $id ? [(string) $id] : [];
    }
    ...
    public static function save(array $data): int
    {
        ...
        $alias = $base;
        $n = 2;
        while ($db->setQuery('SELECT id FROM #__blog_tags WHERE alias=' . $db->quote($alias) . ($id ? ' AND id<>' . $id : ''))->loadResult()) {
            $alias = $base . '-' . $n++;
        }
        ...
        if ($id) {
            $db->updateObject('#__blog_tags', $row, 'id');
        } else {
            $db->insertObject('#__blog_tags', $row, 'id');
        }

        if ($isDefault) {
            $db->setQuery('UPDATE #__blog_tags SET is_default=0 WHERE id<>' . (int) $row->id)->execute();
        }
    ...
    public static function publish(array $ids, int $state): int
    {
        ...
        $db = self::db();
        $db->setQuery(
            $db->createQuery()->update('#__blog_tags')->set('published=' . (int) $state)->whereIn('id', $ids)
        )->execute();
        return count($ids);
    }
    ...
    public static function makeDefault(int $id): bool
    {
        ...
        $db->transactionStart();
        try {
            $db->setQuery('UPDATE #__blog_tags SET is_default=0')->execute();
            $db->setQuery('UPDATE #__blog_tags SET is_default=1 WHERE id=' . $id)->execute();
    ...
    public static function removeDefault(array $ids): int
    {
        ...
        $db = self::db();
        $db->setQuery(
            $db->createQuery()->update('#__blog_tags')->set('is_default=0')->whereIn('id', $ids)
        )->execute();
        return count($ids);
    }
    ...
    public static function delete(array $ids): int
    {
        ...
        $db->transactionStart();
        try {
            $db->setQuery($db->createQuery()->delete('#__blog_tag_map')->whereIn('tag_id', $ids))->execute();
            $db->setQuery($db->createQuery()->delete('#__blog_tags')->whereIn('id', $ids))->execute();
            $db->transactionCommit();
        } catch (\Throwable $e) {
            $db->transactionRollback();
            throw $e;
    ...
    public function getTagIds($id, $typeAlias = 'com_blog.post'): string
    {
        $db = self::db();
        $this->tags = implode(',', $db->setQuery($db->createQuery()->select('tag_id')->from('#__blog_tag_map')->where('content_item_id=' . (int) $id)->where('type_alias=' . $db->quote($typeAlias))->order('tag_id'))->loadColumn());
        return $this->tags;
    }
    ...
    public function getTags($ids): array
    {
        ...
        return $ids ? self::db()->setQuery(self::db()->createQuery()->select('*')->from('#__blog_tags')->whereIn('id', $ids))->loadObjectList() : [];
    }
    ...
    public static function assign(int $postId, array $ids, string $type = 'com_blog.post'): void
    {
        $db = self::db();
        $ids = self::resolve($ids);
        $db->transactionStart(true);
        try {
            $db->setQuery('DELETE FROM #__blog_tag_map WHERE content_item_id=' . $postId . ' AND type_alias=' . $db->quote($type))->execute();
            foreach ($ids as $id) {
