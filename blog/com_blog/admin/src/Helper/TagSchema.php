...
final class TagSchema
{
    public static function ensure(DatabaseInterface $db): void
    {
        ...
        if ($db->setQuery('SELECT version FROM #__blog_tag_migrations WHERE version=1')->loadResult()) {
            return;
        }
        ...
        try {
            if ($legacy) {
                $types = "('com_blog.post','com_blog.article','com_blog.category')";
                ...
                foreach ($tags as $tag) {
                    if (!$db->setQuery('SELECT id FROM #__blog_tags WHERE id=' . (int) $tag->id)->loadResult()) {
                        $base = mb_substr($tag->alias ?: 'tag-' . $tag->id, 0, 170);
                        $tag->alias = $base;
                        if ($db->setQuery('SELECT id FROM #__blog_tags WHERE alias=' . $db->quote($base))->loadResult()) {
                            $tag->alias .= '-' . $tag->id;
                        }
                        $db->insertObject('#__blog_tags', $tag);
                    }
                }
                $db->setQuery("INSERT IGNORE INTO #__blog_tag_map (content_item_id,tag_id,type_alias) SELECT DISTINCT m.content_item_id,m.tag_id,'com_blog.post' FROM #__contentitem_tag_map m INNER JOIN #__blog p ON p.id=m.content_item_id INNER JOIN #__blog_tags t ON t.id=m.tag_id WHERE m.type_alias IN ('com_blog.post','com_blog.article')")->execute();
                $db->setQuery("INSERT IGNORE INTO #__blog_tag_map (content_item_id,tag_id,type_alias) SELECT DISTINCT m.content_item_id,m.tag_id,'com_blog.category' FROM #__contentitem_tag_map m INNER JOIN #__blog_categories c ON c.id=m.content_item_id INNER JOIN #__blog_tags t ON t.id=m.tag_id WHERE m.type_alias='com_blog.category'")->execute();
            }
            $db->setQuery('INSERT INTO #__blog_tag_migrations (version) VALUES (1)')->execute();
