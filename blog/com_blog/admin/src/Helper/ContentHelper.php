...
class ContentHelper extends \Joomla\CMS\Helper\ContentHelper
{
    ...
    {
        if ($form->getName() != 'com_blog.categorycom_blog') {
            return;
        }
        ...
        $component = Factory::getApplication()->bootComponent('com_blog');

        if (
            !$component instanceof WorkflowServiceInterface
            || !$component->isWorkflowActive('com_blog.post')
        ) {
        ...
        $query->select($db->quoteName('title'))
            ...
            ->where(
                [
                    ...
                    $db->quoteName('extension') . ' = ' . $db->quote('com_blog.post'),
                ]
            );
        ...
        $field->addOption('- ' . Text::_('COM_BLOG_WORKFLOWS') . ' -', ['disabled' => 'true']);
    }
}
