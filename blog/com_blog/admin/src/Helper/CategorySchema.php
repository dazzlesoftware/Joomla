...
final class CategorySchema
{
    public static function ensure(DatabaseInterface $db): void
    {
        ...
        $columns = $db->getTableColumns('#__blog_categories');
        if (!isset($columns['allow_autoposting'])) {
            $db->setQuery('ALTER TABLE #__blog_categories ADD COLUMN allow_autoposting TINYINT NOT NULL DEFAULT 1 AFTER language')->execute();
        }
        if (!isset($columns['default_image'])) {
            $db->setQuery("ALTER TABLE #__blog_categories ADD COLUMN default_image VARCHAR(255) NOT NULL DEFAULT '' AFTER allow_autoposting")->execute();
        }
