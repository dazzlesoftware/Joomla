...
class NeuralNetworkController extends BaseController
{
    private function guard(bool $image = false): object
    {
        ...
        if ($id) {
            ...
            $post = $db->setQuery($db->getQuery(true)->select(['created_by','catid'])->from('#__blog')->where('id=' . $id))->loadObject();
            $allowed = $post && ($user->authorise('core.edit', 'com_blog.post.' . $id) || ($user->authorise('core.edit.own', 'com_blog.post.' . $id) && (int) $post->created_by === (int) $user->id));
        } else {
            $allowed = $user->authorise('core.create', $catid ? 'com_blog.category.' . $catid : 'com_blog');
        }
    ...
    public function generate(): void
    {
        $this->respond(function () {
            ...
            $usage = $session->get('com_blog.ai.usage', ['start' => time(), 'count' => 0]);
            ...
            $usage['count']++; $session->set('com_blog.ai.usage', $usage);
            ...
            if (isset($result['image'])) {
                ...
                $session->set('com_blog.ai.image', ['id' => $id, 'created' => time(), 'image' => $result['image']]);
    ...
    public function saveImage(): void
    {
        $this->respond(function () {
            $this->guard(true);
            $session = Factory::getApplication()->getSession();
            $pending = $session->get('com_blog.ai.image', []);
            ...
            $directory = $root;
            foreach (['generated', 'blog'] as $segment) {
                $directory .= '/' . $segment;
            ...
            $session->remove('com_blog.ai.image');
            $path = 'images/generated/blog/' . $name;
