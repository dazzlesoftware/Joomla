...
use Joomla\Component\Blog\Administrator\Helper\ContentHelper;
...
if ($saveOrder && !empty($this->items)) {
    ...
    $saveOrderingUrl = 'index.php?option=com_blog&task=' . $controller . '.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
    HTMLHelper::_('draggablelist.draggable');
}

$workflow_enabled  = ComponentHelper::getParams('com_blog')->get('workflow_enabled');
...
if ($workflow_enabled) :
    ...
    ->useScript('com_blog.posts-status');

    $workflow_state    = Factory::getApplication()->bootComponent('com_blog')->isFunctionalityUsed('core.state', 'com_blog.post');
    $workflow_featured = Factory::getApplication()->bootComponent('com_blog')->isFunctionalityUsed('core.featured', 'com_blog.post');
...
<form action="<?php echo Route::_('index.php?option=com_blog&view=posts'); ?>" method="post" name="adminForm" id="adminForm">
    <div class="row">
        <div class="col-md-12">
            <div id="j-main-container" class="j-main-container">
                <?php
                // Search tools bar
                echo LayoutHelper::render('blog.searchtools.default', ['view' => $this, 'options' => ['selectorFieldName' => 'featured']]);
?>
                ...
                <?php else : ?>
                    <table class="table itemList" id="postList">
                        <caption class="visually-hidden">
                            <?php echo $featured === '1' ? Text::_('COM_BLOG_FEATURED_TABLE_CAPTION') : Text::_('COM_BLOG_POSTS_TABLE_CAPTION'); ?>,
                        ...
                        <?php foreach ($this->items as $i => $item) :
                            ...
                            $assetId              = 'com_blog.post.' . $item->id;
                            $canCreate            = $user->authorise('core.create', 'com_blog.category.' . $item->catid);
                            $canEdit              = $user->authorise('core.edit', 'com_blog.post.' . $item->id);
                            $canCheckin           = $user->authorise('core.manage', 'com_checkin') || $item->checked_out == $userId || is_null($item->checked_out);
                            $canEditOwn           = $user->authorise('core.edit.own', 'com_blog.post.' . $item->id) && $item->created_by == $userId;
                            $canChange            = $user->authorise('core.edit.state', 'com_blog.post.' . $item->id) && $canCheckin;
                            $canExecuteTransition = $user->authorise('core.execute.transition', 'com_blog.post.' . $item->id);
                                ...
                                <th scope="row" class="has-context">
                                    <div class="break-word">
                                        ...
                                        <?php if ($canEdit || $canEditOwn) : ?>
                                            <a href="<?php echo Route::_('index.php?option=com_blog&task=post.edit&id=' . $item->id); ?>" title="<?php echo Text::_('JACTION_EDIT'); ?> <?php echo $this->escape($item->title); ?>">
                                        ...
                                        <div class="small">
                                            <?php
                                    $ParentCatUrl = Route::_('index.php?option=com_blog&view=categories&task=category.edit&id=' . $item->parent_category_id . '&extension=com_blog');
                            $CurrentCatUrl = Route::_('index.php?option=com_blog&view=categories&task=category.edit&id=' . $item->catid . '&extension=com_blog');
                            ...
                            ?>
                                ...
                                <?php if (Multilanguage::isEnabled()) : ?>
                                    <td class="small d-none d-md-table-cell">
                                        <?php echo LayoutHelper::render('blog.content.language', $item); ?>
                                    </td>
                                <?php endif; ?>
                    ...
                    <?php
                    if (
                        $user->authorise('core.create', 'com_blog')
