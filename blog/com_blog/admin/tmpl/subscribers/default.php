...
?>
<form action="<?php echo Route::_('index.php?option=com_blog&view=subscribers'); ?>" method="get" class="row g-2 mb-3">
    <input type="hidden" name="option" value="com_blog"><input type="hidden" name="view" value="subscribers">
    ...
    <div class="col-auto"><button class="btn btn-primary">Filter</button></div><div class="col-auto"><a class="btn btn-secondary" href="<?php echo Route::_('index.php?option=com_blog&view=subscribers'); ?>">Clear</a></div>
</form>
<div class="d-flex flex-wrap gap-2 mb-3">
    <a class="btn btn-success" href="<?php echo Route::_('index.php?option=com_blog&task=subscribers.export'); ?>">Export CSV</a>
    <form action="<?php echo Route::_('index.php?option=com_blog&task=subscribers.import'); ?>" method="post" enctype="multipart/form-data" class="d-flex gap-2"><input class="form-control" type="file" name="csv_file" accept=".csv,text/csv" required><button class="btn btn-primary" type="submit">Import CSV</button><?php echo HTMLHelper::_('form.token'); ?></form>
    <a class="btn btn-primary" href="<?php echo Route::_('index.php?option=com_blog&view=newsletter'); ?>">Create email</a>
</div>
<p class="text-muted">CSV columns: email, name. Removed addresses are suppressed and skipped during future imports.</p>
<form action="<?php echo Route::_('index.php?option=com_blog&view=subscribers'); ?>" method="post" id="adminForm">
