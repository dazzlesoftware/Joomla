...
<div class="row g-4">
<div class="col-lg-8">
<form method="get" class="d-flex gap-2 mb-3"><input type="hidden" name="option" value="com_blog"><input type="hidden" name="view" value="tags"><label class="visually-hidden" for="tag-search">Search tags</label><input class="form-control" id="tag-search" name="search" value="<?php echo $escape(Factory::getApplication()->getInput()->getString('search')); ?>" placeholder="Search tags"><button class="btn btn-primary">Search</button></form>
...
<?php foreach ($this->items as $item) : ?><tr><td><a href="<?php echo Route::_('index.php?option=com_blog&view=tags&id=' . (int) $item->id); ?>"><?php echo $escape($item->title); ?></a></td><td><?php echo $escape($item->alias); ?></td><td><?php echo (int) $item->post_count; ?></td><td><?php echo $item->published == 1 ? 'Published' : 'Unpublished'; ?></td></tr><?php endforeach; ?>
...
<?php if ($user->authorise($tag ? 'core.edit' : 'core.create', 'com_blog')) : ?>
<form method="post" action="<?php echo Route::_('index.php?option=com_blog&task=tags.save'); ?>" class="form-vertical">
...
<label for="tag-published">Status</label><select class="form-select mb-3" id="tag-published" name="published" <?php echo !$user->authorise('core.edit.state', 'com_blog') ? 'disabled' : ''; ?>><option value="1">Published</option><option value="0" <?php echo $tag && !$tag->published ? 'selected' : ''; ?>>Unpublished</option></select>
...
<button class="btn btn-success" type="submit">Save tag</button> <a class="btn btn-outline-secondary" href="<?php echo Route::_('index.php?option=com_blog&view=tags'); ?>">New tag</a>
<?php echo HTMLHelper::_('form.token'); ?></form>
<?php endif; ?>
<?php if ($tag && $user->authorise('core.delete', 'com_blog')) : ?><form method="post" action="<?php echo Route::_('index.php?option=com_blog&task=tags.delete'); ?>" class="mt-3"><input type="hidden" name="id" value="<?php echo (int) $tag->id; ?>"><button class="btn btn-outline-danger" type="submit">Delete tag and its assignments</button><?php echo HTMLHelper::_('form.token'); ?></form><?php endif; ?>
</div></div></div></div>
