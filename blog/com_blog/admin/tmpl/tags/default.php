...
use Joomla\Component\Blog\Administrator\Button\DefaultButton;

/** @var \Joomla\Component\Blog\Administrator\View\Tags\HtmlView $this */
?>
<form action="<?php echo Route::_('index.php?option=com_blog&view=tags'); ?>" method="post" name="adminForm" id="adminForm">
    <div class="alert alert-light border mb-3">
        <h1 class="h4 mb-1"><?php echo Text::_('COM_BLOG_TAGS_TITLE'); ?></h1>
        <p class="mb-0 small text-muted"><?php echo Text::_('COM_BLOG_TAGS_DESC'); ?></p>
    </div>
    ...
    <?php else : ?>
        <div class="table-responsive">
            <table class="table table-hover" id="tagList">
                <caption class="visually-hidden"><?php echo Text::_('COM_BLOG_TAGS_TABLE_CAPTION'); ?></caption>
                <thead>
                    <tr>
                        ...
                        <th scope="col" class="w-5 text-center">
                            <?php echo HTMLHelper::_('searchtools.sort', 'COM_BLOG_HEADING_DEFAULT', 't.is_default', $this->listDirn, $this->listOrder); ?>
                        </th>
                        <th scope="col" class="w-5 text-center">
                            <?php echo HTMLHelper::_('searchtools.sort', 'JSTATUS', 't.published', $this->listDirn, $this->listOrder); ?>
                        </th>
                        <th scope="col" class="w-5 text-center d-none d-md-table-cell">
                            <?php echo HTMLHelper::_('searchtools.sort', 'COM_BLOG_HEADING_POSTS', 'post_count', $this->listDirn, $this->listOrder); ?>
                        </th>
                ...
                <?php foreach ($this->items as $i => $item) : ?>
                    <tr class="row<?php echo $i % 2; ?>">
                        ...
                        <th scope="row">
                            <a href="<?php echo Route::_('index.php?option=com_blog&view=tag&id=' . (int) $item->id); ?>">
                                <?php echo htmlspecialchars($item->title, ENT_QUOTES, 'UTF-8'); ?>
                            </a>
                        </th>
                        <td class="text-center">
                            <?php
                            $defaultOptions = ['task_prefix' => 'tags.', 'disabled' => !$this->getCurrentUser()->authorise('core.edit.state', 'com_blog'), 'id' => 'tag-default-' . $item->id];
                    echo (new DefaultButton())->render((int) $item->is_default, $i, $defaultOptions);
                    ?>
                        </td>
                        <td class="text-center">
                            <?php
                    $options = ['task_prefix' => 'tags.', 'disabled' => !$this->getCurrentUser()->authorise('core.edit.state', 'com_blog'), 'id' => 'tag-state-' . $item->id];
