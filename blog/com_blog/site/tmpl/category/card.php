...
$app->getDocument()->getWebAssetManager()->registerAndUseStyle('com_blog.post-list-styles', 'com_blog/post-list-styles.css', ['version' => hash_file('sha256', JPATH_ROOT . '/media/com_blog/css/post-list-styles.css')]);
$app->getDocument()->getWebAssetManager()->registerAndUseScript('com_blog.post-masonry', 'com_blog/post-masonry.js', ['version' => hash_file('sha256', JPATH_ROOT . '/media/com_blog/js/post-masonry.js')], ['defer' => true]);
...
$app->triggerEvent('onContentPrepare', ['com_blog.categories', &$this->category, &$this->params, 0]);
$this->category->description = $this->category->text;

$results = $app->triggerEvent('onContentAfterTitle', ['com_blog.categories', &$this->category, &$this->params, 0]);
$afterDisplayTitle = trim(implode("\n", $results));

$results = $app->triggerEvent('onContentBeforeDisplay', ['com_blog.categories', &$this->category, &$this->params, 0]);
$beforeDisplayContent = trim(implode("\n", $results));

$results = $app->triggerEvent('onContentAfterDisplay', ['com_blog.categories', &$this->category, &$this->params, 0]);
...
<?php echo \Joomla\Component\Blog\Site\Helper\FeaturedSliderHelper::render($this->params, (int) ($this->category->id ?? 0), (int) ($this->pagination->limitstart ?? 0)); ?>
    <?php if (empty($this->lead_items) && empty($this->link_items) && empty($this->intro_items)) : ?>
        <?php if ($this->params->get('show_no_articles', 1)) : ?>
            <div class="alert alert-info">
                <span class="icon-info-circle" aria-hidden="true"></span><span class="visually-hidden"><?php echo Text::_('INFO'); ?></span>
                <?php echo Text::_('COM_BLOG_NO_POSTS'); ?>
            </div>
        <?php endif; ?>
    <?php endif; ?>
