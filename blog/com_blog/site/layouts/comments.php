...
if (!$postId || $provider === 'disabled') return;
?>
...
<?php if ($provider === 'native') :
    $db=Factory::getContainer()->get(DatabaseInterface::class);$query=$db->createQuery()->select('*')->from($db->quoteName('#__'.$family.'_comments'))->where('post_id='.$postId)->where('state=1')->order('created ASC');$comments=$db->setQuery($query)->loadObjectList();
...
elseif ($provider === 'jcomments') :
    ...
    if (is_file($file)) { require_once $file; if (class_exists('JComments')) echo JComments::showComments($postId, 'com_' . $family, $postTitle); }
    else echo '<p>JComments is selected but is not installed.</p>';
...
elseif ($provider === 'jlex') :
    ...
    if (is_file($file)) { require_once $file; if (class_exists('JLexCommentLoader')) echo JLexCommentLoader::init($family, $postId, $postTitle); }
    else echo '<p>JLex Comments is selected but is not installed.</p>';
elseif ($provider === 'compojoom') :
    $file = JPATH_SITE . '/components/com_comment/helpers/utils.php';
    if (is_file($file)) { require_once $file; if (class_exists('ccommentHelperUtils')) echo ccommentHelperUtils::commentInit('com_' . $family, $displayData); }
    else echo '<p>CompoJoom Comments is selected but is not installed.</p>';
elseif ($provider === 'komento') :
    $file = JPATH_SITE . '/components/com_komento/bootstrap.php';
    if (is_file($file)) { require_once $file; if (class_exists('Komento')) echo Komento::commentify('com_' . $family, $displayData, ['trigger' => 'onDisplayComments']); }
    else echo '<p>Komento is selected but is not installed.</p>';
endif; ?>
</section>
