...
final class SubcategoriesHelper
{
    public static function load(int $parentId, Registry $params, bool $allowRoot = false): array
    {
        ...
        $posts = $db->createQuery()->select('p.catid, COUNT(*) AS numitems')->from('#__blog AS p')
        ...
        $query = $db->createQuery()->select('c.*, COALESCE(counts.numitems, 0) AS numitems')->from('#__blog_categories AS c')
