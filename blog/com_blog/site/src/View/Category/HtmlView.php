...
namespace Joomla\Component\Blog\Site\View\Category;
...
final class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null): void
    {
        ...
        \Joomla\Component\Blog\Site\Helper\ListingSettingsHelper::order($q, $this->params);
        if ($app->getLanguageFilter()) {
    ...
    private function enrichItems(): void
    {
        ...
        $tagsByItem = (new TagsHelper())->getMultipleItemTags('com_blog.post', $ids);
        ...
        foreach ($this->items as $item) {
            ...
            $item->params->set('access-edit', $user->authorise('core.edit', 'com_blog.post.' . $item->id));
            ...
            $app->triggerEvent('onContentPrepare', ['com_blog.category', &$item, &$item->params, 0]);
            $item->summary = $item->text;

            $results = $app->triggerEvent('onContentAfterTitle', ['com_blog.category', &$item, &$item->params, 0]);
            $item->event->afterDisplayTitle = trim(implode("\n", $results));

            $results = $app->triggerEvent('onContentBeforeDisplay', ['com_blog.category', &$item, &$item->params, 0]);
            $item->event->beforeDisplayContent = trim(implode("\n", $results));

            $results = $app->triggerEvent('onContentAfterDisplay', ['com_blog.category', &$item, &$item->params, 0]);
    ...
    {
        $numLeading = 0;
        $numIntro = \Joomla\Component\Blog\Site\Helper\ListingSettingsHelper::count($this->params);
