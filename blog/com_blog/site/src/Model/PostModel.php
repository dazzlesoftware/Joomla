...
class PostModel extends ItemModel
{
     ...
     * @var        string
     */
    protected $_context = 'com_blog.post';
    ...
    protected function populateState()
    {
        ...
        $asset = empty($pk) ? 'com_blog' : 'com_blog.post.' . $pk;

        if ((!$user->authorise('core.edit.state', $asset)) && (!$user->authorise('core.edit', $asset))) {
            $this->setState('filter.published', BlogComponent::CONDITION_PUBLISHED);
            $this->setState('filter.archived', BlogComponent::CONDITION_ARCHIVED);
        }
        ...
        if (!isset($this->_item[$pk])) {
            try {
                ...
                $query->select(
                    ...
                    ->join(
                        'INNER',
                        $db->quoteName('#__blog_categories', 'c'),
                        $db->quoteName('c.id') . ' = ' . $db->quoteName('a.catid')
                    )
                    ->join('LEFT', $db->quoteName('#__blog_frontpage', 'fp'), $db->quoteName('fp.content_id') . ' = ' . $db->quoteName('a.id'))
                ...
                if (
                    !$user->authorise('core.edit.state', 'com_blog.post.' . $pk)
                    && !$user->authorise('core.edit', 'com_blog.post.' . $pk)
                ) {
                ...
                if (empty($data)) {
                    throw new \Exception(Text::_('COM_BLOG_ERROR_POST_NOT_FOUND'), 404);
                }
    ...
    public function storeVote($pk = 0, $rate = 0)
    {
        ...
        if ($rate >= 1 && $rate <= 5 && $pk > 0) {
            ...
            $query->select('*')
                ->from($db->quoteName('#__blog_rating'))
            ...
            if (!$rating) {
                ...
                $query->insert($db->quoteName('#__blog_rating'))
                    ->columns(
                        [
                ...
                if ($userIP != $rating->lastip) {
                    ...
                    $query->update($db->quoteName('#__blog_rating'))
                        ->set(
                            [
        ...
        Factory::getApplication()->enqueueMessage(Text::sprintf('COM_BLOG_INVALID_RATING', $rate), 'error');

        return false;
    }

    /**
     * Cleans the cache of com_blog and content modules
     *
