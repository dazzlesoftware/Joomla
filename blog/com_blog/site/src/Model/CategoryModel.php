...
class CategoryModel extends ListModel
{
     ...
     * @var  string
     */
    protected $_context = 'com_blog.category';
    ...
    protected function populateState($ordering = null, $direction = null)
    {
        ...
        $value = $this->getUserStateFromRequest('com_blog.category.filter.' . $itemid . '.tag', 'filter_tag', 0, 'int', false);
        $this->setState('filter.tag', $value);

        // Optional filter text
        $search = $app->getUserStateFromRequest('com_blog.category.list.' . $itemid . '.filter-search', 'filter-search', '', 'string');
        $this->setState('list.filter', $search);

        // Filter.order
        $orderCol = $app->getUserStateFromRequest('com_blog.category.list.' . $itemid . '.filter_order', 'filter_order', '', 'string');
        ...
        $listOrder = $app->getUserStateFromRequest('com_blog.category.list.' . $itemid . '.filter_order_Dir', 'filter_order_Dir', '', 'cmd');
    ...
    public function getItems()
    {
        ...
        if ($this->_posts === null && $category = $this->getCategory()) {
            ...
            $model = $this->bootComponent('com_blog')->getMVCFactory()
    ...
    protected function _buildContentOrderBy()
    {
        ...
        $itemid    = $app->getInput()->get('id', 0, 'int') . ':' . $app->getInput()->get('Itemid', 0, 'int');
        $orderCol  = $app->getUserStateFromRequest('com_blog.category.list.' . $itemid . '.filter_order', 'filter_order', '', 'string');
        $orderDirn = $app->getUserStateFromRequest('com_blog.category.list.' . $itemid . '.filter_order_Dir', 'filter_order_Dir', '', 'cmd');
        $orderby   = ' ';
