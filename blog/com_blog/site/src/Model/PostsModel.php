...
class PostsModel extends ListModel
{
    ...
    protected function populateState($ordering = 'ordering', $direction = 'ASC')
    {
        ...
        if ((!$user->authorise('core.edit.state', 'com_blog')) && (!$user->authorise('core.edit', 'com_blog'))) {
            ...
            $this->setState('filter.published', BlogComponent::CONDITION_PUBLISHED);
        }
    ...
    protected function getListQuery()
    {
        ...
        $query->select(
            ...
            ->from($db->quoteName('#__blog', 'a'))
            ->join('LEFT', $db->quoteName('#__blog_categories', 'c'), $db->quoteName('c.id') . ' = ' . $db->quoteName('a.catid'))
            ...
            ->join('LEFT', $db->quoteName('#__blog_categories', 'parent'), $db->quoteName('parent.id') . ' = ' . $db->quoteName('c.parent_id'));
        ...
        $query->join($frontpageJoin, $db->quoteName('#__blog_frontpage', 'fp'), $db->quoteName('fp.content_id') . ' = ' . $db->quoteName('a.id'));
        ...
        if (PluginHelper::isEnabled('content', 'vote')) {
            ...
            $query->select(
                ...
                ->join('LEFT', $db->quoteName('#__blog_rating', 'v'), $db->quoteName('a.id') . ' = ' . $db->quoteName('v.content_id'));
        }
        ...
        if (is_numeric($categoryId)) {
            ...
            if ($includeSubcategories) {
                ...
                $subQuery = $db->createQuery()
                    ...
                    ->from($db->quoteName('#__blog_categories', 'sub'))
                    ->join(
                        'INNER',
                        $db->quoteName('#__blog_categories', 'this'),
        ...
        if (
            !(is_numeric($condition) && $condition == BlogComponent::CONDITION_UNPUBLISHED)
            && !(\is_array($condition) && \in_array(BlogComponent::CONDITION_UNPUBLISHED, $condition))
        ) {
            $query->where(
                [
        ...
        if (\is_array($tagId)) {
            ...
            if ($tagId) {
                $subQuery = $db->createQuery()
                    ...
                    ->from($db->quoteName('#__blog_tag_map'))
                    ->where(
                        [
                            $db->quoteName('tag_id') . ' IN (' . implode(',', $query->bindArray($tagId)) . ')',
                            $db->quoteName('type_alias') . ' = ' . $db->quote('com_blog.post'),
                        ]
                    );
            ...
            $query->join(
                'INNER',
                $db->quoteName('#__blog_tag_map', 'tagmap'),
                $db->quoteName('tagmap.content_item_id') . ' = ' . $db->quoteName('a.id')
                    . ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_blog.post')
            )
                ->where($db->quoteName('tagmap.tag_id') . ' = :tagId')
                ->bind(':tagId', $tagId, ParameterType::INTEGER);
        }

        \Joomla\Component\Blog\Site\Helper\ListingFilterHelper::apply($query, $this->getState('params'), 'a');
    ...
    public function getItems()
    {
        ...
        foreach ($items as $item) {
            ...
            // Get display date
            $item->displayDate = \Joomla\Component\Blog\Site\Helper\DateHelper::value($item, $item->params);
        ...
        if ($taggedItems) {
            ...
            foreach ($tagsHelper->getMultipleItemTags('com_blog.post', $itemIds) as $id => $tags) {
