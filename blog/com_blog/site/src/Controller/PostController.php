...
class PostController extends FormController
{
    ...
    protected function allowAdd($data = [])
    {
        ...
        if ($categoryId) {
            ...
            $allow = $user->authorise('core.create', 'com_blog.category.' . $categoryId);
        }
    ...
    protected function allowEdit($data = [], $key = 'id')
    {
        ...
        if ($user->authorise('core.edit', 'com_blog.post.' . $recordId)) {
            return true;
        }

        // Check edit own on the record asset (explicit or inherited)
        if ($user->authorise('core.edit.own', 'com_blog.post.' . $recordId)) {
    ...
    private function getDefaultRedirectLink()
    {
        ...
        switch ($type) {
            case 'archive':
                return 'index.php?option=com_blog&view=archive';

            case 'categories':
                return 'index.php?option=com_blog&view=categories';

            case 'category':
                $catId = (int) $params->get('default_post_redirect_category');

                if ($catId > 0) {
                    return 'index.php?option=com_blog&view=category&id=' . $catId;
                }
            ...
            case 'tags':
                return 'index.php?option=com_blog&view=tags';
        ...
        return 'index.php?option=com_blog&view=featured';
    }
