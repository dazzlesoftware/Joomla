...
final class CommentsController extends BaseController
{
 public function submit(): void
 {
  ...
  $db=Factory::getContainer()->get(DatabaseInterface::class); $valid=(int)$db->setQuery($db->createQuery()->select('COUNT(*)')->from($db->quoteName('#__blog'))->where('id='.(int)$postId))->loadResult();
  if(!$valid){throw new \RuntimeException('Post not found',404);}
  $params=ComponentHelper::getParams('com_blog');$state=$params->get('comments_moderation',1)?0:1;$ipHash=hash('sha256',($_SERVER['REMOTE_ADDR']??'').'blog');$cooldown=max(0,min(3600,(int)$params->get('comments_cooldown',60)));if($cooldown){$since=Factory::getDate('-'.$cooldown.' seconds')->toSql();$recent=(int)$db->setQuery($db->createQuery()->select('COUNT(*)')->from('#__blog_comments')->where('ip_hash='.$db->quote($ipHash))->where('created>='.$db->quote($since)))->loadResult();if($recent){$app->enqueueMessage('Please wait before posting another comment.','warning');$app->redirect($return);return;}}
  ...
  $db->insertObject('#__blog_comments',$row);if($params->get('comments_notify',1)&&$app->get('mailfrom')){try{$mailer=Factory::getContainer()->get(MailerFactoryInterface::class)->createMailer();$mailer->addRecipient($app->get('mailfrom'));$mailer->setSubject('New '.ucfirst('blog').' comment awaiting review');$mailer->setBody("Post ID: $postId\nName: $name\nEmail: $email\n\n$body");$mailer->send();}catch(\Throwable $e){}}$app->enqueueMessage($state?'Comment published.':'Comment submitted for approval.');$app->redirect($return);
 }
}
