...
namespace Joomla\Plugin\Codex\Vote\Extension;
...
final class Vote extends CMSPlugin implements SubscriberInterface
{
     ...
     * Add HTML string containing code for the votes if in com_codex.
     *
     ...
     *
     * @return  string  HTML string containing code for the votes if in com_codex else empty string
     *
     * @since   3.7.0
     */
    private function displayVotingData($context, $row, $params, $page)
    {
        ...
        if (!isset($row->rating, $row->rating_count) && !empty($row->id)) {
            ...
            $rating = $db->setQuery($db->createQuery()->select('rating_sum, rating_count')
                ->from('#__codex_rating')->where('content_id=' . (int) $row->id))->loadObject();
        ...
        $path = PluginHelper::getLayoutPath('codex', 'vote', 'rating');
        ...
        if ($this->getApplication()->getInput()->getString('view', '') === 'post' && $row->state == 1) {
            ...
            $path = PluginHelper::getLayoutPath('codex', 'vote', 'vote');
