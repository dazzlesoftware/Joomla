...
namespace Joomla\Plugin\Codex\Pagenavigation\Extension;
...
final class PageNavigation extends CMSPlugin implements SubscriberInterface
{
    ...
    /**
     * If in the post view and the parameter is enabled shows the page navigation
     *
    ...
    public function onContentBeforeDisplay(BeforeDisplayEvent $event)
    {
        ...
        if ($context === 'com_codex.post' && $view === 'post' && $params->get('show_item_navigation')) {
            $db         = $this->getDatabase();
            ...
            $query->select($db->quoteName(['a.id', 'a.title', 'a.catid', 'a.language']))
                ...
                ->join('LEFT', $db->quoteName('#__codex_categories', 'cc'), $db->quoteName('cc.id') . ' = ' . $db->quoteName('a.catid'));
            ...
            if ($row->prev) {
                ...
                $row->prev       = RouteHelper::getPostRoute($row->prev->slug, $row->prev->catid, $row->prev->language);
            } else {
                $row->prev_label = '';
                $row->prev       = '';
            }
            ...
            if ($row->next) {
                ...
                $row->next       = RouteHelper::getPostRoute($row->next->slug, $row->next->catid, $row->next->language);
            } else {
                $row->next_label = '';
                $row->next       = '';
            }
            ...
            if ($row->prev || $row->next) {
                ...
                $path = PluginHelper::getLayoutPath('codex', 'pagenavigation');
