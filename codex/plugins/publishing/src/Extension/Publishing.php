...
namespace Joomla\Plugin\Codex\Publishing\Extension;
...
final class Publishing extends CMSPlugin implements SubscriberInterface
{
    ...
    protected function enhanceItemForm(Form $form, $data)
    {
        ...
        $form->setFieldAttribute($fieldname, 'label', Text::sprintf('PLG_CODEX_PUBLISHING_PUBLISHED', $label));
    ...
    public function onContentBeforeChangeState(Model\BeforeChangeStateEvent $event)
    {
        ...
        throw new \Exception($this->getApplication()->getLanguage()->_('PLG_CODEX_PUBLISHING_CHANGE_STATE_NOT_ALLOWED'));
    }
    ...
    public function onContentBeforeSave(Model\BeforeSaveEvent $event)
    {
        ...
        if (isset($data[$keyName])) {
            $this->getApplication()->enqueueMessage($this->getApplication()->getLanguage()->_('PLG_CODEX_PUBLISHING_CHANGE_STATE_NOT_ALLOWED'), 'error');
