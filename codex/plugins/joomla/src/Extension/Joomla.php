...
namespace Joomla\Plugin\Codex\Joomla\Extension;
...
final class Joomla extends CMSPlugin implements SubscriberInterface
{
     ...
     * Post is passed by reference, but after the save, so no changes will be saved.
    ...
    public function onContentAfterSave(AfterSaveEvent $event): void
    {
        ...
        foreach ($users as $user_id) {
            if ($user_id != $user->id) {
                ...
                if ($receiver->authorise('core.login.admin') && $receiver->authorise('core.manage', 'com_codex')) {
                    $templateData['url'] = Route::link('administrator', 'index.php?option=com_codex&view=posts&filter[search]=id:' . $post->id, false, $linkMode, true);
                } elseif ($post->state === 1 && \in_array($post->access, $receiver->getAuthorisedViewLevels())) {
                    $templateData['url'] = Route::link('site', 'index.php?option=com_codex&view=post&id=' . $post->id, false, $linkMode, true);
                } elseif ($post->state !== 1 && $receiver->authorise('core.edit', 'com_codex.post.' . $post->id)) {
                    $templateData['url'] = Route::link('site', 'index.php?option=com_codex&view=post&id=' . $post->id, false, $linkMode, true);
                } elseif ($post->state !== 1) {
                ...
                // Send email
                try {
                    $mailer = new MailTemplate('plg_codex_joomla.newpost', $receiver->getParam('admin_language', $this->getLanguage()->getTag()));
    ...
    public function onContentBeforeDelete(BeforeDeleteEvent $event)
    {
        ...
        if (!\in_array($context, ['com_codex.category', 'com_workflow.stage', 'com_workflow.workflow'])) {
            return;
        }
    ...
    public function onSchemaBeforeCompileHead(BeforeCompileHeadEvent $event): void
    {
        ...
        if ($extension === 'com_codex' && $this->params->get('schema_content', 1)) {
    ...
    /**
     * Inject com_codex schemas if needed
     *
     * @param   string    $context  The com_codex context like com_codex.post.5
    ...
    private function injectContentSchema(string $context, Registry $schema)
    {
        ...
        $component = $this->getApplication()->bootComponent('com_codex')->getMVCFactory();
        ...
        if ($view == 'post' && $id > 0) {
            $additionalSchemas = $cache->get(function ($id) use ($component, $baseId) {
                $model = $component->createModel('Post', 'Site');
                ...
                $post->media = new Registry($post->media);

                $postSchema = $this->createPostSchema($post);

                $postSchema['isPartOf'] = ['@id' => $baseId . 'WebPage/base'];
            ...
            $additionalSchemas = $cache->get(function ($view, $id) use ($component, $baseId, $app, $db) {
                ...
                $schemaId = $baseId . 'com_codex/' . $view . ($view == 'category' ? '/' . $id : '');
                ...
                $postIds = ArrayHelper::getColumn($posts, 'id');

                if (!empty($postIds)) {
                    $aContext = 'com_codex.post';

                    // Load the schema data from the database
                    $query = $db->createQuery()
                        ->select('*')
                        ->from($db->quoteName('#__schemaorg'))
                        ->whereIn($db->quoteName('itemId'), $postIds)
                    ...
                    foreach ($posts as $post) {
                        if (isset($schemas[$post->id])) {
                            $localSchema = new Registry($schemas[$post->id]->schema);

                            $localSchema->set('@id', $baseId . str_replace('.', '/', $aContext) . '/' . (int) $post->id);
                        ...
                        // Set to BlogPosting
                        $postSchema['@type'] = 'BlogPosting';

                        $additionalSchemas[] = $postSchema;

                        $additionalSchema['blogPost'][] = ['@id' => $postSchema['@id']];
                    }
                }
    ...
    /**
     * Returns a finished Post schema type based on a given joomla post
     *
     * @param object $post  An post to extract schema data from
     *
     * @return array
     *
     * @since   5.0.0
     */
    private function createPostSchema(object $post)
    {
        $baseId   = Uri::root() . '#/schema/';
        $schemaId = $baseId . 'com_codex/post/' . (int) $post->id;
        ...
        $schema['inLanguage']  = $post->language === '*' ? $this->getApplication()->get('language') : $post->language;

        // Author information
        if ($post->params->get('show_author') && !empty($post->author)) {
            $author = [];

            $author['@type'] = 'Person';
            $author['name']  = $post->created_by_alias ?: $post->author;

            if ($post->params->get('link_author') && !empty($post->contact_link)) {
                $author['url'] = $post->contact_link;
            }
        ...
        // Images
        if ($post->media->get('featured_image')) {
            $schema['thumbnailUrl'] = HTMLHelper::_('cleanImageUrl', $post->media->get('featured_image'))->url;
        }

        if ($post->media->get('featured_image')) {
            $schema['image'] = HTMLHelper::_('cleanImageUrl', $post->media->get('featured_image'))->url;
        }
        ...
        if ($post->params->get('show_parent_category') && !empty($post->parent_id) && $post->parent_id > 1) {
            $categories[] = $post->parent_title;
        }

        // Current category
        if ($post->params->get('show_category')) {
            $categories[] = $post->category_title;
        }
        ...
        if ($dateValue = \Joomla\Component\Codex\Site\Helper\DateHelper::value($post, $post->params)) {
            [, $dateType] = \Joomla\Component\Codex\Site\Helper\DateHelper::options($post->params);
            $schema[['created' => 'dateCreated', 'modified' => 'dateModified', 'published' => 'datePublished'][$dateType]] = Factory::getDate($dateValue)->toISO8601();
        }

        // Hits
        if ($post->params->get('show_hits')) {
            $counter = [];

            $counter['@type']                = 'InteractionCounter';
            $counter['userInteractionCount'] = $post->hits;
    ...
    {
        // Special case for Create post menu item
        if ($table->link !== 'index.php?option=com_codex&view=form&layout=edit') {
            return true;
        }
        ...
        if (isset($params['enable_category']) && $params['enable_category'] === 1 && empty($params['catid'])) {
            $table->setError($this->getApplication()->getLanguage()->_('COM_CODEX_CREATE_POST_ERROR'));
