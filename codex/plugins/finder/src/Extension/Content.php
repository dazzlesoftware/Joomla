...
namespace Joomla\Plugin\Codex\Finder\Extension;
...
/**
 * Smart Search adapter for com_codex.
 *
 * @since  2.5
 */
final class Content extends Adapter implements SubscriberInterface
{
     ...
     * @since  2.5
     */
    protected $type_title = 'Codex Post';
    ...
    {
        // Make sure we're handling com_codex categories.
        if ($event->getExtension() === 'com_codex') {
    ...
    public function onFinderAfterSave(FinderEvent\AfterSaveEvent $event): void
    {
        ...
        if ($context === 'com_codex.post' || $context === 'com_codex.form') {
    ...
    protected function index(Result $item)
    {
        ...
        $item->context = 'com_codex.post';

        // Initialise the item parameters.
        $registry     = new Registry($item->params);
        $item->params = clone ComponentHelper::getParams('com_codex', true);
        ...
        $categories = $this->getApplication()->bootComponent('com_codex')->getCategory(['published' => false, 'access' => false]);
        ...
        $nativeTags = $tagDb->setQuery('SELECT t.title,t.published,t.access,t.language FROM #__codex_tags t INNER JOIN #__codex_tag_map m ON m.tag_id=t.id WHERE m.type_alias=' . $tagDb->quote('com_codex.post') . ' AND m.content_item_id=' . (int) $item->id)->loadObjectList();
        foreach ($nativeTags as $tag) {
            $item->addTaxonomy('Tag', $tag->title, (int) $tag->published, (int) $tag->access, $tag->language);
        }
        // Get content extras.
        Helper::getContentExtras($item);
        Helper::addCustomFields($item, 'com_codex.post');
