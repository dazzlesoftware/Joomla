...
namespace Joomla\Plugin\Codex\Featuring\Extension;
...
final class Featuring extends CMSPlugin implements SubscriberInterface
{
    ...
    protected function enhanceItemForm(Form $form, $data)
    {
        ...
        $form->setFieldAttribute(
            $fieldname,
            'label',
            Text::sprintf('PLG_CODEX_FEATURING_FEATURED', $label)
        );
    ...
    public function onContentBeforeChangeFeatured(FeatureEvent $event)
    {
        ...
        $event->setAbort('PLG_CODEX_FEATURING_CHANGE_STATE_NOT_ALLOWED');
    ...
    public function onContentBeforeSave(Model\BeforeSaveEvent $event)
    {
        ...
        if (isset($data[$keyName])) {
            $this->getApplication()->enqueueMessage(
                $this->getApplication()->getLanguage()->_('PLG_CODEX_FEATURING_CHANGE_STATE_NOT_ALLOWED'),
                'error'
            );
