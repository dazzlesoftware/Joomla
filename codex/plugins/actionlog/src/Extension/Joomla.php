...
namespace Joomla\Plugin\Codex\Actionlog\Extension;
...
final class Joomla extends ActionLogPlugin implements SubscriberInterface
{
     ...
     * @since  3.9.0
     */
    protected $contextAliases = ['com_codex.form' => 'com_codex.post'];
    ...
    public function onApplicationAfterSave(Application\AfterSaveConfigurationEvent $event): void
    {
        ...
        $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_APPLICATION_CONFIG_UPDATED';
        $action             = 'update';

        $message = [
            'action'         => $action,
            'type'           => 'PLG_CODEX_ACTIONLOG_TYPE_APPLICATION_CONFIG',
    ...
    public function onExtensionAfterInstall(Extension\AfterInstallEvent $event): void
    {
        ...
        if ($this->getApplication()->getLanguage()->hasKey(strtoupper('PLG_CODEX_ACTIONLOG_' . $extensionType . '_INSTALLED'))) {
            $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_' . $extensionType . '_INSTALLED';
        } else {
            $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_EXTENSION_INSTALLED';
        }

        $message = [
            'action'         => 'install',
            'type'           => 'PLG_CODEX_ACTIONLOG_TYPE_' . $extensionType,
            'id'             => $eid,
    ...
    public function onExtensionAfterUninstall(Extension\AfterUninstallEvent $event): void
    {
        ...
        // If the extension type has its own language key, use it, otherwise, use default language key
        if ($this->getApplication()->getLanguage()->hasKey(strtoupper('PLG_CODEX_ACTIONLOG_' . $extensionType . '_UNINSTALLED'))) {
            $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_' . $extensionType . '_UNINSTALLED';
        } else {
            $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_EXTENSION_UNINSTALLED';
        }
    ...
    public function onExtensionAfterUpdate(Extension\AfterUpdateEvent $event): void
    {
        ...
        // If the extension type has its own language key, use it, otherwise, use default language key
        if ($this->getApplication()->getLanguage()->hasKey('PLG_CODEX_ACTIONLOG_' . $extensionType . '_UPDATED')) {
            $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_' . $extensionType . '_UPDATED';
        } else {
            $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_EXTENSION_UPDATED';
        }
    ...
    public function onExtensionAfterSave(Model\AfterSaveEvent $event): void
    {
        ...
        $message      = [
            'action'         => $isNew ? 'add' : 'update',
            'type'           => 'PLG_CODEX_ACTIONLOG_TYPE_' . $params->type_title,
            'id'             => $table->$id_holder,
    ...
    public function onUserAfterSave(User\AfterSaveEvent $event): void
    {
        ...
        if (!$jUser->id) {
            $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_USER_REGISTERED';
            $action             = 'register';

            // Registration Activation
            if ($task === 'activate') {
                $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_USER_REGISTRATION_ACTIVATE';
        ...
        $message = [
            'action'      => $action,
            'type'        => 'PLG_CODEX_ACTIONLOG_TYPE_USER',
            'id'          => $user['id'],
            'title'       => $user['name'],
        ...
        if ($task === 'block' || $task === 'unblock') {
            $messageLanguageKey = $task === 'block' ? 'PLG_CODEX_ACTIONLOG_USER_BLOCK' : 'PLG_CODEX_ACTIONLOG_USER_UNBLOCK';
            $message['action']  = $task;
        ...
        if ($action === 'update') {
            ...
            if ($data !== null) {
                $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_USER_UNBLOCK';
                $action             = 'unblock';
                if ($data === 'block') {
                    $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_USER_BLOCK';
                    $action             = 'block';
                }
            }
        }
    ...
    public function onUserAfterSaveGroup(Model\AfterSaveEvent $event): void
    {
        ...
        $message = [
            'action'   => $action,
            'type'     => 'PLG_CODEX_ACTIONLOG_TYPE_USER_GROUP',
            'id'       => $table->id,
            'title'    => $table->title,
    ...
    public function onUserAfterLogin(User\AfterLoginEvent $event): void
    {
        ...
        $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_USER_LOGGED_IN';
        ...
        $message = [
            ...
            'app'         => 'PLG_CODEX_ACTIONLOG_APPLICATION_' . $this->getApplication()->getName(),
        ];
    ...
    public function onUserLoginFailure(User\LoginFailureEvent $event): void
    {
        ...
        $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_USER_LOGIN_FAILED';
    ...
    public function onUserLogout(User\LogoutEvent $event): void
    {
        ...
        $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_USER_LOGGED_OUT';
    ...
    public function onUserAfterRemind(User\AfterRemindEvent $event): void
    {
        ...
        $this->addLog([$message], 'PLG_CODEX_ACTIONLOG_USER_REMIND', $context, $user->id);
    }
    ...
    public function onAfterCheckin(Checkin\AfterCheckinEvent $event): void
    {
        ...
        $this->addLog([$message], 'PLG_CODEX_ACTIONLOG_USER_CHECKIN', $context, $user->id);
    }
    ...
    public function onAfterLogPurge(): void
    {
        ...
        $this->addLog([$message], 'PLG_CODEX_ACTIONLOG_USER_LOG', $context, $user->id);
    }
    ...
    public function onAfterLogExport(): void
    {
        ...
        $this->addLog([$message], 'PLG_CODEX_ACTIONLOG_USER_LOGEXPORT', $context, $user->id);
    }
    ...
    public function onAfterPurge(Cache\AfterPurgeEvent $event): void
    {
        ...
        $this->addLog([$message], 'PLG_CODEX_ACTIONLOG_USER_CACHE', $context, $user->id);
    }
    ...
    public function onAfterDispatch(): void
    {
        ...
        $this->addLog([$message], 'PLG_CODEX_ACTIONLOG_API', $context, $user->id);
    }
    ...
    public function onJoomlaAfterUpdate(Event $event): void
    {
        ...
        $messageKey = ($user->id) ? 'PLG_CODEX_ACTIONLOG_USER_UPDATE' : 'PLG_CODEX_ACTIONLOG_SYSTEM_UPDATE';
    ...
    public function onUserAfterResetRequest(User\AfterResetRequestEvent $event): void
    {
        ...
        $this->addLog([$message], 'PLG_CODEX_ACTIONLOG_USER_RESET_REQUEST', $context, $user->id);
    }
    ...
    public function onUserAfterResetComplete(User\AfterResetCompleteEvent $event)
    {
        ...
        $this->addLog([$message], 'PLG_CODEX_ACTIONLOG_USER_RESET_COMPLETE', $context, $user->id);
    }
    ...
    public function onBeforeTourSaveUserState(AbstractEvent $event): void
    {
        ...
        switch ($state) {
            case 'skipped':
                $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_GUIDEDTOURS_TOURSKIPPED';
                break;
            case 'completed':
                $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_GUIDEDTOURS_TOURCOMPLETED';
                break;
            default:
                $messageLanguageKey = 'PLG_CODEX_ACTIONLOG_GUIDEDTOURS_TOURDELAYED';
        }
