...
class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null): void
    {
        ...
        if ($id) {
            ...
            $query = $db->createQuery()->select(['p.*', 'c.default_image AS category_default_image'])->from('#__codex AS p')
                ->join('INNER', '#__codex_tag_map AS m ON m.content_item_id=p.id')
                ->join('LEFT', '#__codex_categories AS c ON c.id=p.catid')
                ->where('m.type_alias=' . $db->quote('com_codex.post'))->where('m.tag_id=' . (int) $id)
                ...
                ->where('EXISTS (SELECT 1 FROM #__codex_categories c2 WHERE c2.id=p.catid AND c2.published=1 AND c2.access IN (' . implode(',', $groups) . '))');
            ...
            foreach ($this->items as $item) {
                ...
                $app->triggerEvent('onContentPrepare', ['com_codex.tags', &$item, &$item->params, 0]);
