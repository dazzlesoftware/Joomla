...
final class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null): void
    {
        ...
        $selected = \Joomla\Component\Codex\Site\Helper\ListingFilterHelper::ids($this->params->get('listing_categories', []));
        ...
        if ($headerId) {
            $categoryQuery = $db->createQuery()->select('*')->from('#__codex_categories')
        ...
        \Joomla\Component\Codex\Site\Helper\ListingFilterHelper::apply($q, $this->params, 'p', $id);
        \Joomla\Component\Codex\Site\Helper\ListingSettingsHelper::order($q, $this->params);
        if ($app->getLanguageFilter()) {
        ...
        $limit = \Joomla\Component\Codex\Site\Helper\ListingSettingsHelper::count($this->params);
        if ($layoutName !== 'default') {
            $posts = \Joomla\Component\Codex\Site\Helper\ListingSettingsHelper::count($this->params);
        ...
        $this->subcategories = \Joomla\Component\Codex\Site\Helper\SubcategoriesHelper::load((int) $this->category->id, $this->params);
    ...
    {
        $numLeading = 0;
        $numIntro = \Joomla\Component\Codex\Site\Helper\ListingSettingsHelper::count($this->params);
