...
namespace Joomla\Component\Codex\Site\View\Form;
...
use Joomla\Component\Codex\Site\Model\FormModel;
...
class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null)
    {
        ...
        if (empty($this->item->id)) {
            ...
            if ($this->state->get('params')->get('enable_category') == 1 && $catid) {
                $authorised = $user->authorise('core.create', 'com_codex.category.' . $catid);
            } else {
                $authorised = $user->authorise('core.create', 'com_codex') || \count($user->getAuthorisedCategories('com_codex', 'core.create'));
            }
        } else {
        ...
        if (empty($this->item->id)) {
            $authorised = (bool) $authorised && \Joomla\Component\Codex\Site\Helper\SubmissionHelper::hasAccess($user);
        }
        ...
        if (!empty($this->item->id)) {
            $this->item->tags->getItemTags('com_codex.post', $this->item->id);
        ...
        if (
            $this->item->id
            && \Joomla\Component\Codex\Site\Helper\SubmissionHelper::hasAccess($user)
            && ($user->authorise('core.create', 'com_codex') || \count($user->getAuthorisedCategories('com_codex', 'core.create')))
        ) {
    ...
    protected function _prepareDocument()
    {
        ...
        if ($menu) {
            ...
            $this->params->def('page_heading', Text::_('COM_CODEX_FORM_EDIT_POST'));
        }

        $title = $this->params->def('page_title', Text::_('COM_CODEX_FORM_EDIT_POST'));
