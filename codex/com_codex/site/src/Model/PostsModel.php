...
class PostsModel extends ListModel
{
    ...
    protected function populateState($ordering = 'ordering', $direction = 'ASC')
    {
        ...
        if ((!$user->authorise('core.edit.state', 'com_codex')) && (!$user->authorise('core.edit', 'com_codex'))) {
            // Filter on published for those who do not have edit or edit.state rights.
            $this->setState('filter.published', CodexComponent::CONDITION_PUBLISHED);
        }
    ...
    protected function getListQuery()
    {
        ...
        $conditionArchived    = CodexComponent::CONDITION_ARCHIVED;
        $conditionUnpublished = CodexComponent::CONDITION_UNPUBLISHED;
        ...
        $query->select(
            ...
            ->from($db->quoteName('#__codex', 'a'))
            ->join('LEFT', $db->quoteName('#__codex_categories', 'c'), $db->quoteName('c.id') . ' = ' . $db->quoteName('a.catid'))
            ...
            ->join('LEFT', $db->quoteName('#__codex_categories', 'parent'), $db->quoteName('parent.id') . ' = ' . $db->quoteName('c.parent_id'));
        ...
        if (is_numeric($categoryId)) {
            ...
            if ($includeSubcategories) {
                ...
                $subQuery = $db->createQuery()
                    ...
                    ->from($db->quoteName('#__codex_categories', 'sub'))
                    ->join(
                        'INNER',
                        $db->quoteName('#__codex_categories', 'this'),
        ...
        if (
            !(is_numeric($condition) && $condition == CodexComponent::CONDITION_UNPUBLISHED)
            && !(\is_array($condition) && \in_array(CodexComponent::CONDITION_UNPUBLISHED, $condition))
        ) {
            $query->where(
                [
        ...
        if (\is_array($tagId)) {
            ...
            if ($tagId) {
                $subQuery = $db->createQuery()
                    ...
                    ->from($db->quoteName('#__codex_tag_map'))
                    ->where(
                        [
                            $db->quoteName('tag_id') . ' IN (' . implode(',', $query->bindArray($tagId)) . ')',
                            $db->quoteName('type_alias') . ' = ' . $db->quote('com_codex.post'),
                        ]
                    );
            ...
            $query->join(
                'INNER',
                $db->quoteName('#__codex_tag_map', 'tagmap'),
                $db->quoteName('tagmap.content_item_id') . ' = ' . $db->quoteName('a.id')
                    . ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_codex.post')
            )
                ->where($db->quoteName('tagmap.tag_id') . ' = :tagId')
                ->bind(':tagId', $tagId, ParameterType::INTEGER);
        }

        \Joomla\Component\Codex\Site\Helper\ListingFilterHelper::apply($query, $this->getState('params'), 'a');
    ...
    public function getItems()
    {
        ...
        $globalParams = ComponentHelper::getParams('com_codex', true);
        ...
        foreach ($items as $item) {
            ...
            // Get display date
            $item->displayDate = \Joomla\Component\Codex\Site\Helper\DateHelper::value($item, $item->params);
        ...
        if ($taggedItems) {
            ...
            foreach ($tagsHelper->getMultipleItemTags('com_codex.post', $itemIds) as $id => $tags) {
