...
class CategoryModel extends ListModel
{
     ...
     * @var  string
     */
    protected $_context = 'com_codex.category';
    ...
    protected function populateState($ordering = null, $direction = null)
    {
        ...
        $value = $this->getUserStateFromRequest('com_codex.category.filter.' . $itemid . '.tag', 'filter_tag', 0, 'int', false);
        $this->setState('filter.tag', $value);

        // Optional filter text
        $search = $app->getUserStateFromRequest('com_codex.category.list.' . $itemid . '.filter-search', 'filter-search', '', 'string');
        $this->setState('list.filter', $search);

        // Filter.order
        $orderCol = $app->getUserStateFromRequest('com_codex.category.list.' . $itemid . '.filter_order', 'filter_order', '', 'string');
        ...
        $listOrder = $app->getUserStateFromRequest('com_codex.category.list.' . $itemid . '.filter_order_Dir', 'filter_order_Dir', '', 'cmd');
        ...
        if (($app->getInput()->get('layout') === 'card') || $params->get('layout_type') === 'card') {
            $limit = \Joomla\Component\Codex\Site\Helper\ListingSettingsHelper::count($params) + $params->get('num_links');
            $this->setState('list.links', $params->get('num_links'));
        } else {
            $limit = $app->getUserStateFromRequest('com_codex.category.list.' . $itemid . '.limit', 'limit', $params->get('display_num'), 'uint');
        }
    ...
    public function getItems()
    {
        ...
        if ($this->_posts === null && $category = $this->getCategory()) {
            ...
            $model = $this->bootComponent('com_codex')->getMVCFactory()
    ...
    protected function _buildContentOrderBy()
    {
        ...
        $itemid    = $app->getInput()->get('id', 0, 'int') . ':' . $app->getInput()->get('Itemid', 0, 'int');
        $orderCol  = $app->getUserStateFromRequest('com_codex.category.list.' . $itemid . '.filter_order', 'filter_order', '', 'string');
        $orderDirn = $app->getUserStateFromRequest('com_codex.category.list.' . $itemid . '.filter_order_Dir', 'filter_order_Dir', '', 'cmd');
        $orderby   = ' ';
    ...
    public function getCategory()
    {
        if (!\is_object($this->_item)) {
            ...
            $categories  = Categories::getInstance('Codex', $options);
            $this->_item = $categories->get($this->getState('category.id', 'root'));

            // Compute selected asset permissions.
            if (\is_object($this->_item)) {
                $user  = $this->getCurrentUser();
                $asset = 'com_codex.category.' . $this->_item->id;
