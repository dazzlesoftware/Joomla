 ...
 * @since  1.5
 */
class FormModel extends \Joomla\Component\Codex\Administrator\Model\PostModel
{
    /**
    ...
    public function getItem($itemId = null)
    {
        ...
        if ($itemId) {
            ...
            if ($catId) {
                $value->params->set('access-change', $user->authorise('core.edit.state', 'com_codex.category.' . $catId));
                $value->catid = $catId;
            } else {
                $value->params->set('access-change', $user->authorise('core.edit.state', 'com_codex'));
            }
        }
        ...
        if ($itemId) {
            ...
            $value->tags->getTagIds($value->id, 'com_codex.post');
    ...
    public function save($data)
    {
        ...
        if (
            ...
            && $associations = Associations::getAssociations('com_codex', '#__codex', 'com_codex.item', $data['id'])
        ) {
    ...
    public function getForm($data = [], $loadData = true)
    {
        ...
        if ($this->getState('post.id') && Associations::isEnabled()) {
            $associations = Associations::getAssociations('com_codex', '#__codex', 'com_codex.item', $id);
        ...
        if ($params && $params->get('enable_category') == 1 && $params->get('catid')) {
            ...
            if (Multilanguage::isEnabled()) {
                ...
                $query = $db->createQuery()
                    ...
                    ->from($db->quoteName('#__codex_categories'))
