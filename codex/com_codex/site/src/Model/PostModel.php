...
use Joomla\Component\Codex\Administrator\Table\PostTable;
...
class PostModel extends ItemModel
{
     ...
     * @var        string
     */
    protected $_context = 'com_codex.post';
    ...
    protected function populateState()
    {
        ...
        $asset = empty($pk) ? 'com_codex' : 'com_codex.post.' . $pk;

        if ((!$user->authorise('core.edit.state', $asset)) && (!$user->authorise('core.edit', $asset))) {
            $this->setState('filter.published', CodexComponent::CONDITION_PUBLISHED);
            $this->setState('filter.archived', CodexComponent::CONDITION_ARCHIVED);
        }
     ...
     *
     * @param   integer  $pk  The id of the post.
     *
    ...
    public function getItem($pk = null)
    {
        ...
        $pk = (int) ($pk ?: $this->getState('post.id'));
        ...
        if (!isset($this->_item[$pk])) {
            try {
                ...
                $query->select(
                    ...
                    ->join(
                        'INNER',
                        $db->quoteName('#__codex_categories', 'c'),
                        $db->quoteName('c.id') . ' = ' . $db->quoteName('a.catid')
                    )
                    ->join('LEFT', $db->quoteName('#__codex_frontpage', 'fp'), $db->quoteName('fp.content_id') . ' = ' . $db->quoteName('a.id'))
                ...
                if (
                    !$user->authorise('core.edit.state', 'com_codex.post.' . $pk)
                    && !$user->authorise('core.edit', 'com_codex.post.' . $pk)
                ) {
                ...
                if (empty($data)) {
                    throw new \Exception(Text::_('COM_CODEX_ERROR_POST_NOT_FOUND'), 404);
                }
                ...
                $registry = new Registry($data->options);

                $data->params = clone $this->getState('params');
                $globalParams = ComponentHelper::getParams('com_codex', true);

                /**
                 * For menu item parameters set to use_post, we will take value from post option and fallback
                 * to global value if the post option set to Use Global
                 */
    ...
    /**
     * Increment the hit counter for the post.
     *
     * @param   integer  $pk  Optional primary key of the post to increment.
     *
    ...
    public function hit($pk = 0)
    {
        ...
        if ($hitcount) {
            $pk = (!empty($pk)) ? $pk : (int) $this->getState('post.id');

            $table = new PostTable($this->getDatabase());
            $table->hit($pk);
        }
    ...
    public function storeVote($pk = 0, $rate = 0)
    {
        ...
        if ($rate >= 1 && $rate <= 5 && $pk > 0) {
            ...
            $query->select('*')
                ->from($db->quoteName('#__codex_rating'))
            ...
            if (!$rating) {
                ...
                $query->insert($db->quoteName('#__codex_rating'))
                    ->columns(
                        [
                ...
                if ($userIP != $rating->lastip) {
                    ...
                    $query->update($db->quoteName('#__codex_rating'))
                        ->set(
                            [
        ...
        Factory::getApplication()->enqueueMessage(Text::sprintf('COM_CODEX_INVALID_RATING', $rate), 'error');

        return false;
    }

    /**
     * Cleans the cache of com_codex and content modules
     *
