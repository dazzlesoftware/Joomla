...
final class PollsController extends BaseController
{
    public function vote(): void
    {
        ...
        if($identity->guest&&!ComponentHelper::getParams('com_codex')->get('poll_guest_voting',1)){$app->enqueueMessage('Please sign in to vote.','warning');$app->redirect($return);return;}
        ...
        $voter=$identity->guest?hash('sha256',($_SERVER['REMOTE_ADDR']??'').'|'.($_SERVER['HTTP_USER_AGENT']??'')):'user:'.$identity->id;$exists=(int)$db->setQuery($db->createQuery()->select('COUNT(*)')->from('#__codex_poll_votes')->where('poll_id='.$pollId)->where('voter_key='.$db->quote($voter)))->loadResult();
        if($exists){$app->enqueueMessage('You have already voted.','warning');$app->redirect($return);return;}
        if(!$poll->multiple)$choices=[reset($choices)];$recorded=0;foreach($choices as $choice){$valid=(int)$db->setQuery($db->createQuery()->select('COUNT(*)')->from('#__codex_poll_options')->where('id='.$choice)->where('poll_id='.$pollId))->loadResult();if($valid){$vote=(object)['poll_id'=>$pollId,'option_id'=>$choice,'voter_key'=>$voter,'created'=>Factory::getDate()->toSql()];$db->insertObject('#__codex_poll_votes',$vote);$recorded++;}}
