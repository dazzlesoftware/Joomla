...
class PostController extends FormController
{
    ...
    protected function allowAdd($data = [])
    {
        ...
        if ($categoryId) {
            ...
            $allow = $user->authorise('core.create', 'com_codex.category.' . $categoryId);
        }
    ...
    protected function allowEdit($data = [], $key = 'id')
    {
        ...
        if ($user->authorise('core.edit', 'com_codex.post.' . $recordId)) {
            return true;
        }

        // Check edit own on the record asset (explicit or inherited)
        if ($user->authorise('core.edit.own', 'com_codex.post.' . $recordId)) {
    ...
    public function cancel($key = 'a_id')
    {
        ...
        if ($customCancelRedir) {
            ...
            if ($cancelMenuitemId > 0) {
                ...
                // Redirect to the same post submission form (clean form).
    ...
    private function getDefaultRedirectLink()
    {
        ...
        switch ($type) {
            case 'archive':
                return 'index.php?option=com_codex&view=archive';

            case 'categories':
                return 'index.php?option=com_codex&view=categories';

            case 'category':
                $catId = (int) $params->get('default_post_redirect_category');

                if ($catId > 0) {
                    return 'index.php?option=com_codex&view=category&id=' . $catId;
                }
            ...
            case 'tags':
                return 'index.php?option=com_codex&view=tags';
        ...
        return 'index.php?option=com_codex&view=featured';
    }
    ...
    public function save($key = null, $urlVar = 'a_id')
    {
        ...
        $app       = $this->app;
        $postId = $app->getInput()->getInt('a_id');

        // Load the parameters.
        $params   = $app->getParams();
        $menuitem = (int) $params->get('redirect_menuitem');

        // Check for redirection after submission when creating a new post only
        if ($menuitem > 0 && $postId == 0) {
            $lang = '';
            ...
            // Redirect to the post page, use the redirect url set from parent controller
        } else {
            // If ok, redirect to the return page.
            if ($result) {
                $this->setRedirect(Route::_($this->getReturnPage(), false));
    ...
    public function vote()
    {
        ...
        if ($user_rating > -1) {
            ...
            if ($model->storeVote($id, $user_rating)) {
                $this->setRedirect($url, Text::_('COM_CODEX_POST_VOTE_SUCCESS'));
            } else {
                $this->setRedirect($url, Text::_('COM_CODEX_POST_VOTE_FAILURE'));
            }
        }
    }
}
