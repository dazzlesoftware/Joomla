...
if (!$items || !\Joomla\Component\Codex\Site\Helper\CompactPostsHelper::visible($params)) { return; }
Factory::getApplication()->getDocument()->getWebAssetManager()->useStyle('fontawesome')
    ->registerAndUseStyle('com_codex.compact-posts', 'com_codex/compact-posts.css', ['version' => 'auto']);
...
if ($params->get('compact_show_rating', 1)) {
    ...
    $ratings = $db->setQuery($db->createQuery()->select('*')->from('#__codex_rating')
