...
namespace Joomla\Component\Codex\Administrator\View\Posts;
...
use Joomla\Component\Codex\Administrator\Model\PostsModel;
...
class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null)
    {
        ...
        if (ComponentHelper::getParams('com_codex')->get('workflow_enabled')) {
    ...
    {
        $canDo    = ContentHelper::getActions('com_codex', 'category', $this->state->get('filter.category_id'));
        $user     = $this->getCurrentUser();
        $toolbar  = $this->getDocument()->getToolbar();

        ToolbarHelper::title(Text::_('COM_CODEX_POSTS_TITLE'), 'copy post');

        if ($canDo->get('core.create') || \count($user->getAuthorisedCategories('com_codex', 'core.create')) > 0) {
        ...
        if (!$this->isEmptyState && ($canDo->get('core.edit.state') || \count($this->transitions))) {
            ...
            if ($canDo->get('core.execute.transition') && \count($this->transitions)) {
                $childBar->separatorButton('transition-headline')
                    ->text('COM_CODEX_RUN_TRANSITIONS')
            ...
            if ($canDo->get('core.edit.state')) {
                ...
                if ($this->state->get('filter.published') != CodexComponent::CONDITION_TRASHED) {
            ...
            if (
                ...
                $childBar->popupButton('batch', 'JTOOLBAR_BATCH')
                    ->popupType('inline')
                    ->textHeader(Text::_('COM_CODEX_BATCH_OPTIONS'))
        ...
        if (!$this->isEmptyState && $this->state->get('filter.published') == CodexComponent::CONDITION_TRASHED && $canDo->get('core.delete')) {
            $toolbar->delete('posts.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
                ->message('JGLOBAL_CONFIRM_DELETE')
                ->listCheck(true);
        }

        if ($user->authorise('core.admin', 'com_codex') || $user->authorise('core.options', 'com_codex')) {
            $toolbar->preferences('com_codex');
        }
