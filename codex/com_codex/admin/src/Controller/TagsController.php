...
class TagsController extends BaseController
{
    public function save(): void
    {
        ...
        if ($db->setQuery('SELECT id FROM #__codex_tags WHERE alias=' . $db->quote($alias) . ' AND id<>' . $id)->loadResult()) { $app->enqueueMessage('This tag alias is already in use.', 'error'); $this->back($id); return; }
        ...
        $canState = $app->getIdentity()->authorise('core.edit.state', 'com_codex');
        ...
        $id ? $db->updateObject('#__codex_tags', $row, 'id') : $db->insertObject('#__codex_tags', $row, 'id');
    ...
    public function delete(): void
    {
        ...
        if (!$app->getIdentity()->authorise('core.delete', 'com_codex')) throw new \RuntimeException('Not authorised', 403);
        $id = $app->getInput()->post->getInt('id'); $db = TagsHelper::db();
        $db->transactionStart();
        try {
            $db->setQuery('DELETE FROM #__codex_tag_map WHERE tag_id=' . $id)->execute();
            $db->setQuery('DELETE FROM #__codex_tags WHERE id=' . $id)->execute();
            $db->transactionCommit();
        } catch (\Throwable $e) { $db->transactionRollback(); throw $e; }
        $app->enqueueMessage('Tag deleted. Posts were kept.'); $this->back();
    }
    private function back(int $id = 0): void { $this->setRedirect(Route::_('index.php?option=com_codex&view=tags' . ($id ? '&id=' . $id : ''), false)); }
}
