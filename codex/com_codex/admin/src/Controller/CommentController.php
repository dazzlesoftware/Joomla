...
final class CommentController extends BaseController
{
    ...
    {
        Factory::getApplication()->redirect(Route::_('index.php?option=com_codex&view=comment', false));
    }

    public function apply(): void
    {
        $id = $this->doSave();
        Factory::getApplication()->redirect(Route::_('index.php?option=com_codex&view=comment&id=' . $id, false));
    }

    public function save(): void
    {
        $this->doSave();
        Factory::getApplication()->redirect(Route::_('index.php?option=com_codex&view=comments', false));
    }
    ...
    private function doSave(): int
    {
        ...
        if (!$app->getIdentity()->authorise($id ? 'core.edit' : 'core.create', 'com_codex')) {
        ...
        if (!$postId || !(int) $db->setQuery('SELECT id FROM #__codex WHERE id=' . $postId)->loadResult()) {
        ...
        if ($id) {
            $old = $db->setQuery('SELECT ip_hash FROM #__codex_comments WHERE id=' . $id)->loadObject();
            if (!$old) {
                throw new \RuntimeException('Comment not found.', 404);
            }
            $row->ip_hash = $old->ip_hash;
            $db->updateObject('#__codex_comments', $row, 'id');
        } else {
            unset($row->ip_hash);
            $row->ip_hash = '';
            $db->insertObject('#__codex_comments', $row, 'id');
        }
