...
final class AutopostController extends BaseController
{
    ...
    public function connect(): void
    {
        ...
        $session->set('codex.oauth.'.$p.'.state', $state);
        ...
        if ($p === 'facebook') {
            ...
            $session->set('codex.oauth.twitter.verifier', $verifier);
    ...
    public function oauthCallback(): void
    {
        ...
        $expected = (string)$session->get('codex.oauth.'.$p.'.state', '');
        $session->clear('codex.oauth.'.$p.'.state');
    ...
    public function purge(): void
    {
        ...
        $db->setQuery($db->createQuery()->delete('#__codex_autopost_logs'))->execute();
        Factory::getApplication()->enqueueMessage('Autopost logs purged.');
        $this->setRedirect(Route::_('index.php?option=com_codex&view=autopostlogs', false));
    }
    ...
    private function twitter($params, string $code): void
    {
        ...
        $session = Factory::getApplication()->getSession();
        $verifier = (string)$session->get('codex.oauth.twitter.verifier', '');
        $session->clear('codex.oauth.twitter.verifier');
    ...
    {
        return Uri::root().'administrator/index.php?option=com_codex&task=autopost.oauthCallback&provider='.rawurlencode($p);
    }
    private function settings(string $p): string
    {
        return Route::_('index.php?option=com_codex&view=autopost&provider='.$p, false);
    }
    ...
    {
        if (!Factory::getApplication()->getIdentity()->authorise($action, 'com_codex')) {
    ...
    private function store($params): void
    {
        ...
        $q = $db->createQuery()->update('#__extensions')->set('params=:params')->where("type='component'")->where("element='com_codex'")->bind(':params', $json);
