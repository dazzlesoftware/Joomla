...
final class CommentsController extends BaseController
{
    ...
    private function batchState(int $state): void
    {
        ...
        if ($ids) {
            ...
            $db->setQuery(
                $db->createQuery()->update('#__codex_comments')->set('state=' . (int) $state)->whereIn('id', $ids)
            )->execute();
        }
        $app->enqueueMessage($ids ? 'Comment state updated.' : 'No comments selected.', $ids ? 'message' : 'warning');
        $app->redirect(Route::_('index.php?option=com_codex&view=comments', false));
    }
    ...
    public function delete(): void
    {
        ...
        if ($ids) {
            $db = Factory::getContainer()->get(DatabaseInterface::class);
            $db->setQuery($db->createQuery()->delete('#__codex_comments')->whereIn('id', $ids))->execute();
        }
