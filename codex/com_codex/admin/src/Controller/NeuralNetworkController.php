...
use Joomla\Component\Codex\Administrator\Helper\NeuralNetworkService;
...
class NeuralNetworkController extends BaseController
{
    private function guard(bool $image = false): object
    {
        ...
        if ($user->guest || !$params->get('ai_enabled', 0) || !$user->authorise('ai.generate', 'com_codex')) { throw new \RuntimeException('AI tools are disabled or you do not have permission to use them.', 403); }
        $id = $this->input->post->getInt('post_id');
        $catid = $this->input->post->getInt('catid');
        if ($id) {
            $db = Factory::getContainer()->get(DatabaseInterface::class);
            $post = $db->setQuery($db->getQuery(true)->select(['created_by','catid'])->from('#__codex')->where('id=' . $id))->loadObject();
            $allowed = $post && ($user->authorise('core.edit', 'com_codex.post.' . $id) || ($user->authorise('core.edit.own', 'com_codex.post.' . $id) && (int) $post->created_by === (int) $user->id));
        } else {
            $allowed = $user->authorise('core.create', $catid ? 'com_codex.category.' . $catid : 'com_codex');
        }
    ...
    public function generate(): void
    {
        $this->respond(function () {
            ...
            $usage = $session->get('com_codex.ai.usage', ['start' => time(), 'count' => 0]);
            ...
            $usage['count']++; $session->set('com_codex.ai.usage', $usage);
            ...
            if (isset($result['image'])) {
                ...
                $session->set('com_codex.ai.image', ['id' => $id, 'created' => time(), 'image' => $result['image']]);
    ...
    public function saveImage(): void
    {
        $this->respond(function () {
            $this->guard(true);
            $session = Factory::getApplication()->getSession();
            $pending = $session->get('com_codex.ai.image', []);
            ...
            $directory = $root;
            foreach (['generated', 'codex'] as $segment) {
                $directory .= '/' . $segment;
            ...
            $session->remove('com_codex.ai.image');
            $path = 'images/generated/codex/' . $name;
