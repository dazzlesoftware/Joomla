...
class ContentHelper extends \Joomla\CMS\Helper\ContentHelper
{
    ...
    {
        if ($form->getName() != 'com_codex.categorycom_codex') {
            return;
        }
        ...
        $component = Factory::getApplication()->bootComponent('com_codex');

        if (
            !$component instanceof WorkflowServiceInterface
            || !$component->isWorkflowActive('com_codex.post')
        ) {
        ...
        $query->select($db->quoteName('title'))
            ...
            ->where(
                [
                    ...
                    $db->quoteName('extension') . ' = ' . $db->quote('com_codex.post'),
                ]
            );
        ...
        $field->addOption('- ' . Text::_('COM_CODEX_WORKFLOWS') . ' -', ['disabled' => 'true']);
    }
}
