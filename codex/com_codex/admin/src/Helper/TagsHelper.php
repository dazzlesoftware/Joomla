...
final class TagsHelper
{
    ...
    {
        $id = (int) self::db()->setQuery('SELECT id FROM #__codex_tags WHERE is_default=1 AND published=1')->loadResult();
        return $id ? [(string) $id] : [];
    }
    ...
    public static function save(array $data): int
    {
        ...
        $alias = $base;
        $n = 2;
        while ($db->setQuery('SELECT id FROM #__codex_tags WHERE alias=' . $db->quote($alias) . ($id ? ' AND id<>' . $id : ''))->loadResult()) {
            $alias = $base . '-' . $n++;
        }

        $old = $id ? $db->setQuery('SELECT * FROM #__codex_tags WHERE id=' . $id)->loadObject() : null;
        if ($id && !$old) {
        ...
        if ($id) {
            $db->updateObject('#__codex_tags', $row, 'id');
        } else {
            $db->insertObject('#__codex_tags', $row, 'id');
        }

        if ($isDefault) {
            $db->setQuery('UPDATE #__codex_tags SET is_default=0 WHERE id<>' . (int) $row->id)->execute();
        }
    ...
    public static function publish(array $ids, int $state): int
    {
        ...
        $db = self::db();
        $db->setQuery(
            $db->createQuery()->update('#__codex_tags')->set('published=' . (int) $state)->whereIn('id', $ids)
        )->execute();
        return count($ids);
    }
    ...
    public static function makeDefault(int $id): bool
    {
        ...
        $db->transactionStart();
        try {
            $db->setQuery('UPDATE #__codex_tags SET is_default=0')->execute();
            $db->setQuery('UPDATE #__codex_tags SET is_default=1 WHERE id=' . $id)->execute();
    ...
    public static function removeDefault(array $ids): int
    {
        ...
        $db = self::db();
        $db->setQuery(
            $db->createQuery()->update('#__codex_tags')->set('is_default=0')->whereIn('id', $ids)
        )->execute();
        return count($ids);
    }
    ...
    public static function delete(array $ids): int
    {
        ...
        $db->transactionStart();
        try {
            $db->setQuery($db->createQuery()->delete('#__codex_tag_map')->whereIn('tag_id', $ids))->execute();
            $db->setQuery($db->createQuery()->delete('#__codex_tags')->whereIn('id', $ids))->execute();
            $db->transactionCommit();
        } catch (\Throwable $e) {
            $db->transactionRollback();
            throw $e;
        ...
        foreach ($values as $value) {
            ...
            if (ctype_digit($value)) {
                ...
                if (!$id) {
                    ...
                    $alias = $base;
                    $n = 2;
                    while ($db->setQuery('SELECT id FROM #__codex_tags WHERE alias=' . $db->quote($alias))->loadResult()) {
                        $alias = $base . '-' . $n++;
                    }
