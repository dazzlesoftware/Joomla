...
final class CategoriesHelper
{
    ...
    {
        $db = self::db();
        $q = $db->createQuery()->select('id AS value,title AS text')->from('#__codex_categories')->order('lft,title');
        if ($site) {
    ...
    public static function save(array $data): int
    {
        ...
        $n = 2;
        while ((int)$db->setQuery('SELECT id FROM #__codex_categories WHERE alias='.$db->quote($alias).($id ? ' AND id<>'.$id : ''))->loadResult()) {
            $alias = $base.'-'.$n++;
        }
        ...
        if ($id) {
            ...
            }$db->updateObject('#__codex_categories', $row, 'id');
        } else {
