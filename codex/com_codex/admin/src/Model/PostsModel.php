...
class PostsModel extends ListModel
{
    ...
    protected function getListQuery()
    {
        ...
        $query->select(
            ...
            ->join('LEFT', $db->quoteName('#__workflow_associations', 'wa'), $db->quoteName('wa.item_id') . ' = ' . $db->quoteName('a.id') . ' AND ' . $db->quoteName('wa.extension') . ' = ' . $db->quote('com_codex.post'))
            ...
            $subQuery = $db->createQuery()
                ...
                ->where(
                    [
                        ...
                        $db->quoteName('asso1.context') . ' = ' . $db->quote('com_codex.item'),
                    ]
                );
        ...
        if ($published !== '*') {
            if (is_numeric($published)) {
                ...
                $query->whereIn(
                    $db->quoteName('a.state'),
                    [
                        CodexComponent::CONDITION_PUBLISHED,
                        CodexComponent::CONDITION_UNPUBLISHED,
                    ]
                );
            }
        }
        ...
        if ($tag && \is_array($tag)) {
            ...
            if ($includeNone) {
                $subQuery2 = $db->createQuery()
                    ->select('DISTINCT ' . $db->quoteName('content_item_id'))
                    ->from($db->quoteName('#__codex_tag_map'))
                    ->where($db->quoteName('type_alias') . ' = ' . $db->quote('com_codex.post'));
                $query->join(
                    'LEFT',
            ...
            if ($tag === 0) {
                ...
                ->where(
                    [
                        ...
                        $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_codex.post'),
                    ]
                )
        ...
        try {
            if (\count($stage_ids) || \count($workflow_ids)) {
                ...
                foreach ($transitions as $key => $transition) {
                    if (!$user->authorise('core.execute.transition', 'com_codex.transition.' . (int) $transition['value'])) {
