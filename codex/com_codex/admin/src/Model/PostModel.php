...
use Joomla\Component\Codex\Administrator\Event\Model\FeatureEvent;
...
class PostModel extends AdminModel implements WorkflowModelInterface, VersionableModelInterface
{
     ...
     * @since  1.6
     */
    protected $text_prefix = 'COM_CODEX';

    /**
     * The type alias for this content type (for example, 'com_codex.post').
     *
     * @var    string
     * @since  3.2
     */
    public $typeAlias = 'com_codex.post';
     ...
     * @since  3.4.4
     */
    protected $associationsContext = 'com_codex.item';
    ...
    public function __construct($config = [], ?MVCFactoryInterface $factory = null, ?FormFactoryInterface $formFactory = null)
    {
        ...
        $this->setUpWorkflow('com_codex.post');
    }
    ...
    {
        // Check if the post was featured and update the #__codex_frontpage table
        if ($table->featured == 1) {
            $db    = $this->getDatabase();
            $query = $db->createQuery()
                ...
                ->from($db->quoteName('#__codex_frontpage'))
            ...
            if ($featured) {
                $query = $db->createQuery()
                    ->insert($db->quoteName('#__codex_frontpage'))
        ...
        $oldItem->load($oldId);
        $fields = FieldsHelper::getFields('com_codex.post', $oldItem, true);
        ...
        Factory::getApplication()->triggerEvent('onContentAfterSave', ['com_codex.post', &$this->table, false, $fieldsData]);
    }
    ...
    protected function batchMove($value, $pks, $contexts)
    {
        ...
        foreach ($pks as $pk) {
            ...
            $fields = FieldsHelper::getFields('com_codex.post', $this->table, true);
    ...
    protected function canDelete($record)
    {
        ...
        return $this->getCurrentUser()->authorise('core.delete', 'com_codex.post.' . (int) $record->id);
    }
    ...
    protected function canEditState($record)
    {
        ...
        if (!empty($record->id)) {
            return $user->authorise('core.edit.state', 'com_codex.post.' . (int) $record->id);
        }

        // New post, so check against the category.
        if (!empty($record->catid)) {
            return $user->authorise('core.edit.state', 'com_codex.category.' . (int) $record->catid);
        }
    ...
    public function getItem($pk = null)
    {
        if ($item = parent::getItem($pk)) {
            ...
            if (!empty($item->id)) {
                ...
                $item->tags->getTagIds($item->id, 'com_codex.post');
        ...
        if ($assoc) {
            ...
            if ($item->id != null) {
                $associations = Associations::getAssociations('com_codex', '#__codex', 'com_codex.item', $item->id);
    ...
    public function getForm($data = [], $loadData = true)
    {
        ...
        // Get the form.
        $form = $this->loadForm('com_codex.post', 'post', ['control' => 'jform', 'load_data' => $loadData]);
    ...
    protected function loadFormData()
    {
        ...
        $app  = Factory::getApplication();
        $data = $app->getUserState('com_codex.edit.post.data', []);
        ...
        if (empty($data)) {
            ...
            if ($this->getState('post.id') == 0) {
                $filters     = (array) $app->getUserState('com_codex.posts.filter');
        ...
        $this->preprocessData('com_codex.post', $data);
    ...
    {
        if (!$this->getCurrentUser()->authorise('core.admin', 'com_codex')) {
            if (isset($data['rules'])) {
                unset($data['rules']);
            }
        }
    ...
    public function save($data)
    {
        ...
        if (!empty($data['catid']) && !CategoriesHelper::validateCategoryId((int) $data['catid'], 'com_codex')) {
        ...
        if (\in_array($input->get('task'), ['add', 'apply', 'save', 'save2new']) && (!isset($data['id']) || (int) $data['id'] == 0)) {
            ...
            if ($table->load(['alias' => $data['alias'], 'catid' => $data['catid']])) {
                $msg = Text::_('COM_CODEX_SAVE_WARNING');
            }
        ...
        if (parent::save($data)) {
            ...
            if (isset($data['featured']) && !$this->bootComponent('com_codex')->isFunctionalityUsed('core.featured', 'com_codex.post')) {
                if (
                    !$this->featured(
            ...
            try {
                (new \Joomla\Component\Codex\Administrator\Service\AutopostService())->publish(
    ...
    public function featured($pks, $value = 0, $featuredUp = null, $featuredDown = null)
    {
        ...
        if (empty($pks)) {
            $this->setError(Text::_('COM_CODEX_NO_ITEM_SELECTED'));
        ...
        try {
            ...
            if ($value === 0) {
                ...
                $query = $db->createQuery()
                    ->delete($db->quoteName('#__codex_frontpage'))
                ...
                if (\count($oldFeatured)) {
                    $query = $db->createQuery()
                        ->update($db->quoteName('#__codex_frontpage'))
                        ->set(
                            [
    ...
    /**
     * Custom clean the cache of com_codex and content modules
     *
    ...
    {
        return $this->getCurrentUser()->authorise('core.create', 'com_codex');
    }

    /**
     * Delete #__codex_frontpage items if the deleted posts was featured
     *
    ...
    public function delete(&$pks)
    {
        ...
        if ($return) {
            // Now check to see if this posts was featured if so delete it from the #__codex_frontpage table
            $db    = $this->getDatabase();
            $query = $db->createQuery()
                ->delete($db->quoteName('#__codex_frontpage'))
                ->whereIn($db->quoteName('content_id'), $pks);
    ...
    protected function batchTags($value, $pks, $contexts, $removeTags = false)
    {
        ...
        $user = $this->getCurrentUser();
        foreach ($pks as $pk) {
            if (!$user->authorise('core.edit', 'com_codex.post.' . (int) $pk)) {
