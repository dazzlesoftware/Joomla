...
?>
<form action="<?php echo Route::_('index.php?option=com_codex&view=votes'); ?>" method="post" name="adminForm" id="adminForm">
    ...
    <div class="row g-2 align-items-center mb-3">
        ...
        <div class="col-md-2"><button class="btn btn-primary" type="submit" onclick="this.form.task.value='';this.form.limitstart.value=0;">Filter</button> <a class="btn btn-outline-secondary" href="<?php echo Route::_('index.php?option=com_codex&view=votes'); ?>">Clear</a></div>
    </div>
    <div class="table-responsive"><table class="table table-hover">
        ...
        <tbody><?php foreach ($this->items as $i => $item) : ?><tr>
            ...
            <th scope="row"><?php if ($item->post_title !== null) : ?><a href="<?php echo Route::_('index.php?option=com_codex&task=post.edit&id=' . (int) $item->post_id); ?>"><?php echo $escape($item->post_title); ?></a> <a class="small" target="_blank" rel="noopener noreferrer" href="<?php echo $escape(Uri::root() . 'index.php?option=com_codex&view=post&id=' . (int) $item->post_id); ?>">View page</a><?php else : ?>Deleted post<?php endif; ?> <span class="text-muted">#<?php echo (int) $item->post_id; ?></span></th>
            ...
            <td><?php echo HTMLHelper::_('date', $item->created, 'Y-m-d H:i'); ?></td><td><?php if (\Joomla\CMS\Factory::getApplication()->getIdentity()->authorise('core.edit', 'com_codex')) : ?><a href="<?php echo Route::_('index.php?option=com_codex&view=votes&layout=edit&id=' . (int) $item->id); ?>">Edit vote #<?php echo (int) $item->id; ?></a><?php else : ?><?php echo (int) $item->id; ?><?php endif; ?></td>
        </tr><?php endforeach; ?>
