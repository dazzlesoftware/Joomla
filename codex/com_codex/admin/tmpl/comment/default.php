...
/** @var \Joomla\Component\Codex\Administrator\View\Comment\HtmlView $this */

$item = $this->item;
$escape = static fn ($value) => htmlspecialchars((string) $value, ENT_QUOTES, 'UTF-8');
$created = $item->created !== '' ? str_replace(' ', 'T', substr($item->created, 0, 16)) : Factory::getDate()->format('Y-m-d\TH:i');
?>
<form action="<?php echo Route::_('index.php?option=com_codex&task=comment.save'); ?>" method="post" name="adminForm" id="adminForm">
    <div class="alert alert-light border mb-3">
        <h1 class="h4 mb-1"><?php echo Text::_($item->id ? 'COM_CODEX_EDIT_COMMENT' : 'COM_CODEX_ADD_COMMENT'); ?></h1>
        <p class="mb-0 small text-muted"><?php echo Text::_('COM_CODEX_COMMENT_EDIT_DESC'); ?></p>
    </div>
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="row g-3 mb-3">
                <div class="col-md-6">
                    <label class="form-label" for="jform_name"><?php echo Text::_('COM_CODEX_FIELD_COMMENT_NAME_LABEL'); ?></label>
                    <input required maxlength="255" class="form-control" id="jform_name" name="jform[name]" value="<?php echo $escape($item->name); ?>">
                </div>
                <div class="col-md-6">
                    <label class="form-label" for="jform_email"><?php echo Text::_('COM_CODEX_FIELD_COMMENT_EMAIL_LABEL'); ?></label>
                    <input type="email" maxlength="255" class="form-control" id="jform_email" name="jform[email]" value="<?php echo $escape($item->email); ?>">
                </div>
            </div>

            <div class="mb-3">
                <label class="form-label" for="jform_body"><?php echo Text::_('COM_CODEX_FIELD_COMMENT_BODY_LABEL'); ?></label>
            ...
            <div class="row g-3 mb-0">
                <div class="col-md-6">
                    ...
                    <select class="form-select" id="jform_state" name="jform[state]">
                        <option value="1"<?php echo $item->state ? ' selected' : ''; ?>><?php echo Text::_('COM_CODEX_COMMENT_APPROVED'); ?></option>
                        <option value="0"<?php echo !$item->state ? ' selected' : ''; ?>><?php echo Text::_('COM_CODEX_COMMENT_PENDING'); ?></option>
                    </select>
                </div>
                <div class="col-md-6">
                    <label class="form-label" for="jform_created"><?php echo Text::_('COM_CODEX_FIELD_COMMENT_CREATED_LABEL'); ?></label>
