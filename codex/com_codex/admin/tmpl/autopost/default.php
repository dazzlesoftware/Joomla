...
$callback = Uri::root() . 'administrator/index.php?option=com_codex&task=autopost.oauthCallback&provider=' . $p;
$connect = Route::_('index.php?option=com_codex&task=autopost.connect&provider=' . $p . '&' . $token . '=1');
$disconnect = Route::_('index.php?option=com_codex&task=autopost.disconnect&provider=' . $p . '&' . $token . '=1');
...
<nav class="nav nav-tabs mb-4">
    <?php foreach ($labels as $key => $label) : ?>
        <a class="nav-link <?php echo $key === $p ? 'active' : ''; ?>" href="<?php echo Route::_('index.php?option=com_codex&view=autopost&provider=' . $key); ?>"><?php echo $label; ?></a>
    <?php endforeach; ?>
    <a class="nav-link" href="<?php echo Route::_('index.php?option=com_codex&view=autopostlogs'); ?>">Logs</a>
</nav>

<form action="<?php echo Route::_('index.php?option=com_codex&task=autopost.save'); ?>" method="post" id="adminForm" name="adminForm">
