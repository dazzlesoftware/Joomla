...
use Joomla\Component\Codex\Administrator\Button\CategoryDefaultButton;

/** @var \Joomla\Component\Codex\Administrator\View\Categories\HtmlView $this */

$user = $this->getCurrentUser();
$canOrder = $user->authorise('core.edit.state', 'com_codex');
?>
<form action="<?php echo Route::_('index.php?option=com_codex&view=categories'); ?>" method="post" name="adminForm" id="adminForm">
    <div class="alert alert-light border mb-3">
        <h1 class="h4 mb-1"><?php echo Text::_('COM_CODEX_CATEGORIES_TITLE'); ?></h1>
        <p class="mb-0 small text-muted"><?php echo Text::_('COM_CODEX_CATEGORIES_DESC'); ?></p>
    </div>
    ...
    <?php else : ?>
        <div class="table-responsive">
            <table class="table table-hover" id="categoryList">
                <caption class="visually-hidden"><?php echo Text::_('COM_CODEX_CATEGORIES_TABLE_CAPTION'); ?></caption>
                <thead>
                    <tr>
                        ...
                        <th scope="col" class="w-5 text-center">
                            <?php echo HTMLHelper::_('searchtools.sort', 'COM_CODEX_HEADING_DEFAULT', 'c.is_default', $this->listDirn, $this->listOrder); ?>
                        </th>
                        <th scope="col" class="w-5 text-center">
                            <?php echo HTMLHelper::_('searchtools.sort', 'JSTATUS', 'c.published', $this->listDirn, $this->listOrder); ?>
                        </th>
                        <th scope="col" class="w-5 text-center d-none d-md-table-cell">
                            <?php echo HTMLHelper::_('searchtools.sort', 'COM_CODEX_HEADING_POSTS', 'post_count', $this->listDirn, $this->listOrder); ?>
                        </th>
                        <th scope="col" class="w-10 text-center d-none d-md-table-cell">
                            <?php echo HTMLHelper::_('searchtools.sort', 'COM_CODEX_HEADING_SUBCATEGORIES', 'sub_count', $this->listDirn, $this->listOrder); ?>
                        </th>
                ...
                <?php foreach ($this->items as $i => $item) : ?>
                    <tr class="row<?php echo $i % 2; ?>">
                        ...
                        <th scope="row">
                            <a href="<?php echo Route::_('index.php?option=com_codex&view=category&id=' . (int) $item->id); ?>">
                                <?php echo htmlspecialchars($item->title, ENT_QUOTES, 'UTF-8'); ?>
                            </a>
                        </th>
                        <td class="text-center">
                            <?php
                            $defaultOptions = ['task_prefix' => 'categories.', 'disabled' => !$this->getCurrentUser()->authorise('core.edit.state', 'com_codex'), 'id' => 'cat-default-' . $item->id];
