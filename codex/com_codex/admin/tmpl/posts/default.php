...
use Joomla\Component\Codex\Administrator\Button\PostStateButton;
...
use Joomla\Component\Codex\Administrator\Helper\ContentHelper;
...
if ($saveOrder && !empty($this->items)) {
    $controller = $featured === '1' ? 'featured' : 'posts';
    $saveOrderingUrl = 'index.php?option=com_codex&task=' . $controller . '.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
    HTMLHelper::_('draggablelist.draggable');
}

$workflow_enabled  = ComponentHelper::getParams('com_codex')->get('workflow_enabled');
...
if ($workflow_enabled) :
    ...
    ->useScript('com_codex.posts-status');

    $workflow_state    = Factory::getApplication()->bootComponent('com_codex')->isFunctionalityUsed('core.state', 'com_codex.post');
    $workflow_featured = Factory::getApplication()->bootComponent('com_codex')->isFunctionalityUsed('core.featured', 'com_codex.post');
...
<form action="<?php echo Route::_('index.php?option=com_codex&view=posts'); ?>" method="post" name="adminForm" id="adminForm">
    <div class="row">
        <div class="col-md-12">
...
?>
                ...
                <?php else : ?>
                    <table class="table itemList" id="postList">
                        <caption class="visually-hidden">
                            <?php echo $featured === '1' ? Text::_('COM_CODEX_FEATURED_TABLE_CAPTION') : Text::_('COM_CODEX_POSTS_TABLE_CAPTION'); ?>,
                            ...
                            <tr>
                                ...
                                <?php if ($assoc) : ?>
                                    <th scope="col" class="w-5 d-none d-md-table-cell">
                                        <?php echo HTMLHelper::_('searchtools.sort', 'COM_CODEX_HEADING_ASSOCIATION', 'association', $listDirn, $listOrder); ?>
                                    </th>
                                <?php endif; ?>
                                ...
                                <th scope="col" class="w-10 d-none d-md-table-cell text-center">
                                    <?php echo HTMLHelper::_('searchtools.sort', 'COM_CODEX_HEADING_DATE_' . strtoupper($orderingColumn), 'a.' . $orderingColumn, $listDirn, $listOrder); ?>
                                </th>
                                <?php if ($this->hits) : ?>
                        ...
                        <?php foreach ($this->items as $i => $item) :
                            ...
                            $assetId              = 'com_codex.post.' . $item->id;
                            $canCreate            = $user->authorise('core.create', 'com_codex.category.' . $item->catid);
                            $canEdit              = $user->authorise('core.edit', 'com_codex.post.' . $item->id);
                            $canCheckin           = $user->authorise('core.manage', 'com_checkin') || $item->checked_out == $userId || is_null($item->checked_out);
                            $canEditOwn           = $user->authorise('core.edit.own', 'com_codex.post.' . $item->id) && $item->created_by == $userId;
                            $canChange            = $user->authorise('core.edit.state', 'com_codex.post.' . $item->id) && $canCheckin;
                            $canExecuteTransition = $user->authorise('core.execute.transition', 'com_codex.post.' . $item->id);
                            $canEditCat           = $user->authorise('core.edit', 'com_codex.category.' . $item->catid);
                            $canEditOwnCat        = $user->authorise('core.edit.own', 'com_codex.category.' . $item->catid) && $item->category_uid == $userId;
                            $canEditParCat        = $user->authorise('core.edit', 'com_codex.category.' . $item->parent_category_id);
                            $canEditOwnParCat     = $user->authorise('core.edit.own', 'com_codex.category.' . $item->parent_category_id) && $item->parent_category_uid == $userId;
                                ...
                                <th scope="row" class="has-context">
                                    <div class="break-word">
                                        <?php if ($item->checked_out) : ?>
                                            <?php echo HTMLHelper::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'posts.', $canCheckin); ?>
                                        <?php endif; ?>
                                        <?php if ($canEdit || $canEditOwn) : ?>
                                            <a href="<?php echo Route::_('index.php?option=com_codex&task=post.edit&id=' . $item->id); ?>" title="<?php echo Text::_('JACTION_EDIT'); ?> <?php echo $this->escape($item->title); ?>">
                                        ...
                                        <div class="small">
                                            <?php
                                    $ParentCatUrl = Route::_('index.php?option=com_codex&view=categories&task=category.edit&id=' . $item->parent_category_id . '&extension=com_codex');
                            $CurrentCatUrl = Route::_('index.php?option=com_codex&view=categories&task=category.edit&id=' . $item->catid . '&extension=com_codex');
                            $EditCatTxt = Text::_('COM_CODEX_EDIT_CATEGORY');
                                ...
                                <?php if ($assoc) : ?>
                                    <td class="d-none d-md-table-cell">
                                        <?php if ($item->association) : ?>
                                            <?php echo HTMLHelper::_('codexadministrator.association', $item->id); ?>
                                        <?php endif; ?>
                                    </td>
                                <?php endif; ?>
                                <?php if (Multilanguage::isEnabled()) : ?>
                    ...
                    <?php
                    if (
                        $user->authorise('core.create', 'com_codex')
                        && $user->authorise('core.edit', 'com_codex')
                        && $user->authorise('core.edit.state', 'com_codex')
                    ) : ?>
