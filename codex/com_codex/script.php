...
class Com_CodexInstallerScript
{
    public function postflight(string $type, InstallerAdapter $adapter): bool
    {
        // Remove obsolete integration files after the NeuralNetwork rename on upgrades.
        foreach ([
            JPATH_ADMINISTRATOR . '/components/com_codex/src/Helper/AiService.php',
            JPATH_ADMINISTRATOR . '/components/com_codex/src/Helper/AiUiHelper.php',
            JPATH_ADMINISTRATOR . '/components/com_codex/src/Controller/AiController.php',
            JPATH_ROOT . '/components/com_codex/src/Controller/AiController.php',
            JPATH_ROOT . '/media/com_codex/js/ai-editor.js',
        ] as $obsolete) {
    ...
    public function uninstall(InstallerAdapter $adapter): bool
    {
        ...
        $query = $db->getQuery(true)
            ...
            ->where($db->quoteName('type_alias') . ' IN (' . implode(',', $db->quote(['com_codex.post', 'com_codex.category'])) . ')');
    ...
    private function registerContentTypes(): bool
    {
        ...
        foreach (['post', 'category'] as $section) {
            ...
            foreach ($row as $field => $value) {
                if (is_string($value)) {
                    $value = str_replace(['com_content', '#__content', 'Joomla\\\\Component\\\\Content'], ['com_codex', '#__codex', 'Joomla\\\\Component\\\\Codex'], $value);
                    $row->$field = $value;
                }
            }
            if ($section === 'post') {
            ...
            $row->type_title = 'Codex ' . $row->type_title;
    ...
    {
        $db = Joomla\CMS\Factory::getContainer()->get(DatabaseInterface::class);
        $columns = $db->getTableColumns('#__codex');
        if (!isset($columns['excerpt'])) {
            $db->setQuery('ALTER TABLE #__codex ADD excerpt MEDIUMTEXT NULL AFTER summary')->execute();
        }
        ...
        if (!isset($columns['language'])) {
            $db->setQuery("ALTER TABLE #__codex ADD language char(7) NOT NULL DEFAULT '*' AFTER featured")->execute();
        }
        return true;
    }
    ...
    {
        $db = Joomla\CMS\Factory::getContainer()->get(DatabaseInterface::class);
        $db->setQuery("CREATE TABLE IF NOT EXISTS #__codex_subscriber_suppressions (id int unsigned NOT NULL AUTO_INCREMENT,email varchar(320) NOT NULL,reason varchar(64) NOT NULL DEFAULT 'removed',created datetime NOT NULL,PRIMARY KEY (id),UNIQUE KEY idx_email (email)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci")->execute();
        $db->setQuery("CREATE TABLE IF NOT EXISTS #__codex_import_batches (id int unsigned NOT NULL AUTO_INCREMENT,source varchar(255) NOT NULL,created datetime NOT NULL,created_by int unsigned NOT NULL DEFAULT 0,imported_count int unsigned NOT NULL DEFAULT 0,PRIMARY KEY (id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci")->execute();
        $db->setQuery("CREATE TABLE IF NOT EXISTS #__codex_import_items (batch_id int unsigned NOT NULL,post_id int unsigned NOT NULL,PRIMARY KEY (batch_id,post_id),KEY idx_post (post_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci")->execute();
        $columns = $db->getTableColumns('#__codex_subscribers');
        $additions = [
            'confirm_token' => "ALTER TABLE #__codex_subscribers ADD confirm_token varchar(64) NOT NULL DEFAULT '' AFTER token",
            'confirmed' => "ALTER TABLE #__codex_subscribers ADD confirmed datetime NULL AFTER consented",
            'unsubscribed' => "ALTER TABLE #__codex_subscribers ADD unsubscribed datetime NULL AFTER confirmed",
            'consent_ip' => "ALTER TABLE #__codex_subscribers ADD consent_ip varchar(45) NOT NULL DEFAULT '' AFTER unsubscribed",
        ];
        ...
        $db->setQuery("UPDATE #__codex_subscribers SET confirmed=consented WHERE state=1 AND confirmed IS NULL")->execute();
        return true;
    }
    ...
    {
        $db = Joomla\CMS\Factory::getContainer()->get(DatabaseInterface::class);
        $columns = $db->getTableColumns('#__codex_campaigns');
        if (!isset($columns['segment'])) {
            $db->setQuery('ALTER TABLE #__codex_campaigns ADD segment varchar(32) NOT NULL DEFAULT ' . $db->quote('all') . ' AFTER state')->execute();
        }
        if (!isset($columns['scheduled_at'])) {
            $db->setQuery('ALTER TABLE #__codex_campaigns ADD scheduled_at datetime NULL AFTER segment')->execute();
        }
        $queueColumns = $db->getTableColumns('#__codex_mail_queue');
        $queueAdditions = [
            'tracking_token' => "ALTER TABLE #__codex_mail_queue ADD tracking_token char(48) NOT NULL DEFAULT '' AFTER token",
            'opened_count' => "ALTER TABLE #__codex_mail_queue ADD opened_count int unsigned NOT NULL DEFAULT 0 AFTER error",
            'clicked_count' => "ALTER TABLE #__codex_mail_queue ADD clicked_count int unsigned NOT NULL DEFAULT 0 AFTER opened_count",
            'last_opened' => "ALTER TABLE #__codex_mail_queue ADD last_opened datetime NULL AFTER clicked_count",
            'last_clicked' => "ALTER TABLE #__codex_mail_queue ADD last_clicked datetime NULL AFTER last_opened",
        ];
    ...
    private function repairAdminMenu(): bool
    {
        ...
        $query = $db->getQuery(true)
            ...
            ->where($db->quoteName('component_id') . ' = (SELECT ' . $db->quoteName('extension_id') . ' FROM ' . $db->quoteName('#__extensions') . ' WHERE ' . $db->quoteName('element') . ' = ' . $db->quote('com_codex') . ' AND ' . $db->quoteName('type') . ' = ' . $db->quote('component') . ')');
        $db->setQuery($query)->execute();
        $componentId = (int) $db->setQuery($db->getQuery(true)->select('extension_id')->from('#__extensions')->where('type=' . $db->quote('component'))->where('element=' . $db->quote('com_codex')))->loadResult();
        foreach ([
            'codex-posts-group' => ['view=posts'],
            'codex-autopost-group' => ['view=autopost&', 'view=autopostlogs'],
            'codex-marketing-group' => ['view=subscribers', 'view=newsletter', 'view=emailtemplates'],
            'codex-migration-group' => ['view=import', 'view=export'],
        ] as $alias => $needles) {
    ...
    private function seedComponentDefaults(): bool
    {
        ...
        $query = $db->getQuery(true)
            ...
            ->where($db->quoteName('element') . ' IN (' . implode(',', $db->quote(['com_content', 'com_codex'])) . ')');
        $rows = $db->setQuery($query)->loadAssocList('element', 'params');

        $current = trim((string) ($rows['com_codex'] ?? ''));
        ...
        $query = $db->getQuery(true)
            ...
            ->where($db->quoteName('type') . ' = ' . $db->quote('component'))
            ->where($db->quoteName('element') . ' = ' . $db->quote('com_codex'));
        $db->setQuery($query)->execute();
    ...
    private function ensureNativeTags(): bool
    {
        ...
        \Joomla\Component\Codex\Administrator\Helper\TagSchema::ensure(Joomla\CMS\Factory::getContainer()->get(DatabaseInterface::class));
        return true;
    }
    private function ensureNativeCategories(): bool
    {
        require_once __DIR__ . '/admin/src/Helper/CategorySchema.php';
        \Joomla\Component\Codex\Administrator\Helper\CategorySchema::ensure(Joomla\CMS\Factory::getContainer()->get(DatabaseInterface::class));
        return true;
    }
}
