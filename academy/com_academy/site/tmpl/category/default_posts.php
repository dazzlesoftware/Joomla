...
$wa->useScript('com_academy.posts-list');
...
$tagDb = TagsHelper::db();
$tagQuery = $tagDb->createQuery()->select('id AS value, title AS text')->from('#__academy_tags')->where('published=1')->whereIn('access', Factory::getApplication()->getIdentity()->getAuthorisedViewLevels())->where('language IN (' . $tagDb->quote('*') . ',' . $tagDb->quote(Factory::getApplication()->getLanguage()->getTag()) . ')')->order('title');
...
<form action="<?php echo htmlspecialchars(Uri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm" class="com-content-category__posts">
        ...
        <div class="com-content__filter btn-group">
            ...
            <?php else : ?>
                <label class="filter-search-lbl visually-hidden" for="filter-search">
                    <?php echo Text::_('COM_ACADEMY_' . $this->params->get('filter_field') . '_FILTER_LABEL'); ?>
                </label>
                <input type="text" 
                    name="filter-search" 
                    id="filter-search" 
                    ...
                    class="inputbox" 
                    placeholder="<?php echo Text::_('COM_ACADEMY_' . $this->params->get('filter_field') . '_FILTER_LABEL'); ?>"
                >
            <?php endif; ?>
                ...
                <tr>
                    ...
                    <?php if ($date = $this->params->get('list_show_date')) : ?>
                        <th scope="col" id="categorylist_header_date">
                            ...
                            <?php elseif ($date === 'modified') : ?>
                                <?php echo HTMLHelper::_('grid.sort', 'COM_ACADEMY_' . $date . '_DATE', 'a.modified', $listDirn, $listOrder); ?>
                            <?php elseif ($date === 'published') : ?>
                                <?php echo HTMLHelper::_('grid.sort', 'COM_ACADEMY_' . $date . '_DATE', 'a.publish_up', $listDirn, $listOrder); ?>
                            <?php endif; ?>
                        </th>
                    <?php endif; ?>
                    ...
                    <?php if ($this->params->get('list_show_votes', 0) && $this->vote) : ?>
                        <th scope="col" id="categorylist_header_votes">
                            <?php echo HTMLHelper::_('grid.sort', 'COM_ACADEMY_VOTES', 'rating_count', $listDirn, $listOrder); ?>
                        </th>
                    <?php endif; ?>
                    <?php if ($this->params->get('list_show_ratings', 0) && $this->vote) : ?>
                        <th scope="col" id="categorylist_header_ratings">
                            <?php echo HTMLHelper::_('grid.sort', 'COM_ACADEMY_RATINGS', 'rating', $listDirn, $listOrder); ?>
                        </th>
                    <?php endif; ?>
                    <?php if ($isEditable) : ?>
                        <th scope="col" id="categorylist_header_edit"><?php echo Text::_('COM_ACADEMY_EDIT_ITEM'); ?></th>
                    <?php endif; ?>
                </tr>
            </thead>
            <tbody>
            <?php foreach ($this->items as $i => $post) : ?>
                <?php if ($this->items[$i]->state == AcademyComponent::CONDITION_UNPUBLISHED) : ?>
                ...
                <th class="list-title" scope="row">
                    ...
                    <?php else : ?>
                        ...
                        <a href="<?php echo $link; ?>" class="register">
                            <?php echo Text::_('COM_ACADEMY_REGISTER_TO_READ_MORE'); ?>
                        </a>
                        <?php if (Associations::isEnabled() && $this->params->get('show_associations')) : ?>
                            <div class="cat-list-association">
                            <?php $associations = AssociationHelper::displayAssociations($post->id); ?>
                        ...
                        <?php endif; ?>
                    <?php endif; ?>
                    <?php if ($post->state == AcademyComponent::CONDITION_UNPUBLISHED) : ?>
                        <div>
                    ...
                    <td class="list-author">
                        <?php if (!empty($post->author) || !empty($post->created_by_alias)) : ?>
                            ...
                            <?php if (!empty($post->contact_link) && $this->params->get('link_author')) : ?>
                                ...
                                <?php else : ?>
                                    <?php echo Text::sprintf('COM_ACADEMY_WRITTEN_BY', HTMLHelper::_('link', $post->contact_link, $author)); ?>
                                <?php endif; ?>
                            <?php else : ?>
                                <?php if ($this->params->get('show_headings')) : ?>
                                    <?php echo $author; ?>
                                <?php else : ?>
                                    <?php echo Text::sprintf('COM_ACADEMY_WRITTEN_BY', $author); ?>
                                <?php endif; ?>
                            <?php endif; ?>
                        <?php endif; ?>
                    </td>
                ...
                <?php if ($this->params->get('list_show_votes', 0) && $this->vote) : ?>
                    <td class="list-votes">
                        <span class="badge bg-success">
                            ...
                            <?php else : ?>
                                <?php echo Text::sprintf('COM_ACADEMY_VOTES_COUNT', $post->rating_count); ?>
                            <?php endif; ?>
                        </span>
                    </td>
                <?php endif; ?>
                <?php if ($this->params->get('list_show_ratings', 0) && $this->vote) : ?>
                    <td class="list-ratings">
                        <span class="badge bg-warning text-light">
                            ...
                            <?php else : ?>
                                <?php echo Text::sprintf('COM_ACADEMY_RATINGS_COUNT', $post->rating); ?>
                            <?php endif; ?>
                        </span>
                    </td>
                <?php endif; ?>
                <?php if ($isEditable) : ?>
                    <td class="list-edit">
                        <?php if ($post->params->get('access-edit')) : ?>
                            <?php echo HTMLHelper::_('academyicon.edit', $post, $post->params); ?>
                        <?php endif; ?>
                    </td>
                <?php endif; ?>
                </tr>
    ...
    <?php if ($this->category->getParams()->get('access-create')) : ?>
        <?php echo HTMLHelper::_('academyicon.create', $this->category, $this->category->params); ?>
    <?php endif; ?>
