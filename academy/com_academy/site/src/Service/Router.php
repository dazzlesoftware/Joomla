...
class Router extends RouterView
{
    ...
    public function __construct(SiteApplication $app, AbstractMenu $menu, CategoryFactoryInterface $categoryFactory, DatabaseInterface $db)
    {
        ...
        $post->setKey('id')->setParent($category, 'catid');
        ...
        $preprocess = new PreprocessRules($post, '#__academy', 'id', 'catid');
    ...
    public function getCategorySegment($id, $query)
    {
        ...
        while ($current > 0 && !isset($path[$current])) {
            $row = $this->db->setQuery($this->db->createQuery()
                ->select('id, alias, parent_id')->from('#__academy_categories')->where('id=' . $current))->loadObject();
            if (!$row) { break; }
     ...
     *
     * @param   string  $id     ID of the post form to retrieve the segments for
     * @param   array   $query  The request that is built right now
     *
     * @return  array|string  The segments of this item
     *
    ...
    public function getCategoryId($segment, $query)
    {
        ...
        $base = $this->db->createQuery()->select('id, alias')->from('#__academy_categories')->where('parent_id=' . $parent);
        if ($this->noIDs) {
            $lookup = clone $base;
    ...
    public function getPostId($segment, $query)
    {
        if ($this->noIDs) {
            ...
            $dbquery->select($this->db->quoteName('id'))
                ->from($this->db->quoteName('#__academy'))
