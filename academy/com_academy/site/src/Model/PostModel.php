...
class PostModel extends ItemModel
{
     ...
     * @var        string
     */
    protected $_context = 'com_academy.post';
    ...
    protected function populateState()
    {
        ...
        $asset = empty($pk) ? 'com_academy' : 'com_academy.post.' . $pk;

        if ((!$user->authorise('core.edit.state', $asset)) && (!$user->authorise('core.edit', $asset))) {
            $this->setState('filter.published', AcademyComponent::CONDITION_PUBLISHED);
            $this->setState('filter.archived', AcademyComponent::CONDITION_ARCHIVED);
        }
        ...
        if (!isset($this->_item[$pk])) {
            try {
                ...
                $query->select(
                    ...
                    ->join(
                        'INNER',
                        $db->quoteName('#__academy_categories', 'c'),
                        $db->quoteName('c.id') . ' = ' . $db->quoteName('a.catid')
                    )
                    ->join('LEFT', $db->quoteName('#__academy_frontpage', 'fp'), $db->quoteName('fp.content_id') . ' = ' . $db->quoteName('a.id'))
                ...
                if (
                    !$user->authorise('core.edit.state', 'com_academy.post.' . $pk)
                    && !$user->authorise('core.edit', 'com_academy.post.' . $pk)
                ) {
                ...
                if (empty($data)) {
                    throw new \Exception(Text::_('COM_ACADEMY_ERROR_POST_NOT_FOUND'), 404);
                }
                ...
                if (!$user->guest) {
                    $userId = $user->id;
                    $asset  = 'com_academy.post.' . $data->id;
