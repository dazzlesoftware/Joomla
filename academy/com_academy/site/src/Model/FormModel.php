 ...
 * @since  1.5
 */
class FormModel extends \Joomla\Component\Academy\Administrator\Model\PostModel
{
    /**
    ...
    protected function populateState()
    {
        ...
        $this->setState('post.catid', $input->getInt('catid', $catId));
     ...
     *
     * @param   integer  $itemId  The id of the post.
     *
     * @return  mixed  Content item data object on success, false on failure.
     */
    public function getItem($itemId = null)
    {
        $itemId = (int) (!empty($itemId)) ? $itemId : $this->getState('post.id');
        ...
        $value->params = new Registry($value->options);

        // Compute selected asset permissions.
        $user   = $this->getCurrentUser();
        $userId = $user->id;
        $asset  = 'com_academy.post.' . $value->id;
        ...
        if ($itemId) {
            ...
            // New item.
            $catId = (int) $this->getState('post.catid');

            if ($catId) {
                $value->params->set('access-change', $user->authorise('core.edit.state', 'com_academy.category.' . $catId));
                $value->catid = $catId;
            } else {
                $value->params->set('access-change', $user->authorise('core.edit.state', 'com_academy'));
            }
        }

        $value->post_content = $value->summary;

        if (!empty($value->body)) {
            $value->post_content .= '<hr id="system-readmore">' . $value->body;
        }
        ...
        if ($itemId) {
            ...
            $value->tags->getTagIds($value->id, 'com_academy.post');
    ...
    public function save($data)
    {
        ...
        if (
            ...
            && $associations = Associations::getAssociations('com_academy', '#__academy', 'com_academy.item', $data['id'])
        ) {
    ...
    public function getForm($data = [], $loadData = true)
    {
        ...
        if ($id > 0 && !$user->authorise('core.edit.state', 'com_academy.post.' . $id)) {
            $form->setFieldAttribute('catid', 'readonly', 'true');
            $form->setFieldAttribute('catid', 'required', 'false');
            $form->setFieldAttribute('catid', 'filter', 'unset');
        }

        // Prevent messing with post language and category when editing existing post with associations
        if ($this->getState('post.id') && Associations::isEnabled()) {
            $associations = Associations::getAssociations('com_academy', '#__academy', 'com_academy.item', $id);
        ...
        if ($params && $params->get('enable_category') == 1 && $params->get('catid')) {
            ...
            if (Multilanguage::isEnabled()) {
                ...
                $query = $db->createQuery()
                    ...
                    ->from($db->quoteName('#__academy_categories'))
     ...
     * @throws  \Exception
     */
    public function getTable($name = 'Post', $prefix = 'Administrator', $options = [])
    {
