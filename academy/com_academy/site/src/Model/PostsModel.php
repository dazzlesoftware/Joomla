...
namespace Joomla\Component\Academy\Site\Model;
...
use Joomla\Component\Academy\Site\Helper\AssociationHelper;
...
/**
 * This models supports retrieving lists of posts.
 *
 * @since  1.6
 */
class PostsModel extends ListModel
{
    ...
    protected function populateState($ordering = 'ordering', $direction = 'ASC')
    {
        ...
        if ((!$user->authorise('core.edit.state', 'com_academy')) && (!$user->authorise('core.edit', 'com_academy'))) {
            // Filter on published for those who do not have edit or edit.state rights.
            $this->setState('filter.published', AcademyComponent::CONDITION_PUBLISHED);
        }

        $this->setState('filter.language', Multilanguage::isEnabled());

        // Process show_noauth parameter
        if ((!$params->get('show_noauth')) || (!ComponentHelper::getParams('com_academy')->get('show_noauth'))) {
    ...
    protected function getStoreId($id = '')
    {
        ...
        $id .= ':' . serialize($this->getState('filter.post_id'));
        $id .= ':' . $this->getState('filter.post_id.include');
    ...
    /**
     * Get the master query for retrieving a list of posts subject to the model state.
     *
    ...
    protected function getListQuery()
    {
        ...
        $conditionArchived    = AcademyComponent::CONDITION_ARCHIVED;
        $conditionUnpublished = AcademyComponent::CONDITION_UNPUBLISHED;
        ...
        $query->select(
            $this->getState(
                ...
                [
                    ...
                    $db->quoteName('a.summary'),
                    $db->quoteName('a.excerpt'),
                    $db->quoteName('a.body'),
                    ...
                    $db->quoteName('a.media'),
                    $db->quoteName('a.options'),
                    ...
                    $query->length($db->quoteName('a.body')) . ' AS ' . $db->quoteName('readmore'),
            ...
            ->select(
                [
                    ...
                    // Published/archived post in archived category is treated as archived post. If category is not published then force 0.
            ...
            ->join('LEFT', $db->quoteName('#__academy_categories', 'c'), $db->quoteName('c.id') . ' = ' . $db->quoteName('a.catid'))
            ...
            ->join('LEFT', $db->quoteName('#__academy_categories', 'parent'), $db->quoteName('parent.id') . ' = ' . $db->quoteName('c.parent_id'));

        $params      = $this->getState('params');
        $orderby_sec = $params->get('orderby_sec');

        // Join over the frontpage posts if required.
        $frontpageJoin = 'LEFT';
        ...
        if (is_numeric($condition) && $condition == 2) {
            /**
             * If category is archived then post has to be published or archived.
             * Or category is published then post has to be archived.
             */
        ...
        // Filter by a single or group of posts.
        $postId = $this->getState('filter.post_id');

        if (is_numeric($postId)) {
            $postId = (int) $postId;
            $type      = $this->getState('filter.post_id.include', true) ? ' = ' : ' <> ';
            $query->where($db->quoteName('a.id') . $type . ':postId')
                ->bind(':postId', $postId, ParameterType::INTEGER);
        } elseif (\is_array($postId)) {
            $postId = ArrayHelper::toInteger($postId);

            if ($this->getState('filter.post_id.include', true)) {
                $query->whereIn($db->quoteName('a.id'), $postId);
            } else {
                $query->whereNotIn($db->quoteName('a.id'), $postId);
            }
        }
        ...
        if (is_numeric($categoryId)) {
            ...
            if ($includeSubcategories) {
                ...
                $subQuery = $db->createQuery()
                    ...
                    ->from($db->quoteName('#__academy_categories', 'sub'))
                    ->join(
                        'INNER',
                        $db->quoteName('#__academy_categories', 'this'),
        ...
        if (is_numeric($checkedOut)) {
            if ($checkedOut == -1) {
                // Only checked out posts
                $query->where($db->quoteName('a.checked_out') . ' > 0');
            } elseif ($checkedOut == 0) {
                // Only not checked out posts
        ...
        if (
            !(is_numeric($condition) && $condition == AcademyComponent::CONDITION_UNPUBLISHED)
            && !(\is_array($condition) && \in_array(AcademyComponent::CONDITION_UNPUBLISHED, $condition))
        ) {
            $query->where(
                [
        ...
        if (\is_array($tagId)) {
            ...
            if ($tagId) {
                $subQuery = $db->createQuery()
                    ...
                    ->from($db->quoteName('#__academy_tag_map'))
                    ->where(
                        [
                            $db->quoteName('tag_id') . ' IN (' . implode(',', $query->bindArray($tagId)) . ')',
                            $db->quoteName('type_alias') . ' = ' . $db->quote('com_academy.post'),
                        ]
                    );
            ...
            $query->join(
                'INNER',
                $db->quoteName('#__academy_tag_map', 'tagmap'),
                $db->quoteName('tagmap.content_item_id') . ' = ' . $db->quoteName('a.id')
                    . ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_academy.post')
            )
    ...
    public function getItems()
    {
        ...
        $globalParams = ComponentHelper::getParams('com_academy', true);

        $taggedItems = [];

        // Convert the parameter fields into objects.
        foreach ($items as $item) {
            $postParams = new Registry($item->options);

            // Unpack readmore and layout params
            $item->alternative_readmore = $postParams->get('alternative_readmore');
            $item->layout               = $postParams->get('layout');

            $item->params = clone $this->getState('params');

            /**
             * For blogs, post params override menu item params only if menu param = 'use_post'
            ...
            if (
                ...
                foreach ($menuParamsArray as $key => $value) {
                    if ($value === 'use_post') {
                        // If the post has a value, use it
                        if ($postParams->get($key) != '') {
                            // Get the value from the post
                            $postArray[$key] = $postParams->get($key);
                        } else {
                            // Otherwise, use the global value
                            $postArray[$key] = $globalParams->get($key);
                        }
                    }
                }

                // Merge the selected post params
                if (\count($postArray) > 0) {
                    $postParams = new Registry($postArray);
                    $item->params->merge($postParams);
                }
            } else {
                // For non-blog layouts, merge all of the post params
                $item->params->merge($postParams);
            }
             ...
             */
            if (!$guest) {
                $asset = 'com_academy.post.' . $item->id;
            ...
            if ($access) {
                // If the access filter has been set, we already have only the posts this user can view.
        ...
        if ($taggedItems) {
            ...
            foreach ($tagsHelper->getMultipleItemTags('com_academy.post', $itemIds) as $id => $tags) {
