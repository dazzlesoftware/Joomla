...
use Joomla\Component\Academy\Site\Helper\QueryHelper;
...
/**
 * This models supports retrieving a category, the posts associated with the category,
...
class CategoryModel extends ListModel
{
    ...
    /**
     * Array of posts in the category
     *
     * @var \stdClass[]
     */
     ...
     * @var  string
     */
    protected $_context = 'com_academy.category';
    ...
    protected function populateState($ordering = null, $direction = null)
    {
        ...
        $value = $this->getUserStateFromRequest('com_academy.category.filter.' . $itemid . '.tag', 'filter_tag', 0, 'int', false);
        $this->setState('filter.tag', $value);

        // Optional filter text
        $search = $app->getUserStateFromRequest('com_academy.category.list.' . $itemid . '.filter-search', 'filter-search', '', 'string');
        $this->setState('list.filter', $search);

        // Filter.order
        $orderCol = $app->getUserStateFromRequest('com_academy.category.list.' . $itemid . '.filter_order', 'filter_order', '', 'string');
        ...
        $listOrder = $app->getUserStateFromRequest('com_academy.category.list.' . $itemid . '.filter_order_Dir', 'filter_order_Dir', '', 'cmd');
        ...
        if (($app->getInput()->get('layout') === 'blog') || $params->get('layout_type') === 'blog') {
            ...
            $limit = $app->getUserStateFromRequest('com_academy.category.list.' . $itemid . '.limit', 'limit', $params->get('display_num'), 'uint');
        }
     ...
     *
     * @return  array|bool  An array of posts or false if an error occurs.
     *
     * @since   1.5
     */
    public function getItems()
    {
        ...
        if ($this->_posts === null && $category = $this->getCategory()) {
            /**
            ...
            $model = $this->bootComponent('com_academy')->getMVCFactory()
                ->createModel('Posts', 'Site', ['ignore_request' => true]);
            ...
            // Filter.subcategories indicates whether to include posts from subcategories in the list or blog
            $model->setState('filter.subcategories', $this->getState('filter.subcategories'));
            $model->setState('filter.max_category_levels', $this->getState('filter.max_category_levels'));
            $model->setState('list.links', $this->getState('list.links'));

            if ($limit >= 0) {
                $this->_posts = $model->getItems();

                if ($this->_posts === false) {
    ...
    protected function _buildContentOrderBy()
    {
        ...
        $itemid    = $app->getInput()->get('id', 0, 'int') . ':' . $app->getInput()->get('Itemid', 0, 'int');
        $orderCol  = $app->getUserStateFromRequest('com_academy.category.list.' . $itemid . '.filter_order', 'filter_order', '', 'string');
        $orderDirn = $app->getUserStateFromRequest('com_academy.category.list.' . $itemid . '.filter_order_Dir', 'filter_order_Dir', '', 'cmd');
        $orderby   = ' ';
    ...
    public function getCategory()
    {
        if (!\is_object($this->_item)) {
            if (isset($this->state) && !empty($this->state->get('params'))) {
                $params                = $this->state->get('params');
                $options               = [];
                $options['countItems'] = $params->get('show_cat_num_posts', 1) || !$params->get('show_empty_categories_cat', 0);
                $options['access']     = $params->get('check_access_rights', 1);
            } else {
                $options['countItems'] = 0;
            }

            $categories  = Categories::getInstance('Academy', $options);
            $this->_item = $categories->get($this->getState('category.id', 'root'));

            // Compute selected asset permissions.
            if (\is_object($this->_item)) {
                $user  = $this->getCurrentUser();
                $asset = 'com_academy.category.' . $this->_item->id;
