...
final class FeaturedSliderHelper
{
    ...
    public static function items($params, int $categoryId = 0, ?DatabaseInterface $db = null): array
    {
        ...
        $q = $db->createQuery()->select('p.*, u.name AS author, c.title AS category_title, r.rating_sum, r.rating_count')
            ...
            ->join('LEFT', '#__academy_rating AS r ON r.content_id=p.id')
            ->join('LEFT', '#__academy_frontpage AS fp ON fp.content_id=p.id')
    ...
    public static function render($overrides, int $categoryId = 0, int $start = 0): string
    {
        ...
        foreach ($items as $item) {
            ...
            Factory::getApplication()->triggerEvent('onContentPrepare', ['com_academy.featuredslider', &$item, &$item->params, 0]);
        ...
        return LayoutHelper::render('featured.' . $style, ['items' => $items, 'params' => $params], JPATH_ROOT . '/components/com_academy/layouts');
    }
}
