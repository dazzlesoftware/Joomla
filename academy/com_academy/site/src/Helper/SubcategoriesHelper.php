...
final class SubcategoriesHelper
{
    public static function load(int $parentId, Registry $params): array
    {
        ...
        $posts = $db->createQuery()->select('p.catid, COUNT(*) AS numitems')->from('#__academy AS p')
        ...
        $query = $db->createQuery()->select('c.*, COALESCE(counts.numitems, 0) AS numitems')->from('#__academy_categories AS c')
