...
final class ListingSettingsHelper
{
    ...
    public static function order($query, $params): void
    {
        ...
        if (in_array($order, ['vote', 'rvote', 'rank', 'rrank'], true)) {
            $query->join('LEFT', '#__academy_rating AS ordering_rating ON ordering_rating.content_id=p.id');
        }
        if ($order === 'front') {
            $query->join('LEFT', '#__academy_frontpage AS ordering_front ON ordering_front.content_id=p.id');
        }
