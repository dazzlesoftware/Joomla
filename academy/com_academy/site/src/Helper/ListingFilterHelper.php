...
final class ListingFilterHelper
{
    ...
    private static function categories(array $ids, bool $children, $db): array
    {
        ...
        $parents = $db->setQuery($db->createQuery()->select('lft,rgt')->from('#__academy_categories')->whereIn('id', $ids))->loadObjectList();
        $ranges = [];
        foreach ($parents as $parent) { $ranges[] = '(lft >= ' . (int) $parent->lft . ' AND rgt <= ' . (int) $parent->rgt . ')'; }
        if (!$ranges) { return $ids; }
        $query = $db->createQuery()->select('id')->from('#__academy_categories')->where('(' . implode(' OR ', $ranges) . ')');
    ...
    public static function apply($query, $params, string $alias = 'p', int $categoryId = 0, ?DatabaseInterface $db = null): void
    {
        ...
        if ($tags) {
            $sub = $db->createQuery()->select('tm.content_item_id')->from('#__academy_tag_map AS tm')
                ->join('INNER', '#__academy_tags AS t ON t.id=tm.tag_id AND t.published=1')
                ->where('tm.type_alias=' . $db->quote('com_academy.post'))->where('tm.tag_id IN (' . implode(',', $tags) . ')');
