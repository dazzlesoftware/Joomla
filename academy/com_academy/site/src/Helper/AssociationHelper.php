...
abstract class AssociationHelper extends CategoryAssociationHelper
{
    ...
    public static function getAssociations($id = 0, $view = null, $layout = null)
    {
        ...
        if ($layout === null && $jinput->get('view') == $view && $component == 'com_academy') {
        ...
        if ($view === 'post') {
            if ($id) {
                ...
                foreach ($associations as $tag => $item) {
                    $return[$tag] = RouteHelper::getPostRoute($item->id, (int) $item->catid, $item->language, $layout);
                }
        ...
        if (($view === 'category' || $view === 'categories') && $id) {
            ...
            // find rows for this component's own native #__academy_categories table.
            $associations = Associations::getAssociations(
                'com_academy',
                '#__academy_categories',
                'com_academy.category',
                $id,
                'id',
                'alias',
                ''
    ...
    /**
     * Method to display in frontend the associations for a given post
     *
    ...
    public static function displayAssociations($id)
    {
        ...
        if ($associations = self::getAssociations($id, 'post')) {
