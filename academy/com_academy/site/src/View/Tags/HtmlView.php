...
class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null): void
    {
        ...
        if ($id) {
            ...
            $query = $db->createQuery()->select('p.*')->from('#__academy AS p')
                ...
                ->where('EXISTS (SELECT 1 FROM #__academy_categories c WHERE c.id=p.catid AND c.published=1 AND c.access IN (' . implode(',', $groups) . '))');
