...
class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null): void
    {
        ...
        $db = TagsHelper::db();
        $q = $db->createQuery()->select('*')->from('#__academy_tags')->where('published=1')->whereIn('access', $app->getIdentity()->getAuthorisedViewLevels())->where('language IN (' . $db->quote('*') . ',' . $db->quote($app->getLanguage()->getTag()) . ')')->order('title');
        ...
        if ($id) {
            ...
            $query = $db->createQuery()->select(['p.*', 'c.default_image AS category_default_image'])->from('#__academy AS p')
                ->join('INNER', '#__academy_tag_map AS m ON m.content_item_id=p.id')
                ->join('LEFT', '#__academy_categories AS c ON c.id=p.catid')
                ->where('m.type_alias=' . $db->quote('com_academy.post'))->where('m.tag_id=' . (int) $id)
                ...
                ->where('EXISTS (SELECT 1 FROM #__academy_categories c2 WHERE c2.id=p.catid AND c2.published=1 AND c2.access IN (' . implode(',', $groups) . '))');
