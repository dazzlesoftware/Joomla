...
namespace Joomla\Component\Academy\Site\View\Archive;
...
use Joomla\Component\Academy\Site\Model\ArchiveModel;
...
class HtmlView extends BaseHtmlView
{
    ...
    /**
     * An array containing archived posts
     *
     * @var   \stdClass[]
     */
    protected $items = [];
    ...
    public function display($tpl = null)
    {
        ...
        foreach ($items as $item) {
            ...
            Factory::getApplication()->triggerEvent('onContentPrepare', ['com_academy.archive', &$item, &$item->params, 0]);

            // Old plugins: Use processed text as summary
            $item->summary = $item->text;

            $results                        = Factory::getApplication()->triggerEvent('onContentAfterTitle', ['com_academy.archive', &$item, &$item->params, 0]);
            $item->event->afterDisplayTitle = trim(implode("\n", $results));

            $results                           = Factory::getApplication()->triggerEvent('onContentBeforeDisplay', ['com_academy.archive', &$item, &$item->params, 0]);
            $item->event->beforeDisplayContent = trim(implode("\n", $results));

            $results                          = Factory::getApplication()->triggerEvent('onContentAfterDisplay', ['com_academy.archive', &$item, &$item->params, 0]);
