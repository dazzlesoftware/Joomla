...
class FeedView extends AbstractView
{
    public function display($tpl = null)
    {
        ...
        $selected = \Joomla\Component\Academy\Site\Helper\ListingFilterHelper::ids($params->get('listing_categories', []));
        $headerId = $id ?: (count($selected) === 1 ? $selected[0] : 0);
        $category = (object) ['title' => $params->get('page_heading', 'Posts'), 'description' => '', 'language' => '*'];
        if ($headerId) {
            $cq = $db->createQuery()->select('*')->from('#__academy_categories')->where('id=' . $headerId)->where('published=1')->whereIn('access', $levels);
        ...
        \Joomla\Component\Academy\Site\Helper\ListingFilterHelper::apply($query, $params, 'p', $id);
        ...
        $document->link = Route::_('index.php?option=com_academy&view=category&layout=card' . ($id ? '&id=' . $id : '') . '&Itemid=' . $app->getInput()->getInt('Itemid'), false, Route::TLS_IGNORE, true);
        ...
        foreach ($items as $item) {
            ...
            if (!$params->get('feed_summary', 0)) {
                ...
                $app->triggerEvent('onContentPrepare', ['com_academy.category', &$item, &$params, 0]);
