...
$grid = $params->get('subcategories_style', 'list') === 'image_grid';
$columns = $params->get('subcategories_listing_layout', 'columns') === 'columns' ? max(2, min(6, (int) $params->get('subcategories_columns', 3))) : 1;
$masonry = $grid && $columns > 1 && $params->get('subcategories_column_style', 'grid') === 'masonry';
if ($grid) {
...
$render = static function (array $items) use (&$render, $params, $escape, $grid, $columns, $masonry): void {
    echo $grid ? '<ul class="category-subcategories category-directory list-unstyled row row-cols-1 row-cols-md-' . $columns . ' g-4 mb-4"' . ($masonry ? ' data-post-masonry' : '') . '>' : '<ul class="category-subcategories list-unstyled ms-3">';
    foreach ($items as $child) {
        $url = $escape(Route::_(RouteHelper::getCategoryRoute($child->id, $child->language)));
        echo $grid ? '<li class="col"><div class="card category-directory-card' . ($masonry ? '' : ' h-100') . '">' : '<li class="mb-3">';
        if ($grid) {
        ...
        if ($params->get('show_subcat_desc', 1) && $child->description !== '') {
            echo '<div class="category-desc">' . HTMLHelper::_('content.prepare', $child->description, '', 'com_academy.category') . '</div>';
        }
