...
$item = $displayData;
$date = \Joomla\Component\Academy\Site\Helper\DateHelper::value($item, $item->params ?? null);
    ...
    <div class="post-card-footer-details d-flex flex-column gap-1 small">
        <?php if ($item->params->get('show_author', 1)) : ?>
            <span><span class="fa-solid fa-user me-1" aria-hidden="true"></span><?php
                ...
                if ($item->params->get('link_author', 1)) {
                    $author = HTMLHelper::_('link', Route::_('index.php?option=com_academy&view=author&id=' . (int) $item->created_by), $author);
                }
                echo \Joomla\CMS\Language\Text::sprintf('COM_ACADEMY_WRITTEN_BY', $author);
            ?></span>
        <?php endif; ?>
        <?php if ($item->params->get('show_category', 1) && $category !== '') : ?>
            <span class="post-card-footer-category"><span class="fa-solid fa-folder-open me-1" aria-hidden="true"></span><?php
                ...
                echo \Joomla\CMS\Language\Text::sprintf('COM_ACADEMY_CATEGORY', $categoryText);
            ?></span>
        <?php endif; ?>
        <?php if ($date) : ?>
            <span class="post-card-footer-date"><span class="fa-solid fa-calendar me-1" aria-hidden="true"></span><?php echo \Joomla\Component\Academy\Site\Helper\DateHelper::render($item, $item->params); ?></span>
        <?php endif; ?>
    </div>
