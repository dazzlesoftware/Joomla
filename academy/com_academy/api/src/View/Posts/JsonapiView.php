...
namespace Joomla\Component\Academy\Api\View\Posts;
...
use Joomla\Component\Academy\Api\Helper\ContentHelper;
use Joomla\Component\Academy\Api\Serializer\ContentSerializer;
...
'created',
    ...
    {
        foreach (FieldsHelper::getFields('com_academy.post') as $field) {
    ...
    protected function prepareItem($item)
    {
        ...
        Factory::getApplication()->triggerEvent('onContentPrepare', ['com_academy.post', &$item, &$item->params]);

        foreach (FieldsHelper::getFields('com_academy.post', $item, true) as $field) {
        ...
        if (!empty($item->tags->tags)) {
            ...
            $item->tags = [];
            $tags       = new TagsHelper();
            $tagsIds    = $tags->getTagIds($item->id, 'com_academy.post');
