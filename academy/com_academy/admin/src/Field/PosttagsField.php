...
class PosttagsField extends ListField
{
    ...
    {
        $db = TagsHelper::db();
        $q = $db->createQuery()->select('id AS value, title AS text')->from('#__academy_tags')->order('title');
    ...
    protected function getInput()
    {
        ...
        if ($this->fieldname === 'tags' && $this->multiple && !$this->disabled && !$this->readonly) {
            ...
            if (Factory::getApplication()->getIdentity()->authorise('core.create', 'com_academy')) {
