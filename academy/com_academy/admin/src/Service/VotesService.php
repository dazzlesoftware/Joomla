...
final class VotesService
{
    ...
    public function save(int $id, int $postId, int $rating, ?int $userId = null): int
    {
        ...
        $db->transactionStart();
        try {
            if (!$db->setQuery($db->createQuery()->select('id')->from('#__academy')->where('id=' . $postId))->loadResult()) {
                throw new \InvalidArgumentException('The selected post does not exist.');
            }
            $old = $id ? $db->setQuery($db->createQuery()->select('*')->from('#__academy_rating_votes')->where('id=' . $id))->loadObject() : null;
            ...
            $duplicate = $db->setQuery($db->createQuery()->select('id')->from('#__academy_rating_votes')->where('post_id=' . $postId)->where('voter_key=' . $db->quote($voterKey))->where('id!=' . $id))->loadResult();
            ...
            if ($old) {
                ...
                $db->updateObject('#__academy_rating_votes', $row, 'id');
            } else {
                $row = (object) ['post_id' => $postId, 'rating' => $rating, 'voter_key' => $voterKey, 'created' => gmdate('Y-m-d H:i:s')];
                $db->insertObject('#__academy_rating_votes', $row, 'id');
                $id = (int) $row->id;
            }
    ...
    {
        $db = $this->db;
        $rating = $db->setQuery($db->createQuery()->select('COALESCE(SUM(rating),0) AS rating_sum, COUNT(*) AS rating_count')->from('#__academy_rating_votes')->where('post_id=' . $postId))->loadObject();
        $db->setQuery($db->createQuery()->delete('#__academy_rating')->where('content_id=' . $postId))->execute();
        if ((int) $rating->rating_count) {
            ...
            $db->insertObject('#__academy_rating', $row);
        }
    }
    public function delete(array $ids): int
    {
        ...
        $db->transactionStart();
        try {
            $votes = $db->setQuery($db->createQuery()->select(['id', 'post_id'])->from('#__academy_rating_votes')->whereIn('id', $ids))->loadObjectList();
            if (!$votes) { $db->transactionCommit(); return 0; }
            $db->setQuery($db->createQuery()->delete('#__academy_rating_votes')->whereIn('id', $ids))->execute();
