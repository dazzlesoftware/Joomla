...
class Icon
{
    ...
    public function create($category, $params, $attribs = [], $legacy = false)
    {
        ...
        $url = 'index.php?option=com_academy&task=post.add&return=' . base64_encode($uri) . '&a_id=0&catid=' . $category->id;
        ...
        $text .= Text::_('COM_ACADEMY_NEW_POST');
    ...
    public function edit($post, $params, $attribs = [], $legacy = false)
    {
        ...
        if (
            ...
            $tooltip      = Text::sprintf('COM_ACADEMY_CHECKED_OUT_BY', $checkoutUser->name)
                . ' <br> ' . $date;

            $text = LayoutHelper::render('academy.content.icons.edit_lock', ['post' => $post, 'tooltip' => $tooltip, 'legacy' => $legacy]);
        ...
        if ($post->state == Workflow::CONDITION_UNPUBLISHED) {
            $tooltip = Text::_('COM_ACADEMY_EDIT_UNPUBLISHED_POST');
        } else {
            $tooltip = Text::_('COM_ACADEMY_EDIT_PUBLISHED_POST');
        }

        $text = LayoutHelper::render('academy.content.icons.edit', ['post' => $post, 'tooltip' => $tooltip, 'legacy' => $legacy]);
