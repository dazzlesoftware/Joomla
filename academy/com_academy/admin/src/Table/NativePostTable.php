...
namespace Joomla\Component\Academy\Administrator\Table;
...
class NativePostTable extends Table implements CurrentUserInterface
{
    ...
    {
        $this->typeAlias = 'com_academy.post';

        parent::__construct('#__academy', 'id', $db, $dispatcher);
    ...
    protected function _getAssetName()
    {
        ...
        return 'com_academy.post.' . (int) $this->$k;
    }
    ...
    public function bind($array, $ignore = '')
    {
        if (array_key_exists('catid', (array) $array) && (int) $array['catid'] > 0) {
            $exists = (int) $this->getDatabase()->setQuery('SELECT id FROM #__academy_categories WHERE id=' . (int) $array['catid'])->loadResult();
        ...
        if (array_key_exists('tags', (array) $array)) {
            ...
            $create = Factory::getApplication()->getIdentity()->authorise('core.create', 'com_academy');
            if ($new !== '') {
    ...
    public function check()
    {
        ...
        if (trim($this->title) == '') {
            $this->setError(Text::_('COM_ACADEMY_WARNING_PROVIDE_VALID_NAME'));
    ...
    public function store($updateNulls = true)
    {
        ...
        if ($table->load(['alias' => $this->alias, 'catid' => $this->catid]) && ($table->id != $this->id || $this->id == 0)) {
            ...
            $this->setError(Text::_('COM_ACADEMY_ERROR_UNIQUE_ALIAS'));

            if ($table->state === -2) {
                $this->setError(Text::_('COM_ACADEMY_ERROR_UNIQUE_ALIAS_TRASHED'));
            }
    ...
    public function delete($pk = null)
    {
        ...
        try {
            ...
            $db->setQuery('DELETE FROM #__academy_tag_map WHERE content_item_id=' . $id . ' AND type_alias=' . $db->quote('com_academy.post'))->execute();
            $db->transactionCommit(true);
            return true;
        } catch (\Throwable $e) { $db->transactionRollback(true); throw $e; }
    }
