...
final class SettingsController extends BaseController
{
    ...
    {
        $this->setRedirect(Route::_('index.php?option=com_academy&view=dashboard', false));
    }
    ...
    private function saveSettings(bool $close): void
    {
        ...
        foreach (['openai', 'claude'] as $provider) {
            ...
            if ($key !== '') { $params->set('ai_' . $provider . '_secret', \Joomla\Component\Academy\Administrator\Helper\NeuralNetworkService::seal($key)); }
        ...
        $app->enqueueMessage(Text::_('COM_ACADEMY_SETTINGS_SAVED'));
        $view = $close ? 'dashboard' : 'settings';
        $this->setRedirect(Route::_('index.php?option=com_academy&view=' . $view, false));
    }
}
