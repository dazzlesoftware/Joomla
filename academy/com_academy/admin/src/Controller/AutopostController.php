...
final class AutopostController extends BaseController
{
    ...
    public function connect(): void
    {
        ...
        $session->set('academy.oauth.'.$p.'.state', $state);
        ...
        if ($p === 'facebook') {
            ...
            $session->set('academy.oauth.twitter.verifier', $verifier);
    ...
    public function oauthCallback(): void
    {
        ...
        $expected = (string)$session->get('academy.oauth.'.$p.'.state', '');
        $session->clear('academy.oauth.'.$p.'.state');
    ...
    public function purge(): void
    {
        ...
        $db->setQuery($db->createQuery()->delete('#__academy_autopost_logs'))->execute();
        Factory::getApplication()->enqueueMessage('Autopost logs purged.');
        $this->setRedirect(Route::_('index.php?option=com_academy&view=autopostlogs', false));
    }
    ...
    private function twitter($params, string $code): void
    {
        ...
        $session = Factory::getApplication()->getSession();
        $verifier = (string)$session->get('academy.oauth.twitter.verifier', '');
        $session->clear('academy.oauth.twitter.verifier');
    ...
    {
        return Uri::root().'administrator/index.php?option=com_academy&task=autopost.oauthCallback&provider='.rawurlencode($p);
    }
    private function settings(string $p): string
    {
        return Route::_('index.php?option=com_academy&view=autopost&provider='.$p, false);
    }
    ...
    {
        if (!Factory::getApplication()->getIdentity()->authorise($action, 'com_academy')) {
    ...
    private function store($params): void
    {
        ...
        $q = $db->createQuery()->update('#__extensions')->set('params=:params')->where("type='component'")->where("element='com_academy'")->bind(':params', $json);
