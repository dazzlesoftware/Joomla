...
final class SubscribersController extends BaseController
{
    ...
    public function edit(): void
    {
        ...
        Factory::getApplication()->redirect(Route::_('index.php?option=com_academy&view=subscriber&id=' . $ids[0], false));
    }
    ...
    public function state(): void
    {
        ...
        if ($ids) {
            ...
            $db->setQuery($db->createQuery()->update('#__academy_subscribers')->set('state=' . Factory::getApplication()->getInput()->getInt('value'))->whereIn('id', $ids))->execute();
        }
        $this->back();
    }
    ...
    public function delete(): void
    {
        ...
        if ($ids) {
            $db = Factory::getContainer()->get(DatabaseInterface::class);
            $emails = $db->setQuery($db->createQuery()->select('email')->from('#__academy_subscribers')->whereIn('id', $ids))->loadColumn();
            foreach ($emails as $email) {
                $existing = (int) $db->setQuery($db->createQuery()->select('id')->from('#__academy_subscriber_suppressions')->where('email=' . $db->quote(strtolower($email))))->loadResult();
                if (!$existing) {
            ...
            $db->setQuery($db->createQuery()->delete('#__academy_subscribers')->whereIn('id', $ids))->execute();
        }
        $this->back();
    }
    ...
    public function import(): void
    {
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            $blocked = (int) $db->setQuery($db->createQuery()->select('COUNT(*)')->from('#__academy_subscriber_suppressions')->where('email=' . $db->quote($email)))->loadResult();
            $exists = (int) $db->setQuery($db->createQuery()->select('COUNT(*)')->from('#__academy_subscribers')->where('email=' . $db->quote($email)))->loadResult();
            if ($blocked || $exists) {
                $skipped++;
                continue;
            }
    ...
    public function export(): void
    {
        $this->allowed();
        $db = Factory::getContainer()->get(DatabaseInterface::class);
        $rows = $db->setQuery($db->createQuery()->select(['email', 'name', 'state', 'created', 'consented', 'confirmed', 'unsubscribed'])->from('#__academy_subscribers')->order('email'))->loadAssocList();
        $app = Factory::getApplication();
        $app->setHeader('Content-Type', 'text/csv; charset=utf-8', true);
        $app->setHeader('Content-Disposition', 'attachment; filename="academy-subscribers.csv"', true);
        $app->sendHeaders();
