...
final class CommentController extends BaseController
{
    ...
    {
        $id = $this->doSave();
        Factory::getApplication()->redirect(Route::_('index.php?option=com_academy&view=comment&id=' . $id, false));
    }

    public function save(): void
    {
        $this->doSave();
        Factory::getApplication()->redirect(Route::_('index.php?option=com_academy&view=comments', false));
    }
    ...
    private function doSave(): int
    {
        ...
        if (!$postId || !(int) $db->setQuery('SELECT id FROM #__academy WHERE id=' . $postId)->loadResult()) {
        ...
        if ($id) {
            $old = $db->setQuery('SELECT ip_hash FROM #__academy_comments WHERE id=' . $id)->loadObject();
            if (!$old) {
                throw new \RuntimeException('Comment not found.', 404);
            }
            $row->ip_hash = $old->ip_hash;
            $db->updateObject('#__academy_comments', $row, 'id');
        } else {
            unset($row->ip_hash);
            $row->ip_hash = '';
            $db->insertObject('#__academy_comments', $row, 'id');
        }
