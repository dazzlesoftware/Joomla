...
use Joomla\Component\Academy\Administrator\Table\PostTable;
...
final class ImportController extends BaseController
{
    public function run(): void
    {
        ...
        if (!$preview) {
            ...
            $db->insertObject('#__academy_import_batches', $batch, 'id');
        ...
        foreach ($files as $file) {
            try {
                ...
                foreach ($posts as $post) {
                    if ($forced !== 'source') {
                        ...
                        $exists = (int)$db->setQuery($db->createQuery()->select('COUNT(*)')->from('#__academy')->where('alias='.$db->quote($alias)))->loadResult();
                    ...
                    if ($postId) {
                        ...
                        $db->insertObject('#__academy_import_items', $importItem);
                        $added++;
                    } else {
                        $skipped++;
                    }
            ...
            $db->setQuery($db->createQuery()->update('#__academy_import_batches')->set('imported_count='.$added)->where('id='.$batchId))->execute();
            $app->enqueueMessage("Imported $added post(s); skipped $skipped; failed $failed. Batch #$batchId can be rolled back below.");
        }$app->redirect(Route::_('index.php?option=com_academy&view=import', false));
    }
    public function database(): void
    {
        ...
        foreach ($posts as $post) {
            try {
                ...
                $db->insertObject('#__academy_import_items', $item);
                $added++;
            } catch (\Throwable $error) {
                $failed++;
    ...
    private function store(array $post, int $catid, string $duplicates): int
    {
        ...
        foreach ((array)($post['categories'] ?? []) as $category) {
            $match = (int)$db->setQuery($db->createQuery()->select('id')->from('#__academy_categories')->where('LOWER(title)='.$db->quote(mb_strtolower($category))))->loadResult();
            if ($match) {
                $targetCatid = $match;
                break;
            }
    ...
    private function downloadImage(string $url): ?string
    {
        ...
        }$folder = 'images/academy-imports/'.gmdate('Y/m');
    ...
    public function export(): void
    {
        ...
        $root->setAttribute('component', 'academy');
        ...
        foreach ($posts as $post) {
            ...
            foreach ((new TagsHelper())->getItemTags('com_academy.post', (int)$post->id) as $tag) {
        ...
        $app->setHeader('Content-Disposition', 'attachment; filename="academy-posts-'.gmdate('Y-m-d-His').'.xml"', true);
    ...
    public function rollback(): void
    {
        ...
        $db = Factory::getContainer()->get(DatabaseInterface::class);
        $ids = array_map('intval',$db->setQuery($db->createQuery()->select('post_id')->from('#__academy_import_items')->where('batch_id='.$batchId))->loadColumn());
        $table = new PostTable($db);
        foreach ($ids as $id) {
            $table->delete($id);
        }$db->setQuery($db->createQuery()->delete('#__academy_import_items')->where('batch_id='.$batchId))->execute();
        $db->setQuery($db->createQuery()->delete('#__academy_import_batches')->where('id='.$batchId))->execute();
