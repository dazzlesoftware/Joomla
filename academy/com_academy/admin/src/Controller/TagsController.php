...
class TagsController extends BaseController
{
    public function save(): void
    {
        ...
        if (!$app->getIdentity()->authorise($id ? 'core.edit' : 'core.create', 'com_academy')) throw new \RuntimeException('Not authorised', 403);
        ...
        if ($db->setQuery('SELECT id FROM #__academy_tags WHERE alias=' . $db->quote($alias) . ' AND id<>' . $id)->loadResult()) { $app->enqueueMessage('This tag alias is already in use.', 'error'); $this->back($id); return; }
        ...
        $canState = $app->getIdentity()->authorise('core.edit.state', 'com_academy');
        ...
        $id ? $db->updateObject('#__academy_tags', $row, 'id') : $db->insertObject('#__academy_tags', $row, 'id');
    ...
    public function delete(): void
    {
        ...
        if (!$app->getIdentity()->authorise('core.delete', 'com_academy')) throw new \RuntimeException('Not authorised', 403);
        $id = $app->getInput()->post->getInt('id'); $db = TagsHelper::db();
        $db->transactionStart();
        try {
            $db->setQuery('DELETE FROM #__academy_tag_map WHERE tag_id=' . $id)->execute();
            $db->setQuery('DELETE FROM #__academy_tags WHERE id=' . $id)->execute();
            $db->transactionCommit();
        } catch (\Throwable $e) { $db->transactionRollback(); throw $e; }
        $app->enqueueMessage('Tag deleted. Posts were kept.'); $this->back();
    }
    private function back(int $id = 0): void { $this->setRedirect(Route::_('index.php?option=com_academy&view=tags' . ($id ? '&id=' . $id : ''), false)); }
}
