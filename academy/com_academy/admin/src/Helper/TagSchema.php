...
final class TagSchema
{
    ...
    {
        $db->setQuery("CREATE TABLE IF NOT EXISTS #__academy_tags (id INT UNSIGNED NOT NULL AUTO_INCREMENT, title VARCHAR(255) NOT NULL, alias VARCHAR(191) NOT NULL, description TEXT NOT NULL, published TINYINT NOT NULL DEFAULT 1, access INT UNSIGNED NOT NULL DEFAULT 1, language VARCHAR(7) NOT NULL DEFAULT '*', params TEXT NOT NULL, PRIMARY KEY(id), UNIQUE KEY idx_alias(alias), KEY idx_state(published)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci")->execute();
        ...
        if ($db->setQuery('SELECT version FROM #__academy_tag_migrations WHERE version=1')->loadResult()) return;
            ...
            if ($legacy) {
                ...
                foreach ($tags as $tag) {
                    if (!$db->setQuery('SELECT id FROM #__academy_tags WHERE id=' . (int) $tag->id)->loadResult()) {
                        ...
                        $tag->alias = $base;
                        if ($db->setQuery('SELECT id FROM #__academy_tags WHERE alias=' . $db->quote($base))->loadResult()) $tag->alias .= '-' . $tag->id;
