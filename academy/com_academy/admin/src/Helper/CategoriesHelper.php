...
final class CategoriesHelper
{
    ...
    public static function save(array $data): int
    {
        ...
        $row = (object)['id' => $id ?: null,'title' => $title,'alias' => $alias,'description' => (string)($data['description'] ?? ''),'published' => (int)($data['published'] ?? 1),'access' => (int)($data['access'] ?? 1),'language' => (string)($data['language'] ?? '*'),'parent_id' => $parentId,'extension' => 'com_academy','is_default' => $isDefault,'allow_autoposting' => (int)($data['allow_autoposting'] ?? 1),'default_image' => (string)($data['default_image'] ?? ''),'default_tags' => (string)($data['default_tags'] ?? ''),'created_time' => Factory::getDate()->toSql(),'created_user_id' => $userId,'modified_time' => Factory::getDate()->toSql(),'modified_user_id' => $userId,'metadata' => '{}','params' => '{}'];
        if ($id) {
        ...
        if ($isDefault) {
            $db->setQuery('UPDATE #__academy_categories SET is_default=0 WHERE id<>' . (int) $row->id)->execute();
        }
    ...
    {
        return (int) self::db()->setQuery('SELECT id FROM #__academy_categories WHERE is_default=1 AND published=1')->loadResult();
    }
    ...
    public static function makeDefault(int $id): bool
    {
        ...
        $db->transactionStart();
        try {
            $db->setQuery('UPDATE #__academy_categories SET is_default=0')->execute();
            $db->setQuery('UPDATE #__academy_categories SET is_default=1 WHERE id=' . $id)->execute();
    ...
    public static function removeDefault(array $ids): int
    {
        ...
        self::db()->setQuery('UPDATE #__academy_categories SET is_default=0 WHERE id IN (' . implode(',', $ids) . ')')->execute();
        return count($ids);
    }
    ...
    /**
     * Stores an uploaded "Default Post Cover" image under images/academy_categories/ and
     * returns its site-relative path (e.g. "images/academy_categories/xxxx.jpg"), or ''
    ...
    public static function saveDefaultImage(array $file): string
    {
        ...
        $dir = JPATH_ROOT . '/images/academy_categories';
        if (!is_dir($dir)) {
        ...
        return 'images/academy_categories/' . $name;
    }
