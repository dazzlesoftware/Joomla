...
class ContentHelper extends \Joomla\CMS\Helper\ContentHelper
{
    ...
    {
        if ($form->getName() != 'com_academy.categorycom_academy') {
            return;
        }
        ...
        $component = Factory::getApplication()->bootComponent('com_academy');

        if (
            !$component instanceof WorkflowServiceInterface
            || !$component->isWorkflowActive('com_academy.post')
        ) {
        ...
        $query->select($db->quoteName('title'))
            ...
            ->where(
                [
                    ...
                    $db->quoteName('extension') . ' = ' . $db->quote('com_academy.post'),
                ]
            );
        ...
        $field->addOption('- ' . Text::_('COM_ACADEMY_WORKFLOWS') . ' -', ['disabled' => 'true']);
    }
}
