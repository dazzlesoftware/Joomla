...
class PostsModel extends ListModel
{
    ...
    protected function getListQuery()
    {
        ...
        $query->select(
            ...
            ->join('LEFT', $db->quoteName('#__workflow_associations', 'wa'), $db->quoteName('wa.item_id') . ' = ' . $db->quoteName('a.id') . ' AND ' . $db->quoteName('wa.extension') . ' = ' . $db->quote('com_academy.post'))
            ...
            $subQuery = $db->createQuery()
                ...
                ->where(
                    [
                        ...
                        $db->quoteName('asso1.context') . ' = ' . $db->quote('com_academy.item'),
                    ]
                );
        ...
        if ($published !== '*') {
            if (is_numeric($published)) {
                ...
                $query->whereIn(
                    $db->quoteName('a.state'),
                    [
                        AcademyComponent::CONDITION_PUBLISHED,
                        AcademyComponent::CONDITION_UNPUBLISHED,
                    ]
                );
            }
        }
        ...
        if ($tag && \is_array($tag)) {
            ...
            if ($includeNone) {
                $subQuery2 = $db->createQuery()
                    ->select('DISTINCT ' . $db->quoteName('content_item_id'))
                    ->from($db->quoteName('#__academy_tag_map'))
                    ->where($db->quoteName('type_alias') . ' = ' . $db->quote('com_academy.post'));
                $query->join(
                    'LEFT',
            ...
            if ($tag === 0) {
                ...
                ->where(
                    [
                        ...
                        $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_academy.post'),
                    ]
                )
        ...
        try {
            if (\count($stage_ids) || \count($workflow_ids)) {
                ...
                foreach ($transitions as $key => $transition) {
                    if (!$user->authorise('core.execute.transition', 'com_academy.transition.' . (int) $transition['value'])) {
    ...
    public function getItems()
    {
        ...
        foreach ($items as $item) {
            $item->typeAlias = 'com_academy.post';
