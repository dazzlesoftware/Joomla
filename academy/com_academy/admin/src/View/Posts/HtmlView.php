...
namespace Joomla\Component\Academy\Administrator\View\Posts;
...
use Joomla\Component\Academy\Administrator\Model\PostsModel;
...
class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null)
    {
        ...
        $this->vote          = PluginHelper::isEnabled('academy', 'vote');
        $this->hits          = ComponentHelper::getParams('com_academy')->get('record_hits', 1) == 1;

        if (!\count($this->items) && $this->isEmptyState = $model->getIsEmptyState()) {
            $this->setLayout('emptystate');
        }

        if (ComponentHelper::getParams('com_academy')->get('workflow_enabled')) {
        ...
        if ($this->getLayout() !== 'modal') {
            ...
            // In post associations modal we need to remove language filter if forcing a language.
    ...
    {
        $canDo    = ContentHelper::getActions('com_academy', 'category', $this->state->get('filter.category_id'));
        $user     = $this->getCurrentUser();
        $toolbar  = $this->getDocument()->getToolbar();

        ToolbarHelper::title(Text::_('COM_ACADEMY_POSTS_TITLE'), 'copy post');

        if ($canDo->get('core.create') || \count($user->getAuthorisedCategories('com_academy', 'core.create')) > 0) {
        ...
        if (!$this->isEmptyState && ($canDo->get('core.edit.state') || \count($this->transitions))) {
            ...
            if ($canDo->get('core.execute.transition') && \count($this->transitions)) {
                $childBar->separatorButton('transition-headline')
                    ->text('COM_ACADEMY_RUN_TRANSITIONS')
                    ->buttonClass('text-center py-2 h3');

                $cmd      = "Joomla.submitbutton('posts.runTransition');";
            ...
            if ($canDo->get('core.edit.state')) {
                ...
                $childBar->standardButton('featured', 'JFEATURE', 'posts.featured')->listCheck(true);

                $childBar->standardButton('unfeatured', 'JUNFEATURE', 'posts.unfeatured')->listCheck(true);

                $childBar->archive('posts.archive')->listCheck(true);

                $childBar->checkin('posts.checkin');

                if ($this->state->get('filter.published') != AcademyComponent::CONDITION_TRASHED) {
            ...
            if (
                ...
                $childBar->popupButton('batch', 'JTOOLBAR_BATCH')
                    ->popupType('inline')
                    ->textHeader(Text::_('COM_ACADEMY_BATCH_OPTIONS'))
        ...
        if (!$this->isEmptyState && $this->state->get('filter.published') == AcademyComponent::CONDITION_TRASHED && $canDo->get('core.delete')) {
            $toolbar->delete('posts.delete', 'JTOOLBAR_DELETE_FROM_TRASH')
                ->message('JGLOBAL_CONFIRM_DELETE')
                ->listCheck(true);
        }

        if ($user->authorise('core.admin', 'com_academy') || $user->authorise('core.options', 'com_academy')) {
            $toolbar->preferences('com_academy');
        }
