...
namespace Joomla\Component\Academy\Administrator\View\Import;
...
final class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null): void
    {
        ...
        $q = $db->createQuery()->select(['id','title','level'])->from('#__academy_categories')->where('published=1')->order('lft');
        $this->categories = $db->setQuery($q)->loadObjectList();
        $this->batches = $db->setQuery($db->createQuery()->select('*')->from('#__academy_import_batches')->order('created DESC'), 0, 20)->loadObjectList();
