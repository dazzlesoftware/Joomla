...
final class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null): void
    {
        ...
        $query = $db->createQuery()
            ->select([
                't.*',
                '(SELECT COUNT(*) FROM #__academy_tag_map m WHERE m.tag_id = t.id AND m.type_alias = ' . $db->quote('com_academy.post') . ') AS post_count',
                'u.name AS author_name',
            ])
            ->from('#__academy_tags AS t')
    ...
    protected function addToolbar(): void
    {
        ...
        ToolbarHelper::title(Text::_('COM_ACADEMY_TAGS_TITLE'), 'tag');
        ...
        if ($user->authorise('core.edit.state', 'com_academy')) {
            ...
            $toolbar->standardButton('star', 'COM_ACADEMY_MAKE_DEFAULT', 'tags.makedefault')->listCheck(true);
            $toolbar->standardButton('star-empty', 'COM_ACADEMY_REMOVE_DEFAULT', 'tags.removedefault')->listCheck(true);
        }
