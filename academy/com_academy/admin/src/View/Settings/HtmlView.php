...
namespace Joomla\Component\Academy\Administrator\View\Settings;
...
final class HtmlView extends BaseHtmlView
{
    ...
    public array $sections = [
        ...
        'lists' => ['label' => 'Post Lists & Blog Layouts', 'icon' => 'list', 'fieldsets' => ['list_display', 'listing_filters', 'featured_slider', 'blog_default_parameters', 'compact_posts', 'list_default_parameters', 'shared']],
    ...
    {
        if (!Factory::getApplication()->getIdentity()->authorise('core.options', 'com_academy')) {
        ...
        $dateParams = clone ComponentHelper::getParams('com_academy');
        [$showDate, $dateType] = \Joomla\Component\Academy\Site\Helper\DateHelper::options($dateParams);
        $dateParams->set('show_date', (int) $showDate);
        $dateParams->set('date_type', $dateType);
        $dateParams->set('posts_per_page', \Joomla\Component\Academy\Site\Helper\ListingSettingsHelper::count($dateParams));
        $this->form->bind(['params' => $dateParams->toArray()]);

        ToolbarHelper::title('Academy Settings', 'cog');
