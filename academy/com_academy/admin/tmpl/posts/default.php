...
use Joomla\Component\Academy\Administrator\Button\PostStateButton;
...
if ($saveOrder && !empty($this->items)) {
    ...
    $saveOrderingUrl = 'index.php?option=com_academy&task=' . $controller . '.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
    HTMLHelper::_('draggablelist.draggable');
}

$workflow_enabled  = ComponentHelper::getParams('com_academy')->get('workflow_enabled');
...
if ($workflow_enabled) :
    ...
    ->useScript('com_academy.posts-status');

    $workflow_state    = Factory::getApplication()->bootComponent('com_academy')->isFunctionalityUsed('core.state', 'com_academy.post');
    $workflow_featured = Factory::getApplication()->bootComponent('com_academy')->isFunctionalityUsed('core.featured', 'com_academy.post');
...
<form action="<?php echo Route::_('index.php?option=com_academy&view=posts'); ?>" method="post" name="adminForm" id="adminForm">
    <div class="row">
        <div class="col-md-12">
            <div id="j-main-container" class="j-main-container">
                <?php
                // Search tools bar
                echo LayoutHelper::render('academy.searchtools.default', ['view' => $this, 'options' => ['selectorFieldName' => 'featured']]);
?>
                ...
                <?php else : ?>
                    <table class="table itemList" id="postList">
                        <caption class="visually-hidden">
                            <?php echo $featured === '1' ? Text::_('COM_ACADEMY_FEATURED_TABLE_CAPTION') : Text::_('COM_ACADEMY_POSTS_TABLE_CAPTION'); ?>,
                            ...
                            <tr>
                                ...
                                <?php if ($assoc) : ?>
                                    <th scope="col" class="w-5 d-none d-md-table-cell">
                                        <?php echo HTMLHelper::_('searchtools.sort', 'COM_ACADEMY_HEADING_ASSOCIATION', 'association', $listDirn, $listOrder); ?>
                                    </th>
                                <?php endif; ?>
                                ...
                                <th scope="col" class="w-10 d-none d-md-table-cell text-center">
                                    <?php echo HTMLHelper::_('searchtools.sort', 'COM_ACADEMY_HEADING_DATE_' . strtoupper($orderingColumn), 'a.' . $orderingColumn, $listDirn, $listOrder); ?>
                                </th>
                                <?php if ($this->hits) : ?>
                        ...
                        <?php foreach ($this->items as $i => $item) :
                            ...
                            $assetId              = 'com_academy.post.' . $item->id;
                            $canCreate            = $user->authorise('core.create', 'com_academy.category.' . $item->catid);
                            $canEdit              = $user->authorise('core.edit', 'com_academy.post.' . $item->id);
                            $canCheckin           = $user->authorise('core.manage', 'com_checkin') || $item->checked_out == $userId || is_null($item->checked_out);
                            $canEditOwn           = $user->authorise('core.edit.own', 'com_academy.post.' . $item->id) && $item->created_by == $userId;
                            $canChange            = $user->authorise('core.edit.state', 'com_academy.post.' . $item->id) && $canCheckin;
                            $canExecuteTransition = $user->authorise('core.execute.transition', 'com_academy.post.' . $item->id);
                            $canEditCat           = $user->authorise('core.edit', 'com_academy.category.' . $item->catid);
                            $canEditOwnCat        = $user->authorise('core.edit.own', 'com_academy.category.' . $item->catid) && $item->category_uid == $userId;
                            $canEditParCat        = $user->authorise('core.edit', 'com_academy.category.' . $item->parent_category_id);
                            $canEditOwnParCat     = $user->authorise('core.edit.own', 'com_academy.category.' . $item->parent_category_id) && $item->parent_category_uid == $userId;
                                ...
                                <th scope="row" class="has-context">
                                    <div class="break-word">
                                        ...
                                        <?php if ($canEdit || $canEditOwn) : ?>
                                            <a href="<?php echo Route::_('index.php?option=com_academy&task=post.edit&id=' . $item->id); ?>" title="<?php echo Text::_('JACTION_EDIT'); ?> <?php echo $this->escape($item->title); ?>">
                                ...
                                <?php if ($assoc) : ?>
                                    <td class="d-none d-md-table-cell">
                                        <?php if ($item->association) : ?>
                                            <?php echo HTMLHelper::_('academyadministrator.association', $item->id); ?>
                                        <?php endif; ?>
                                    </td>
                                <?php endif; ?>
                                <?php if (Multilanguage::isEnabled()) : ?>
                    ...
                    if (
                        ...
                        && $user->authorise('core.edit', 'com_academy')
                        && $user->authorise('core.edit.state', 'com_academy')
                    ) : ?>
