...
namespace Joomla\Plugin\Academy\Vote\Extension;
...
final class Vote extends CMSPlugin implements SubscriberInterface
{
    ...
    /**
     * Displays the voting area when viewing an post and the voting section is displayed before the post.
     * Add HTML string containing code for the votes if in com_academy.
     *
    ...
    /**
     * Displays the voting area when viewing an post and the voting section is displayed after the post.
     * Add HTML string containing code for the votes if in com_academy.
     *
     ...
     * @param   object   $row     The post object
     * @param   object   $params  The post params
     * @param   integer  $page     The 'page' number
     *
     * @return  string  HTML string containing code for the votes if in com_academy else empty string
     *
     * @since   3.7.0
     */
    private function displayVotingData($context, $row, $params, $page)
    {
        ...
        if (!isset($row->rating, $row->rating_count) && !empty($row->id)) {
            ...
            $rating = $db->setQuery($db->createQuery()->select('rating_sum, rating_count')
                ->from('#__academy_rating')->where('content_id=' . (int) $row->id))->loadObject();
        ...
        $path = PluginHelper::getLayoutPath('academy', 'vote', 'rating');
        ...
        if ($this->getApplication()->getInput()->getString('view', '') === 'post' && $row->state == 1) {
            // Get the path for the voting form layout file
            $path = PluginHelper::getLayoutPath('academy', 'vote', 'vote');
    ...
    protected function prepareAggregateRating($context)
    {
        ...
        if ($view === 'post') {
            ...
            if ($post->rating_count > 0) {
                return ['@isPartOf' => ['@id' => $schemaId, 'aggregateRating' => ['@type' => 'AggregateRating','ratingCount' => (string) $post->rating_count,'ratingValue' => (string) $post->rating]]];
            }
        }
    ...
    protected function prepareProductAggregateRating($context)
    {
        ...
        if ($view === 'post') {
            ...
            $post   = $model->getItem($id);
            if ($post->rating_count > 0) {
                return ['@isPartOf' => ['@id' => $schemaId, '@type' => 'Product', 'name' => $post->title, 'aggregateRating' => ['@type' => 'AggregateRating', 'ratingCount' => (string) $post->rating_count, 'ratingValue' => (string) $post->rating]]];
            }
        }
