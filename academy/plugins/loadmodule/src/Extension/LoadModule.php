...
namespace Joomla\Plugin\Academy\Loadmodule\Extension;
...
/**
 * Plugin to enable loading modules into content (e.g. posts)
...
final class LoadModule extends CMSPlugin implements SubscriberInterface
{
    ...
    public function onContentPrepare(ContentPrepareEvent $event)
    {
        ...
        // Only execute if $post is an object and has a text property
        if (!\is_object($post) || !property_exists($post, 'text') || \is_null($post->text)) {
            return;
        }
        ...
        if ($context === 'com_finder.indexer') {
            if (str_contains($post->text, 'loadposition')) {
                $post->text = preg_replace($regex, '', $post->text);
            }

            if (str_contains($post->text, 'loadmoduleid')) {
                $post->text = preg_replace($regexmodid, '', $post->text);
            }

            if (str_contains($post->text, 'loadmodule')) {
                $post->text = preg_replace($regexmod, '', $post->text);
            }
        ...
        if (str_contains($post->text, '{loadposition ')) {
            ...
            preg_match_all($regex, $post->text, $matches, PREG_SET_ORDER);
            ...
            if ($matches) {
                foreach ($matches as $match) {
                    ...
                    if (($start = strpos($post->text, $match[0])) !== false) {
                        $post->text = substr_replace($post->text, $output, $start, \strlen($match[0]));
                    }
        ...
        if (str_contains($post->text, '{loadmodule ')) {
            ...
            preg_match_all($regexmod, $post->text, $matchesmod, PREG_SET_ORDER);
            ...
            if ($matchesmod) {
                foreach ($matchesmod as $matchmod) {
                    ...
                    // We should replace only first occurrence in order to allow positions with the same name to regenerate their content:
                    if (($start = strpos($post->text, $matchmod[0])) !== false) {
                        $post->text = substr_replace($post->text, $output, $start, \strlen($matchmod[0]));
                    }
        ...
        if (str_contains($post->text, '{loadmoduleid ')) {
            ...
            preg_match_all($regexmodid, $post->text, $matchesmodid, PREG_SET_ORDER);
