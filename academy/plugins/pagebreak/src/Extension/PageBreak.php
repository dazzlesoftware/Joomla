...
namespace Joomla\Plugin\Academy\Pagebreak\Extension;
...
final class PageBreak extends CMSPlugin implements SubscriberInterface
{
    ...
    public function onContentPrepare(ContentPrepareEvent $event)
    {
        ...
        $canProceed = $context === 'com_academy.post';
        ...
        if ($full || $view !== 'post' || $params->get('intro_only') || $params->get('popup')) {
        ...
        if ($n > 1) {
            ...
            if ($style === 'pages') {
                ...
                if ($style === 'tabs') {
                    $t[] = (string) HTMLHelper::_('uitab.startTabSet', 'myTab', ['active' => 'post' . $row->id . '-' . $style . '0', 'view' => 'tabs']);
                } else {
                    $t[] = (string) HTMLHelper::_('bootstrap.startAccordion', 'myAccordion', ['active' => 'post' . $row->id . '-' . $style . '0']);
                }

                foreach ($text as $key => $subtext) {
                    $index = 'post' . $row->id . '-' . $style . $key;
                    ...
                    if ($key >= 1) {
                        ...
                        if (isset($match['alt'])) {
                            ...
                            $title = Text::sprintf('PLG_ACADEMY_PAGEBREAK_PAGE_NUM', $key + 1);
                        }
     ...
     *
     * @param   object   &$row      The post object.  Note $post->text is also available
    ...
    {
        $heading     = $row->title ?? $this->getApplication()->getLanguage()->_('PLG_ACADEMY_PAGEBREAK_NO_TITLE');
        $input       = $this->getApplication()->getInput();
        $limitstart  = $input->getUint('limitstart', 0);
        $showall     = $input->getInt('showall', 0);
        $headingtext = '';

        if ($this->params->get('post_index', 1) == 1) {
            $headingtext = $this->getApplication()->getLanguage()->_('PLG_ACADEMY_PAGEBREAK_POST_INDEX');

            if ($this->params->get('post_index_text')) {
                $headingtext = htmlspecialchars($this->params->get('post_index_text'), ENT_QUOTES, 'UTF-8');
            }
        }

        // TOC first Page link.
        $this->list[1]         = new \stdClass();
        $this->list[1]->link   = RouteHelper::getPostRoute($row->slug, $row->catid, $row->language);
        ...
        foreach ($matches as $bot) {
            if (@$bot[0]) {
                ...
                if (@$attrs2['alt']) {
                    ...
                    $title = Text::sprintf('PLG_ACADEMY_PAGEBREAK_PAGE_NUM', $i);
                }
            } else {
                $title = Text::sprintf('PLG_ACADEMY_PAGEBREAK_PAGE_NUM', $i);
            }
            ...
            $this->list[$i]->link   = RouteHelper::getPostRoute($row->slug, $row->catid, $row->language) . '&limitstart=' . ($i - 1);
        ...
        if ($this->params->get('showall')) {
            $this->list[$i]         = new \stdClass();
            $this->list[$i]->link   = RouteHelper::getPostRoute($row->slug, $row->catid, $row->language) . '&showall=1';
            $this->list[$i]->title  = $this->getApplication()->getLanguage()->_('PLG_ACADEMY_PAGEBREAK_ALL_PAGES');
            $this->list[$i]->active = ($limitstart === $i - 1);
        }

        $list = $this->list;
        $path = PluginHelper::getLayoutPath('academy', 'pagebreak', 'toc');
        ob_start();
        include $path;
    ...
    private function createNavigation(&$row, $page, $n)
    {
        ...
        if ($page < $n - 1) {
            $links['next'] = RouteHelper::getPostRoute($row->slug, $row->catid, $row->language) . '&limitstart=' . ($page + 1);
        }

        if ($page > 0) {
            $links['previous'] = RouteHelper::getPostRoute($row->slug, $row->catid, $row->language);
        ...
        $path = PluginHelper::getLayoutPath('academy', 'pagebreak', 'navigation');
        ob_start();
        include $path;
