...
namespace Joomla\Plugin\Academy\Joomla\Extension;
...
final class Joomla extends CMSPlugin implements SubscriberInterface
{
    ...
    public function onContentAfterSave(AfterSaveEvent $event): void
    {
        ...
        foreach ($users as $user_id) {
            if ($user_id != $user->id) {
                ...
                if ($receiver->authorise('core.login.admin') && $receiver->authorise('core.manage', 'com_academy')) {
                    $templateData['url'] = Route::link('administrator', 'index.php?option=com_academy&view=posts&filter[search]=id:' . $post->id, false, $linkMode, true);
                } elseif ($post->state === 1 && \in_array($post->access, $receiver->getAuthorisedViewLevels())) {
                    $templateData['url'] = Route::link('site', 'index.php?option=com_academy&view=post&id=' . $post->id, false, $linkMode, true);
                } elseif ($post->state !== 1 && $receiver->authorise('core.edit', 'com_academy.post.' . $post->id)) {
                    $templateData['url'] = Route::link('site', 'index.php?option=com_academy&view=post&id=' . $post->id, false, $linkMode, true);
                } elseif ($post->state !== 1) {
                ...
                // Send email
                try {
                    $mailer = new MailTemplate('plg_academy_joomla.newpost', $receiver->getParam('admin_language', $this->getLanguage()->getTag()));
    ...
    public function onContentBeforeDelete(BeforeDeleteEvent $event)
    {
        ...
        if (!\in_array($context, ['com_academy.category', 'com_workflow.stage', 'com_workflow.workflow'])) {
            return;
        }
    ...
    public function onSchemaBeforeCompileHead(BeforeCompileHeadEvent $event): void
    {
        ...
        if ($extension === 'com_academy' && $this->params->get('schema_content', 1)) {
    ...
    /**
     * Inject com_academy schemas if needed
     *
     * @param   string    $context  The com_academy context like com_academy.post.5
    ...
    private function injectContentSchema(string $context, Registry $schema)
    {
        ...
        $component = $this->getApplication()->bootComponent('com_academy')->getMVCFactory();
        ...
        if ($view == 'post' && $id > 0) {
            ...
            $additionalSchemas = $cache->get(function ($view, $id) use ($component, $baseId, $app, $db) {
                ...
                $schemaId = $baseId . 'com_academy/' . $view . ($view == 'category' ? '/' . $id : '');
    ...
    {
        $baseId   = Uri::root() . '#/schema/';
        $schemaId = $baseId . 'com_academy/post/' . (int) $post->id;
    ...
    private function canDeleteCategories($data)
    {
        ...
        $tableInfo = [
            ...
            'com_academy'   => ['table_name' => '#__academy'],
    ...
    private function checkMenuItemBeforeSave($context, $table, $isNew, $data)
    {
        ...
        if ($table->link !== 'index.php?option=com_academy&view=form&layout=edit') {
            return true;
        }
        ...
        if (isset($params['enable_category']) && $params['enable_category'] === 1 && empty($params['catid'])) {
            $table->setError($this->getApplication()->getLanguage()->_('COM_ACADEMY_CREATE_POST_ERROR'));
