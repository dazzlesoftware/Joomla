...
namespace Joomla\Plugin\Academy\Confirmconsent\Extension;
...
final class ConfirmConsent extends CMSPlugin implements SubscriberInterface
{
    ...
    public function onContentPrepareForm(PrepareFormEvent $event)
    {
        ...
        // Get the consent box Text & the selected privacypost
        $consentboxText  = (string) $this->params->get(
            'consentbox_text',
            $this->getApplication()->getLanguage()->_('PLG_ACADEMY_CONFIRMCONSENT_FIELD_NOTE_DEFAULT')
        );
        $privacyPost  = $this->params->get('privacy_post', false);
        $privacyType     = $this->params->get('privacy_type', 'post');
			...
			<form>
				<fieldset name="default" addfieldprefix="Joomla\\Plugin\\Content\\ConfirmConsent\\Field">
					<field
						...
						label="PLG_ACADEMY_CONFIRMCONSENT_CONSENTBOX_LABEL"
						required="true"
						>
