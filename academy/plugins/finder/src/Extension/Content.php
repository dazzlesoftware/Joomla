...
namespace Joomla\Plugin\Academy\Finder\Extension;
...
/**
 * Smart Search adapter for com_academy.
 *
 * @since  2.5
 */
final class Content extends Adapter implements SubscriberInterface
{
     ...
     * @since  2.5
     */
    protected $extension = 'com_academy';
     ...
     * @since  2.5
     */
    protected $type_title = 'Academy Post';
    ...
    {
        // Make sure we're handling com_academy categories.
        if ($event->getExtension() === 'com_academy') {
    ...
    public function onFinderAfterSave(FinderEvent\AfterSaveEvent $event): void
    {
        ...
        if ($context === 'com_academy.post' || $context === 'com_academy.form') {
        ...
        if ($context === 'com_academy.category') {
            // Check if the access levels are different.
    ...
    protected function index(Result $item)
    {
        ...
        $item->context = 'com_academy.post';

        // Initialise the item parameters.
        $registry     = new Registry($item->params);
        $item->params = clone ComponentHelper::getParams('com_academy', true);
        ...
        $item->route = RouteHelper::getPostRoute($item->slug, $item->catid, $item->language);
        ...
        $images = $item->media ? json_decode($item->media) : false;

        // Add the image.
        if ($images && !empty($images->featured_image)) {
            $item->imageUrl = $images->featured_image;
            $item->imageAlt = $images->featured_image_alt ?? '';
        }
        ...
        // Translate the state. Posts should only be published if the category is published.
        ...
        $categories = $this->getApplication()->bootComponent('com_academy')->getCategory(['published' => false, 'access' => false]);
        ...
        $nativeTags = $tagDb->setQuery('SELECT t.title,t.published,t.access,t.language FROM #__academy_tags t INNER JOIN #__academy_tag_map m ON m.tag_id=t.id WHERE m.type_alias=' . $tagDb->quote('com_academy.post') . ' AND m.content_item_id=' . (int) $item->id)->loadObjectList();
        foreach ($nativeTags as $tag) {
            $item->addTaxonomy('Tag', $tag->title, (int) $tag->published, (int) $tag->access, $tag->language);
        }
        // Get content extras.
        Helper::getContentExtras($item);
        Helper::addCustomFields($item, 'com_academy.post');
    ...
    protected function getListQuery($query = null)
    {
        ...
        $query = $query instanceof QueryInterface ? $query : $db->createQuery()
            ->select('a.id, a.title, a.alias, a.summary AS summary, a.body AS body')
            ->select('a.media')
            ->select('a.state, a.catid, a.created AS start_date, a.created_by')
            ->select('a.created_by_alias, a.modified, a.modified_by, a.options AS params')
